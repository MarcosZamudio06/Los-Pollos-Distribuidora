...
const exclusivePattern = /\b(?:describe|it|test)\.(?:only|skip)\s*\(|\b(?:xdescribe|xit|xtest)\s*\(/
...
function collectTestFiles(directory) {
  return readdirSync(directory, { withFileTypes: true }).flatMap((entry) => {
    ...
    return entry.isFile() && testFilePattern.test(entry.name) && extname(entry.name)
      ? [path]
      : []
  })
}
...
if (invalidFiles.length > 0) {
  throw new Error(
    ['Focused or skipped tests are not allowed:', ...invalidFiles.map((path) => `- ${path}`)].join('\n'),
  )
}
