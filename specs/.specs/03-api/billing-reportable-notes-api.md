...
Requiere rol `ADMIN`, motivo obligatorio, encabezado `Idempotency-Key`, `expectedRemediationVersion`, `expectedSaleVersion` y `expectedDocumentVersions[]`. Cada partida enviada en `correction.items[]` incluye su `expectedVersion`. Puede recibir una corrección específica para el código o ejecutar únicamente la validación posterior si los datos fueron corregidos por otro flujo. La operación usa una transacción serializable, bloquea la remediación y la venta, compara las versiones del snapshot, aplica la corrección mediante escrituras condicionadas, reevalúa la inconsistencia y solo entonces registra la resolución y `BillingAuditLog`. Si la inconsistencia persiste, responde `REMEDIATION_STILL_PRESENT` y revierte toda la transacción. Una remediación ya resuelta responde de forma idempotente únicamente cuando coinciden la clave y el hash del payload.

Antes de una corrección monetaria, la misma transacción ejecuta el analizador de impacto sobre pagos, cuenta por cobrar, solicitudes y reservas, aplicaciones y facturas, documentos impresos, corte POS, turno de caja y liquidación de ruta. Si existe una relación cerrada, vigente o monetariamente incompatible, responde `REMEDIATION_REQUIRES_ACCOUNTING_ADJUSTMENT` con `blockers[]` de códigos estables y no modifica la venta ni sus partidas. Periodos y documentos cerrados requieren reapertura versionada o una reversión o ajuste explícito en su flujo canónico.

El listado entrega `version` para la remediación y la venta, y `version` para cada partida y documento. Para `AMBIGUOUS_SALE_DOCUMENT`, `expectedDocumentVersions[]` debe cubrir exactamente todos los documentos candidatos evaluados. Tokens faltantes, duplicados, ajenos o desactualizados responden `VERSION_CONFLICT`. Reutilizar una clave idempotente con otro comando responde `IDEMPOTENCY_CONFLICT`.

Después de aplicar la corrección, el backend ejecuta el validador canónico completo de la venta sin depender del código de remediación. Valida partidas, cabecera, base gravable, descuento autorizado, impuestos, pagos, cuenta por cobrar, solicitudes activas y aplicaciones vigentes de factura. Cualquier hallazgo responde `SALE_CONSISTENCY_VALIDATION_FAILED` con `findings[]`; no se persisten la corrección, la resolución ni la auditoría de éxito.
