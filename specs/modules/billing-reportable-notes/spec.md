...
Antes de aplicar `UNALLOCATED_ITEM_AMOUNTS` o `INVALID_SALE_TOTAL`, un `RemediationImpactAnalyzer` debe evaluar la venta bloqueada y el importe propuesto dentro de la misma transacción. La modificación directa se rechaza con `REMEDIATION_REQUIRES_ACCOUNTING_ADJUSTMENT` cuando la venta está cancelada; pertenece a un turno o corte POS cerrado; pertenece a una liquidación de ruta cerrada; dejaría pagos aplicados o la cuenta por cobrar en contradicción con el nuevo total; tiene una solicitud activa o una reserva vigente; tiene una aplicación de factura vigente por documento o partida; está relacionada con una factura `ACTIVE`, `SUBSTITUTED` o `CANCELLED`; o conserva un documento emitido o cobrado con snapshots impresos.

La compatibilidad monetaria se calcula sin reescribir historia: en venta de contado, los pagos aplicados deben coincidir con el nuevo total; en venta a crédito no pueden excederlo. Cuando existe cuenta por cobrar, su importe original debe coincidir con el nuevo total menos los pagos aplicados directamente a la venta, y su saldo debe coincidir con el importe original menos los pagos aplicados a la cuenta. Una relación incompatible bloquea la remediación; no se corrigen pagos ni cartera implícitamente.

Una venta asociada a un periodo o documento cerrado no se modifica mediante remediación. Requiere reapertura versionada cuando el flujo canónico lo permita o una reversión o ajuste explícito, auditable y enlazado con el registro original. Este módulo no implementa implícitamente ese ajuste.

El comando de resolución usa concurrencia optimista sobre el snapshot entregado a la interfaz. Requiere `expectedRemediationVersion`, `expectedSaleVersion`, la versión de cada partida monetaria afectada y la versión de todos los documentos evaluados al resolver una ambigüedad. También requiere `Idempotency-Key`; la clave y el hash canónico del comando se persisten en `BillingDataRemediation`. Una repetición con la misma clave y payload devuelve la resolución previa, mientras que la misma clave con otro payload responde `IDEMPOTENCY_CONFLICT`.

Toda comparación ocurre después de bloquear las filas y antes de escribir. Una diferencia responde `VERSION_CONFLICT` sin cambios parciales. Las actualizaciones de `Sale`, `SaleItem`, `SaleDocument` y `BillingDataRemediation` se condicionan por su versión esperada e incrementan esa versión. La cobertura de versiones debe coincidir exactamente con las partidas o documentos afectados; no se aceptan tokens faltantes, duplicados o ajenos a la venta.
