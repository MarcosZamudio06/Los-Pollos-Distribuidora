...
- Fleet keeps `VehiclePosition` online for 365 days by default, configurable through `FLEET_POSITION_RETENTION_DAYS`. The daily retention job deletes only positions older than the cutoff that are not referenced by `GeofenceEvent` or `DeliveryIncident`; mutable `VehicleGeofenceState.lastPositionId` pointers are cleared before deletion. External archival is not part of the current contract, so positions older than the configured window are not available through the Fleet history endpoint.
