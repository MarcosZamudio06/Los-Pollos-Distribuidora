...
function buildLineSubtotal(line: ProductLine): number {
  return roundMoney(
    line.unitPrice *
      (line.quantityKg > 0 ? line.quantityKg : line.quantityPieces),
  );
}

function buildSaleItemLines(
  products: BaseProduct[],
  saleIndex: number,
  saleChannel: SaleChannel,
): ProductLine[] {
  const whole =
    products.find((product) => product.sku === 'DEV-WHOLE-CHICKEN-KG') ??
    products[0];
  const breast =
    products.find((product) => product.sku === 'DEV-BREAST-KG') ?? products[1];
  const wings =
    products.find((product) => product.sku === 'DEV-WINGS-PIECE') ??
    products[2];
...
function buildSalesPlans(ctx: SeedContext, baseDate: Date): SalePlan[] {
  const retailCustomers = seedCustomers.filter(
    (customer) => customer.customerType === 'RETAIL',
  );
  const wholesaleCustomers = seedCustomers.filter(
    (customer) => customer.customerType === 'WHOLESALE',
  );
  const institutionalCustomers = seedCustomers.filter(
    (customer) => customer.customerType === 'INSTITUTIONAL',
  );
  const routeCustomers = seedCustomers.filter(
    (customer) => customer.assignedRouteId,
  );
  ...
  for (let day = 0; day < SALES_DAYS; day += 1) {
    ...
    const branchLocation = ctx.branchLocationsByCode.get(
      branchCodes[day % branchCodes.length],
    );
    const routeLocation = route.routeStockLocationId;
    const externalLocation =
      externalLocations[day % externalLocations.length].id;

    const daySales: Array<
      Pick<
        SalePlan,
        | 'saleChannel'
        | 'paymentType'
    ...
    > = [
      {
        ...
        customerType:
          retailCustomers[day % retailCustomers.length].customerType,
        customerName: retailCustomers[day % retailCustomers.length].name,
        commercialPolicyId:
          retailCustomers[day % retailCustomers.length].commercialPolicyId,
        routeId: null,
        documentType:
          day % 3 === 0
            ? SaleDocumentType.SIMPLE_NOTE
            : SaleDocumentType.INTERNAL_RECEIPT,
        requiresAdministrativeInvoice: false,
        initialPaymentAmount: 0,
        initialPaymentMethod:
          day % 2 === 0 ? PaymentMethod.CASH : PaymentMethod.CARD,
      ...
      },
      {
        ...
        customerId:
          day % 4 === 0
            ? retailCustomers[(day + 3) % retailCustomers.length].id
            : day % 5 === 0
              ? null
              : retailCustomers[(day + 3) % retailCustomers.length].id,
        customerType:
          day % 4 === 0
            ? retailCustomers[(day + 3) % retailCustomers.length].customerType
            : day % 5 === 0
              ? null
              : retailCustomers[(day + 3) % retailCustomers.length]
                  .customerType,
        customerName:
          day % 4 === 0
            ? retailCustomers[(day + 3) % retailCustomers.length].name
            : day % 5 === 0
              ? null
              : retailCustomers[(day + 3) % retailCustomers.length].name,
        commercialPolicyId:
        ...
        initialPaymentAmount: 0,
        initialPaymentMethod:
          day % 4 === 0 ? PaymentMethod.TRANSFER : PaymentMethod.CASH,
      ...
      },
      {
        ...
        customerType:
          routeCustomers[(day + 1) % routeCustomers.length].customerType,
        customerName: routeCustomers[(day + 1) % routeCustomers.length].name,
        commercialPolicyId:
          routeCustomers[(day + 1) % routeCustomers.length].commercialPolicyId,
        routeId: route.id,
        documentType: SaleDocumentType.SCALE_TICKET,
        requiresAdministrativeInvoice:
          routeCustomers[(day + 1) % routeCustomers.length].requiresBilling,
        initialPaymentAmount: 0,
        initialPaymentMethod:
          day % 10 === 0 ? PaymentMethod.CASH : PaymentMethod.TRANSFER,
      ...
      },
      {
        ...
        locationId:
          branchLocation?.id ??
          ctx.branchLocationsByCode.values().next().value.id,
        customerId: wholesaleCustomers[day % wholesaleCustomers.length].id,
        customerType:
          wholesaleCustomers[day % wholesaleCustomers.length].customerType,
        customerName: wholesaleCustomers[day % wholesaleCustomers.length].name,
        commercialPolicyId:
          wholesaleCustomers[day % wholesaleCustomers.length]
            .commercialPolicyId,
        routeId: null,
        documentType: SaleDocumentType.LARGE_NOTE,
        requiresAdministrativeInvoice:
          wholesaleCustomers[day % wholesaleCustomers.length].requiresBilling,
        initialPaymentAmount: 0,
      ...
      },
      {
        saleChannel:
          day % 2 === 0
            ...
            : SaleChannel.EXTERNAL_POINT_OF_SALE,
        paymentType:
          day % 2 === 0
            ? SalePaymentType.CREDIT_SALE
            : SalePaymentType.CASH_SALE,
        locationId:
          day % 2 === 0
            ? (branchLocation?.id ?? firstBranch.id)
            : externalLocation,
        customerId:
          day % 2 === 0
            ? institutionalCustomers[day % institutionalCustomers.length].id
            : retailCustomers[(day + 7) % retailCustomers.length].id,
        customerType:
          day % 2 === 0
            ? institutionalCustomers[day % institutionalCustomers.length]
                .customerType
            : retailCustomers[(day + 7) % retailCustomers.length].customerType,
        customerName:
          day % 2 === 0
            ? institutionalCustomers[day % institutionalCustomers.length].name
            : retailCustomers[(day + 7) % retailCustomers.length].name,
        commercialPolicyId:
          day % 2 === 0
            ? institutionalCustomers[day % institutionalCustomers.length]
                .commercialPolicyId
            : retailCustomers[(day + 7) % retailCustomers.length]
                .commercialPolicyId,
        routeId: null,
        documentType:
          day % 4 === 0
    ...
    daySales.forEach((sale, slot) => {
      ...
      const items = buildSaleItemLines(
        Array.from(ctx.productsBySku.values()),
        index,
        sale.saleChannel,
      );
      const subtotal = roundMoney(
        items.reduce((sum, item) => sum + buildLineSubtotal(item), 0),
      );
      const discount =
        sale.saleChannel === SaleChannel.WHOLESALE
          ? roundMoney((index % 3) * 5)
          : 0;
      const total = roundMoney(subtotal - discount);
      const expectedOutstanding =
        sale.paymentType === SalePaymentType.CASH_SALE ? 0 : total;
      ...
      plans.push({
        ...
        physicalFolio:
          sale.documentType === SaleDocumentType.SCALE_TICKET
            ? (sale.scaleTicketFolio ?? `STK-${index}`)
            : null,
        requiresAdministrativeInvoice: sale.requiresAdministrativeInvoice,
        userId:
          sale.saleChannel === SaleChannel.ROUTE
            ? ctx.driverUser.id
            : ctx.sellerUser.id,
        discount,
        items,
        initialPaymentAmount:
          sale.paymentType === SalePaymentType.CASH_SALE ? total : 0,
        initialPaymentMethod:
          sale.paymentType === SalePaymentType.CASH_SALE
            ? sale.initialPaymentMethod
            : null,
        initialPaymentAt:
          sale.paymentType === SalePaymentType.CASH_SALE ? dayBase : null,
        expectedOutstanding,
...
function buildPurchasePlans(ctx: SeedContext, baseDate: Date): PurchasePlan[] {
  ...
  for (let index = 1; index <= PURCHASE_COUNT; index += 1) {
    const purchaseDate = addDays(
      baseDate,
      Math.floor((index - 1) * (SALES_DAYS / PURCHASE_COUNT)),
    );
...
function buildDailyClosePlans(
  ...
  for (let index = 1; index <= DAILY_CLOSE_COUNT; index += 1) {
    plans.push({
      ...
      businessDate: addDays(
        baseDate,
        SALES_DAYS - DAILY_CLOSE_COUNT + index - 1,
      ),
...
async function loadSeedContext(prisma: PrismaClient): Promise<SeedContext> {
  ...
  if (
    ...
    throw new Error(
      'Missing base seed users. Run backend/prisma/seed.ts first.',
    );
  }
  ...
  if (products.length !== 3) {
    throw new Error(
      'Missing base seed products. Run backend/prisma/seed.ts first.',
    );
  }
  ...
  return {
    ...
    collectionsUser: {
      id: collectionsUser.id,
      email: collectionsUser.email,
      name: collectionsUser.name,
    },
    branchLocationsByCode: new Map(
...
async function deleteOperationalData(
  ...
  dailyClosePlans: DailyClosePlan[],
) {
  const saleIds = salePlans.map((plan) => plan.id);
  const deliveryOrderIds = salePlans
    .filter((plan) => plan.saleChannel === SaleChannel.ROUTE)
    .map((plan) => seedId(`delivery-order-${plan.saleNumber}`));
  ...
  const externalPosIds = seedExternalPosLocations.map(
    (location) => location.id,
  );
  await prisma.deliveryEvidence.deleteMany({
    where: { deliveryOrderId: { in: deliveryOrderIds } },
  });
  ...
  await prisma.scaleTicketReference.deleteMany({
    where: { saleId: { in: saleIds } },
  });
  ...
  await prisma.pointOfSaleDailyCloseLine.deleteMany({
    where: { pointOfSaleDailyCloseId: { in: dailyCloseIds } },
  });
  ...
  await prisma.pointOfSaleDailyClose.deleteMany({
    where: { id: { in: dailyCloseIds } },
  });

  await prisma.routeSettlement.deleteMany({
    ...
    where: {
      locationId: {
        in: [
          ...routeStockIds,
          ...externalPosIds,
          ...Array.from(ctx.branchLocationsByCode.values()).map(
            (location) => location.id,
          ),
        ],
      },
  ...
  await prisma.operationalLocation.deleteMany({
    where: { id: { in: [...routeStockIds, ...externalPosIds] } },
  });
  await prisma.customer.deleteMany({
    where: { id: { in: seedCustomers.map((customer) => customer.id) } },
  });
  await prisma.supplier.deleteMany({
    where: { id: { in: seedSuppliers.map((supplier) => supplier.id) } },
  });
  await prisma.commercialPolicy.deleteMany({
    where: { id: { in: seedCommercialPolicies.map((policy) => policy.id) } },
  });
}

async function upsertCommercialPolicies(
  prisma: PrismaClient,
  ctx: SeedContext,
) {
...
async function upsertOperationalLocations(
  prisma: PrismaClient,
  ctx: SeedContext,
) {
  const routeStockRecords = seedRouteStockLocations.map((location) => ({
    ...
    code: location.code,
    parentId:
      ctx.branchLocationsByCode.get(location.parentIdByCode)?.id ?? null,
  ...
  ctx.routeStockLocations = new Map(
    routeStockRecords.map((location) => [
      location.id,
      { id: location.id, code: location.code ?? location.name },
    ]),
  );
  ctx.externalPosLocations = new Map(
    externalPosRecords.map((location) => [
      location.id,
      { id: location.id, code: location.code ?? location.name },
    ]),
  );
...
async function createInventoryMovement(
  ...
  await tx.inventoryMovement.create({
    data: {
      ...
      type,
      quantity:
        change.quantityKg > 0 ? change.quantityKg : change.quantityPieces,
...
async function seedPurchases(
  ...
  for (const plan of plans) {
    await prisma.$transaction(
      async (tx) => {
        const subtotal = roundMoney(
          plan.items.reduce((sum, item) => sum + buildLineSubtotal(item), 0),
        );
        ...
        await tx.purchaseItem.createMany({
          data: plan.items.map((item, itemIndex) => ({
            ...
            quantity:
              item.quantityKg > 0 ? item.quantityKg : item.quantityPieces,
        ...
        for (const [itemIndex, item] of plan.items.entries()) {
          const product = ctx.productsBySku.get(
      ...
      async (tx) => {
          ...
          data: {
            ...
            items: {
              create: plan.items.map((item, itemIndex) => ({
                id: seedId(
                  `transfer-item-${plan.transferNumber}-${itemIndex + 1}`,
                ),
                productId: item.productId,
                quantityKg: item.quantityKg > 0 ? item.quantityKg : null,
                quantityPieces:
                  item.quantityPieces > 0 ? item.quantityPieces : null,
                unit: item.unit,
        ...
        await tx.inventoryTransfer.update({
          ...
          data: {
            ...
            confirmedAt: new Date(plan.requestedAt.getTime() + 15 * 60 * 1000),
          },
        });
      },
      { isolationLevel: Prisma.TransactionIsolationLevel.Serializable },
...
async function seedSales(
  ...
  const creditReceivables: Array<{
    salePlan: SalePlan;
    receivableId: string;
    dueDate: Date;
    outstanding: number;
  }> = [];
  const routeSales: Array<{
    saleId: string;
    routeId: string;
    amount: number;
    paidAt: Date;
  }> = [];
  const scaleTicketPlans: Array<{
    saleId: string;
    locationId: string;
    saleDate: Date;
    ...
    await prisma.$transaction(
      async (tx) => {
        ...
        const collectionStatus =
          plan.paymentType === SalePaymentType.CASH_SALE
        ...
        await tx.saleDocument.create({
          data: {
            ...
            requiresAdministrativeInvoice: plan.requiresAdministrativeInvoice,
            deliveredByUserId:
              plan.saleChannel === SaleChannel.ROUTE ? plan.userId : null,
        ...
        if (plan.paymentType === SalePaymentType.CASH_SALE) {
          ...
          const customer = seedCustomers.find(
            (candidate) => candidate.id === plan.customerId,
          );
          if (!customer) {
          ...
          await tx.accountReceivable.create({
            data: {
              ...
              paidAt: null,
              cancelledAt: null,
              agingStatus: resolveAgingStatus(
                dueDate,
                plan.saleDate,
                outstanding,
              ),
        ...
        if (plan.saleChannel === SaleChannel.ROUTE) {
          routeSales.push({
            saleId: plan.id,
            routeId: plan.routeId ?? seedRoutes[0].id,
            amount: total,
            paidAt: plan.saleDate,
          });
        }

        if (
          plan.saleChannel === SaleChannel.ROUTE &&
          plan.documentType === SaleDocumentType.SCALE_TICKET &&
          plan.scaleTicketFolio
        ) {
          scaleTicketPlans.push({
            saleId: plan.id,
            locationId: plan.locationId,
            saleDate: plan.saleDate,
            physicalFolio: plan.scaleTicketFolio,
          });
        }
      },
      { isolationLevel: Prisma.TransactionIsolationLevel.Serializable },
  ...
  await seedDeliveryOrders(
    prisma,
    ctx,
    plans.filter((plan) => plan.saleChannel === SaleChannel.ROUTE),
  );
...
async function seedCreditCollections(
  ...
  for (const [index, receivable] of collectionPayments.entries()) {
    const payDate =
      receivable.dueDate > receivable.salePlan.saleDate
        ? addDays(receivable.salePlan.saleDate, 3 + (index % 6))
        : addDays(receivable.salePlan.saleDate, 1);
    const paymentAmount =
      index % 4 === 0
        ? roundMoney(receivable.outstanding)
        : roundMoney(receivable.outstanding * 0.6);
    const paymentId = seedId(
      `collection-${receivable.salePlan.saleNumber}-${index + 1}`,
    );
    ...
    await prisma.$transaction(
      async (tx) => {
        ...
        const newOutstandingAmount = roundMoney(
          toNumber(current.outstandingAmount) - paymentAmount,
        );
        const nextStatus =
        ...
        const agingStatus = resolveAgingStatus(
          current.dueDate,
          payDate,
          newOutstandingAmount,
        );
        ...
        await tx.payment.create({
          data: {
            ...
            operationalLocationId:
              seedRoutes[index % seedRoutes.length].routeStockLocationId,
      ...
      data: {
        ...
        accountReceivableId:
          sale.paymentType === SalePaymentType.CREDIT_SALE
            ? seedId(`receivable-${sale.saleNumber}`)
            : null,
        status: [
        ...
        deliveryAddress: sale.customerName
          ? `Entrega a ${sale.customerName}`
          : 'Entrega ruta',
        ...
        collectionPass:
          sale.paymentType === SalePaymentType.CREDIT_SALE ? 1 : null,
      },
    });
  }
}

async function seedScaleTickets(
  prisma: PrismaClient,
  ctx: SeedContext,
  scaleTickets: Array<{
    saleId: string;
...
async function seedRouteSettlements(
  prisma: PrismaClient,
  ctx: SeedContext,
  routeSales: Array<{
    saleId: string;
  ...
  const byRoute = new Map<
    string,
    Array<{ saleId: string; amount: number; paidAt: Date }>
  >();
  ...
  for (const [index, [routeId, sales]] of entries.entries()) {
    ...
    const expectedCashAmount = roundMoney(
      sales
        ...
        .reduce((sum, sale) => sum + sale.amount, 0) * 0.5,
    );
...
async function seedDailyCloses(
  prisma: PrismaClient,
  ctx: SeedContext,
  dailyClosePlans: DailyClosePlan[],
) {
      ...
      async (tx) => {
        await tx.pointOfSaleDailyClose.create({
          data: {
            ...
            cashCountedTotal: roundMoney(
              7000 + index * 650 + (index % 2 === 0 ? 0 : 35),
            ),
            ...
            notes: seedNotes(
              `Daily close ${plan.businessDate.toISOString().slice(0, 10)}`,
            ),
