...
describe('UsersController employee API', () => {
  ...
  let service: jest.Mocked<Pick<UsersService, 'findAll' | 'create'>>;
  beforeEach(async () => {
    service = {
      findAll: jest
        .fn()
        .mockResolvedValue({ items: [employee], total: 1, page: 1, limit: 20 }),
  ...
  it('creates only valid employees and exposes temporary password only at creation', async () => {
    ...
    await request(app.getHttpServer())
      ...
      .expect(201)
      .expect(({ body }) => {
        expect(body.data.temporaryPassword).toBe('temporary-secret');
    ...
    await request(app.getHttpServer())
      .post('/api/users')
      .set('Authorization', 'Bearer admin-token')
      .send({ ...payload, phone: 'bad' })
      .expect(400);
  });
  it('forwards combined pagination filters for admins', async () => {
    await request(app.getHttpServer())
      .get(
        '/api/users?search=ana&roleId=role-1&operationalLocationId=location-1&status=inactive&page=2&limit=10',
      )
      .set('Authorization', 'Bearer admin-token')
      .expect(200)
      .expect(({ body }) => {
