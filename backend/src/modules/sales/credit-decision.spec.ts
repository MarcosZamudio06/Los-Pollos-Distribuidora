...
describe('evaluateCreditDecision', () => {
  ...
  const evaluate = (overrides: Record<string, unknown> = {}) =>
    evaluateCreditDecision(tx as never, {
      customer,
  ...
  it.each([CreditStatus.BLOCKED, CreditStatus.SUSPENDED])(
    'never overrides administrative status %s',
    async (creditStatus) => {
      await expect(
        evaluate({
        ...
        response: expect.objectContaining({
          code: 'CREDIT_ADMINISTRATIVELY_BLOCKED',
        }),
      });
    },
  );
  ...
  it.each(['future', 'expired'])(
    'treats a %s policy as ineligible null-mode',
    async () => {
      ...
      expect(tx.commercialPolicy.findFirst).toHaveBeenCalledWith(
  ...
  it('permits WARN_ONLY with a warning and null mode without an overdue warning', async () => {
    ...
    tx.commercialPolicy.findFirst.mockResolvedValueOnce({
      ...policy,
  ...
  it('preserves hard credit-limit denial with a stable code', async () => {
    tx.accountReceivable.findMany.mockResolvedValue([
      {
        dueDate: new Date('2026-08-01T06:00:00Z'),
        outstandingAmount: 900,
        status: CollectionStatus.UNPAID,
      },
    ]);
    await expect(evaluate()).rejects.toMatchObject({
      response: expect.objectContaining({ code: 'CREDIT_LIMIT_EXCEEDED' }),
    });
  });
  ...
  it('accepts only an authorized policy-backed ADMIN override for overdue and limit', async () => {
    ...
    await expect(
      evaluate({ overrideReason: 'Requested' }),
    ).rejects.toBeInstanceOf(BadRequestException);
    await expect(
      evaluate({
        actor: { id: 'admin-1', role: 'ADMIN' },
...
describe('calculateCreditState', () => {
  ...
  it.each([
    ...
    'derives %s overdue policy consistently',
    (overdueBlockingMode, effectiveCreditStatus, blockingReasons) => {
      expect(
        calculateCreditState({
          ...base,
          policy:
  ...
  it('prioritizes administrative status and derives limit blocking', () => {
    expect(
      ...
      blockingReasons: ['CREDIT_ADMINISTRATIVELY_BLOCKED'],
      canAdministrativeOverride: false,
    });
    expect(
      calculateCreditState({ ...base, creditLimit: 100, policy: null }),
    ).toMatchObject({
      effectiveCreditStatus: 'BLOCKED',
      blockingReasons: ['CREDIT_LIMIT_EXCEEDED'],
      canAdministrativeOverride: false,
    });
  });
