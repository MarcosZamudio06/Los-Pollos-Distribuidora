...
describe('InvoiceCancellationService', () => {
  ...
  it('cancels atomically, reverses applications, and audits the released balance', async () => {
    await service.cancel(
      'invoice-1',
      { expectedVersion: 3, reason: 'Customer correction' },
      actor,
      'cancel-key-1',
    );

    expect(tx.invoiceSaleItemApplication.updateMany).toHaveBeenCalledWith(
      expect.objectContaining({
        where: {
          invoiceSaleDocumentId: { in: ['application-1'] },
          reversedAt: null,
        },
        data: expect.objectContaining({
          reversedByUserId: actor.id,
          reversalReason: 'Customer correction',
        }),
      }),
    );
    expect(tx.invoiceSaleDocument.updateMany).toHaveBeenCalledWith(
      expect.objectContaining({
  ...
  it('returns the original result for an identical idempotent retry', async () => {
    ...
    tx.invoice.findFirst.mockResolvedValue({
      ...invoice,
      status: InvoiceStatus.CANCELLED,
      cancellationPayloadHash: firstHash,
    });
  ...
  it('rejects stale versions and incompatible substitution chains', async () => {
    ...
    await expect(
      service.cancel(
        'invoice-1',
        { expectedVersion: 3, reason: 'Correction' },
        actor,
        'key-a',
      ),
