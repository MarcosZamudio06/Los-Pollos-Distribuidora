...
describe('billing balance database guards', () => {
  const migration = readFileSync(
    resolve(
      process.cwd(),
      'prisma/migrations/20260718234500_billing_business_rules_phase2/migration.sql',
    ),
    'utf8',
  );
  ...
  it('serializes request and invoice balance checks per sale document', () => {
    expect(migration).toContain(
      'pg_advisory_xact_lock(hashtextextended(NEW."saleDocumentId", 0))',
    );
  });
  ...
  it('rejects invoice applications above the remaining document balance', () => {
    expect(migration).toContain(
      'active_invoiced + NEW."totalApplied" > available_total',
    );
...
describe('billing request invoice application database guard', () => {
  const migration = readFileSync(
    resolve(
      process.cwd(),
      'prisma/migrations/20260719023000_link_invoice_application_to_request/migration.sql',
    ),
    'utf8',
  );
  ...
  it('requires the application and request relation to reference the same sale document', () => {
    ...
    expect(migration).toContain(
      "MESSAGE = 'BILLING_REQUEST_DOCUMENT_MISMATCH'",
    );
  });
...
describe('billing request sale item database guard', () => {
  const migration = readFileSync(
    resolve(
      process.cwd(),
      'prisma/migrations/20260720090000_normalize_billing_request_sale_items/migration.sql',
    ),
    'utf8',
  );
  ...
  it('serializes reservations per sale item and rejects duplicate active reservations', () => {
    expect(migration).toContain(
      'pg_advisory_xact_lock(hashtextextended(NEW."saleItemId", 0))',
    );
  ...
  it('bounds item applications by the authorized subtotal, tax, and total', () => {
    expect(migration).toContain(
      'consumed_subtotal + NEW."subtotalApplied" > requested_subtotal',
    );
    expect(migration).toContain(
      'consumed_tax + NEW."taxApplied" > requested_tax',
    );
    expect(migration).toContain(
      'consumed_total + NEW."totalApplied" > requested_total',
    );
    expect(migration).toContain(
      "MESSAGE = 'BILLING_REQUEST_ITEM_AMOUNT_EXCEEDED'",
    );
  });
});
