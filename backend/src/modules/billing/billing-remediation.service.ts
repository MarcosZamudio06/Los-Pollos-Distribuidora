...
const saleInclude = {
  items: true,
  payments: {
    select: { status: true, amount: true, accountReceivableId: true },
  },
  accountReceivable: {
    include: {
      payments: {
...
type SaleForRemediation = Prisma.SaleGetPayload<{
...
export class BillingRemediationService {
  ...
  async list(query: BillingRemediationQueryDto) {
    ...
    return {
      items: items.map((item) => ({
        ...item,
        sale: salesById.get(item.entityId) ?? null,
      })),
      pagination: {
        page: query.page,
        limit: query.limit,
  ...
  async resolve(
    id: string,
    dto: ResolveBillingRemediationDto,
    actor: Actor,
    ...
    const payloadHash = this.hashPayload({
      ...
      expectedDocumentVersions: dto.expectedDocumentVersions,
    ...
    try {
      return await this.prisma.$transaction(
        async (tx) => {
          const replay = await tx.billingDataRemediation.findFirst({
            where: { resolutionIdempotencyKey: idempotencyKey },
          });
          if (replay) {
            if (replay.resolutionPayloadHash !== payloadHash)
              throw new ConflictException('IDEMPOTENCY_CONFLICT');
            return replay;
          }

          await tx.$queryRaw`SELECT "id" FROM "BillingDataRemediation" WHERE "id" = ${id} FOR UPDATE`;
          const remediation = await tx.billingDataRemediation.findUnique({
            where: { id },
          });
          if (!remediation)
            throw new NotFoundException('Billing remediation not found');
          if (remediation.version !== dto.expectedRemediationVersion)
            throw new ConflictException('VERSION_CONFLICT');
          if (remediation.resolvedAt)
            throw new ConflictException('REMEDIATION_ALREADY_RESOLVED');
          if (remediation.entityType !== 'Sale')
            throw new BadRequestException('UNSUPPORTED_REMEDIATION_ENTITY');

          await tx.$queryRaw`SELECT "id" FROM "Sale" WHERE "id" = ${remediation.entityId} FOR UPDATE`;
          let sale = await tx.sale.findUnique({
            where: { id: remediation.entityId },
            include: saleInclude,
          });
          if (!sale) throw new NotFoundException('Remediation sale not found');
          if (sale.version !== dto.expectedSaleVersion)
            throw new ConflictException('VERSION_CONFLICT');
          ...
          if (this.isInconsistencyPresent(remediation.code, sale))
            throw new ConflictException('REMEDIATION_STILL_PRESENT');
          const findings = this.consistencyValidator.validate(sale);
          if (findings.length) {
            throw new ConflictException({
              code: 'SALE_CONSISTENCY_VALIDATION_FAILED',
              message:
                'La venta conserva inconsistencias monetarias y no puede cerrarse.',
              findings,
            });
          }
          ...
          const resolution = await tx.billingDataRemediation.updateMany({
            where: {
              id,
              version: dto.expectedRemediationVersion,
              resolvedAt: null,
            },
            data: {
              resolvedAt,
          ...
          if (resolution.count !== 1)
            throw new ConflictException('VERSION_CONFLICT');
          const resolved = await tx.billingDataRemediation.findUniqueOrThrow({
            where: { id },
          });
          await tx.billingAuditLog.create({
            data: {
              ...
              context: {
                ...
                expectedRemediationVersion: dto.expectedRemediationVersion,
                expectedSaleVersion: dto.expectedSaleVersion,
                expectedDocumentVersions: dto.expectedDocumentVersions.map(
                  (document) => ({
      ...
      if (
        ...
        const replay = await this.prisma.billingDataRemediation.findFirst({
          where: { resolutionIdempotencyKey: idempotencyKey },
        });
        if (!replay)
          throw new ConflictException('CONCURRENT_REMEDIATION_RESOLUTION');
        if (replay.resolutionPayloadHash !== payloadHash)
          throw new ConflictException('IDEMPOTENCY_CONFLICT');
        return replay;
      }
  ...
  private assertDocumentVersions(
    code: string,
    sale: SaleForRemediation,
    dto: ResolveBillingRemediationDto,
  ): void {
    const expected = new Map(
      dto.expectedDocumentVersions.map((document) => [
        document.saleDocumentId,
        document.expectedVersion,
      ]),
    );
    if (expected.size !== dto.expectedDocumentVersions.length)
      throw new ConflictException('VERSION_CONFLICT');
    const affected =
      code === 'AMBIGUOUS_SALE_DOCUMENT'
        ? sale.documents.filter(
            (document) =>
              document.documentType === sale.documentType &&
              document.status !== SaleDocumentStatus.CANCELLED,
          )
        : [];
    if (
      affected.length !== expected.size ||
      affected.some(
        (document) => expected.get(document.id) !== document.version,
      )
    ) {
      throw new ConflictException('VERSION_CONFLICT');
    }
  ...
  private assertMonetaryCorrectionAllowed(
    code: string,
    sale: SaleForRemediation,
    dto: ResolveBillingRemediationDto,
  ): void {
    if (
      !dto.correction ||
      !['UNALLOCATED_ITEM_AMOUNTS', 'INVALID_SALE_TOTAL'].includes(code)
    )
      return;
    const proposedTotal =
      code === 'INVALID_SALE_TOTAL' && dto.correction.total !== undefined
    ...
    if (blockers.length) {
      throw new ConflictException({
        ...
        message:
          'La venta tiene relaciones contables que impiden una corrección monetaria directa.',
        blockers,
      });
    }
  }
  ...
  private async applyCorrection(
    ...
    if (code === 'MISSING_LEGAL_ENTITY_MAPPING' && correction.legalEntityId) {
      const legalEntity = await tx.legalEntity.findFirst({
        where: { id: correction.legalEntityId, isActive: true },
      });
      if (!legalEntity)
        throw new BadRequestException('LEGAL_ENTITY_NOT_ACTIVE');
      const updated = await tx.sale.updateMany({
        where: { id: sale.id, version: dto.expectedSaleVersion },
        data: { legalEntityId: legalEntity.id, version: { increment: 1 } },
      });
    ...
    if (
      code === 'AMBIGUOUS_SALE_DOCUMENT' &&
      correction.selectedSaleDocumentId
    ) {
      const matching = sale.documents.filter(
        (document) =>
          document.documentType === sale.documentType &&
          document.status !== SaleDocumentStatus.CANCELLED,
      );
      const selected = matching.find(
        (document) => document.id === correction.selectedSaleDocumentId,
      );
      if (!selected)
        throw new BadRequestException('SELECTED_DOCUMENT_NOT_ELIGIBLE');
      ...
      if (
        duplicates.some(
          (document) =>
            document.billingRequestDocuments.length ||
            document.invoiceDocuments.length,
        )
      ) {
    ...
    if (code === 'UNALLOCATED_ITEM_AMOUNTS' && correction.items) {
      ...
      if (
        correction.items.length !== expectedIds.size ||
        correction.items.some((item) => !expectedIds.delete(item.saleItemId)) ||
        expectedIds.size
      ) {
      ...
      for (const item of correction.items) {
        const current = sale.items.find(
          (saleItem) => saleItem.id === item.saleItemId,
        );
        ...
        const updated = await tx.saleItem.updateMany({
          where: { id: item.saleItemId, version: item.expectedVersion },
          data: {
            subtotal: item.subtotal,
            discount: item.discount,
            taxableBase: new Prisma.Decimal(item.subtotal).minus(item.discount),
            tax: item.tax,
            total: item.total,
            version: { increment: 1 },
          },
    ...
    if (
      ...
      correction.subtotal !== undefined &&
      correction.discount !== undefined &&
      correction.tax !== undefined &&
      correction.total !== undefined
    ) {
      this.assertValidAmounts(
        correction.subtotal,
        correction.discount,
  ...
  private isInconsistencyPresent(
    ...
    if (code === 'INVALID_SALE_TOTAL')
      return (
        this.money(sale.total) <= 0 ||
        this.money(sale.subtotal) -
          this.money(sale.discount) +
          this.money(sale.tax) !==
          this.money(sale.total)
      );
    if (code === 'UNALLOCATED_ITEM_AMOUNTS') {
      if (!sale.items.length) return true;
      const sums = sale.items.reduce(
        (acc, item) => ({
          subtotal: acc.subtotal + this.money(item.subtotal),
          discount: acc.discount + this.money(item.discount),
          tax: acc.tax + this.money(item.tax),
          total: acc.total + this.money(item.total),
        }),
        { subtotal: 0, discount: 0, tax: 0, total: 0 },
      );
      const invalidItem = sale.items.some(
        (item) =>
      ...
      return (
        invalidItem ||
        sums.subtotal !== this.money(sale.subtotal) ||
        sums.discount !== this.money(sale.discount) ||
        sums.tax !== this.money(sale.tax) ||
        sums.total !== this.money(sale.total)
      );
    }
    throw new BadRequestException('UNSUPPORTED_REMEDIATION_CODE');
  }

  private assertValidAmounts(
    subtotal: string,
    discount: string,
    tax: string,
    total: string,
  ) {
    const values = [subtotal, discount, tax, total].map(
      (value) => new Prisma.Decimal(value),
    );
    const taxableBase = values[0].minus(values[1]);
    if (
      values.some((value) => value.isNegative()) ||
      taxableBase.isNegative() ||
      !taxableBase.plus(values[2]).toDecimalPlaces(2).equals(values[3])
    )
      throw new BadRequestException('INVALID_AMOUNT_EQUATION');
  }
  ...
  private auditSnapshot(
    ...
    return {
      remediation: {
        ...
        resolvedByUserId: remediation.resolvedByUserId ?? null,
        resolutionNotes: remediation.resolutionNotes ?? null,
      },
      sale: {
        id: sale.id,
