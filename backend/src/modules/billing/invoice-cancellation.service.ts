    ...
    try {
      return await this.prisma.$transaction(
        async (tx) => {
          ...
          if (invoice.substitutedByInvoiceId || invoice.substitutes)
            throw new BadRequestException('INCOMPATIBLE_INVOICE_SUBSTITUTION');

          const applicationIds = invoice.documents
          ...
          await tx.invoiceSaleItemApplication.updateMany({
            where: {
              invoiceSaleDocumentId: { in: applicationIds },
              reversedAt: null,
            },
            data: {
              reversedAt,
          ...
          await tx.billingAuditLog.create({
            data: {
              ...
              before: {
                ...
                activeApplicationIds: applicationIds,
              },
              after: {
                status: cancelled.status,
                version: cancelled.version,
                reversedApplicationIds: applicationIds,
              },
              reason,
              correlationId: idempotencyKey,
              context: {
                releasedBillingBalance: true,
                reversedDocumentApplications: applicationIds.length,
              },
            },
          });
          return cancelled;
