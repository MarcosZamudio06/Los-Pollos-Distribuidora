...
describe('BillingReportService', () => {
  ...
  const row = {
    ...
    activeRequested: new Prisma.Decimal('0'),
    activeInvoiced: new Prisma.Decimal('0'),
    pendingInvoice: new Prisma.Decimal('100.00'),
    pendingSubtotal: new Prisma.Decimal('90.00'),
    pendingTax: new Prisma.Decimal('10.00'),
    pendingTotal: new Prisma.Decimal('100.00'),
    requestableItems: [],
    activePaid: new Prisma.Decimal('20.00'),
    collectionBalance: new Prisma.Decimal('80.00'),
    billingStatus: 'BILLABLE',
    blockingCodes: [],
    deliveryStatus: 'DELIVERED',
    paymentStatus: 'PARTIALLY_PAID',
    fiscalProfileComplete: true,
    deadline: new Date('2026-08-17T12:00:00Z'),
    updatedAt: new Date('2026-07-18T12:30:00Z'),
  };

  beforeEach(() => {
    raw.mockReset();
    auditCreate.mockReset();
    exporter.createFile.mockClear();
  });

  it('uses exactly one row query and one summary query for a paginated list', async () => {
    raw
      .mockResolvedValueOnce([{ ...row, totalCount: 1n }])
      .mockResolvedValueOnce([
        {
          totalDocuments: 1n,
          billableDocuments: 1n,
          blockedDocuments: 0n,
          totalBillable: new Prisma.Decimal(100),
          totalRequested: new Prisma.Decimal(0),
          totalInvoiced: new Prisma.Decimal(0),
          totalPending: new Prisma.Decimal(100),
        },
      ]);
  ...
  it('treats dateTo as an exclusive boundary at the start of the following day', async () => {
    ...
    await service.list(
      query({ dateFrom: '2026-07-01', dateTo: '2026-07-19' }),
      admin,
    );
    ...
    expect(reportQuery.values).toContainEqual(
  ...
  it('loads detail with one batched query and returns decimal strings', async () => {
    raw.mockResolvedValueOnce([
      {
        ...
        items: [],
        requests: [],
        activeInvoices: [{ id: 'invoice-active' }],
        invoiceHistory: [{ id: 'invoice-cancelled' }],
        payments: [],
        delivery: null,
      },
    ]);
    ...
    expect(sql).toContain(
      'idoc."reversedAt" IS NULL AND i."status" = \'ACTIVE\'',
    );
    expect(sql).toContain(
      'idoc."reversedAt" IS NOT NULL OR i."status" <> \'ACTIVE\'',
    );
  });

  it('throws when detail is not visible or does not exist', async () => {
    raw.mockResolvedValueOnce([]);
    await expect(service.detail('missing', admin)).rejects.toBeInstanceOf(
      NotFoundException,
    );
  });

  it('uses one bounded query for export read-model rows', async () => {
    raw.mockResolvedValueOnce([row]).mockResolvedValueOnce([
      {
        totalDocuments: 1n,
        billableDocuments: 1n,
        blockedDocuments: 0n,
        ...
        totalInvoiced: new Prisma.Decimal(0),
        totalPending: new Prisma.Decimal(100),
        totalCollected: new Prisma.Decimal(20),
        totalReceivable: new Prisma.Decimal(80),
      },
    ]);
    const result = await service.exportFile(query({ format: 'csv' }), admin);
    expect(raw).toHaveBeenCalledTimes(2);
    expect(exporter.createFile).toHaveBeenCalledWith(
  ...
  it('applies identical filters to table, summary, and export-source totals', async () => {
    const filters = {
      ...
      search: 'reconciliation-needle',
      customerId: 'customer-filter',
      billingStatus: 'BLOCKED' as const,
      hasRequest: true,
    };
    raw
      .mockResolvedValueOnce([])
    ...
    const serializedQueries = raw.mock.calls.map(([query]) =>
  ...
  it('enforces the report and export RBAC matrix in the service layer', async () => {
    ...
    await expect(
      service.list(query(), { id: 'driver-1', name: 'Driver', role: 'DRIVER' }),
    ).rejects.toBeInstanceOf(ForbiddenException);
    await expect(
      service.exportFile(query(), {
        id: 'seller-1',
        name: 'Seller',
        role: 'SELLER',
      }),
