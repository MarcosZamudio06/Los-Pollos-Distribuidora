...
describe('BillingReportExporter', () => {
  ...
  const metadata = {
    ...
    totals: {
      ...
      billableDocuments: 1,
      blockedDocuments: 0,
      totalBillable: '100.50',
      totalRequested: '0.00',
      totalInvoiced: '0.00',
      totalPending: '100.50',
      totalCollected: '40.25',
      totalReceivable: '60.25',
    },
    truncated: false,
  };
  const rows = [
    {
      ...
      saleNumber: 'V-1',
      physicalFolio: '00123',
      issuedAt: new Date('2026-07-18T12:00:00.000Z'),
      ...
      activeInvoiced: '0.00',
      pendingInvoice: '100.50',
      total: '100.50',
      locationName: 'Centro',
      sellerName: 'Vendedor Uno',
      routeName: 'Ruta Norte',
      activePaid: '40.25',
      collectionBalance: '60.25',
      blockingCodes: ['DELIVERY_PENDING', 'MISSING_FISCAL_PROFILE'],
      deadline: new Date('2026-08-17T00:00:00.000Z'),
      deliveryStatus: 'PENDING',
      currencyCode: 'MXN',
      invoiceUuids: '00000000-0000-0000-0000-000000000001',
      invoiceFolios: 'A-001',
    },
  ];
  ...
  it('serializes CSV with metadata, control totals, numeric amounts and escaped text', () => {
    ...
    expect(csv).toContain(
      'Ubicación,Vendedor,Ruta,Saldo cobrado,Saldo por cobrar,Códigos de bloqueo,Fecha límite,Estado de entrega',
    );
    expect(csv).toContain(
      'Centro,Vendedor Uno,Ruta Norte,40.25,60.25,DELIVERY_PENDING | MISSING_FISCAL_PROFILE',
    );
  ...
  it('writes XLSX using typed date, numeric and text cells', async () => {
    ...
    const headerRowNumber = sheet!
      .getColumn(1)
      .values.findIndex((value) => value === 'ID documento');
    ...
    expect(dataRow.getCell(headerRow.indexOf('Saldo cobrado')).value).toBe(
      40.25,
    );
    expect(
      dataRow.getCell(headerRow.indexOf('Fecha límite')).value,
    ).toBeInstanceOf(Date);
