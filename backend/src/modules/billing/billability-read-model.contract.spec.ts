...
const migration = read(
  'prisma/migrations/20260719053000_canonical_billability_read_model/migration.sql',
);
...
const commandService = read(
  'src/modules/billing-requests/billing-requests.service.ts',
);
...
describe('canonical billability read model contract', () => {
  ...
  it('keeps every TypeScript evaluator blocking code represented in SQL', () => {
    const evaluatorCodes = [
      ...evaluator.matchAll(/blockingCodes\.push\([^']*'([A-Z_]+)'/g),
    ].map((match) => match[1]);
    expect(evaluatorCodes.length).toBeGreaterThan(0);
    for (const code of new Set(evaluatorCodes))
  ...
  it('uses the same persisted policy and local-calendar deadline semantics', () => {
    for (const field of [
      ...
      'allowInternalReceipt',
      'requireConfirmedDelivery',
      'deadlineDays',
      'deadlineBasis',
      'timezone',
    ]) {
  ...
  it('aggregates only active, non-reversed invoice references', () => {
    const uuidAggregate =
      migration.match(
        /STRING_AGG\(DISTINCT i\."uuid"[\s\S]+?AS "invoiceUuids"/,
      )?.[0] ?? '';
    const folioAggregate =
      migration.match(
        /STRING_AGG\(DISTINCT CONCAT_WS[\s\S]+?AS "invoiceFolios"/,
      )?.[0] ?? '';
