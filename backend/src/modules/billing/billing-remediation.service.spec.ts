...
describe('BillingRemediationService', () => {
  ...
  function setup(
    ...
    const tx = {
      ...
      billingDataRemediation: {
        ...
        findUnique: jest
          .fn()
          .mockImplementation(() => Promise.resolve(currentRemediation)),
        ...
        updateMany: jest.fn().mockImplementation(({ data }) => {
          currentRemediation = {
            ...currentRemediation,
            ...data,
            version: Number(currentRemediation.version) + 1,
          };
        ...
        update: jest
          ...
          .mockImplementation(({ data }) =>
            Promise.resolve({ ...remediation, ...data }),
          ),
      },
      sale: {
        findUnique: jest
          .fn()
          .mockImplementation(() => Promise.resolve(currentSale)),
        findUniqueOrThrow: jest
          .fn()
          .mockImplementation(() => Promise.resolve(currentSale)),
        updateMany: jest.fn().mockImplementation(({ data }) => {
          currentSale = {
            ...currentSale,
            ...data,
            version: Number(currentSale.version) + 1,
          };
          return Promise.resolve({ count: 1 });
        }),
        update: jest
          .fn()
          .mockImplementation(({ data }) =>
            Promise.resolve({ ...sale, ...data }),
          ),
      },
      legalEntity: {
        findFirst: jest
          .fn()
          .mockResolvedValue({ id: 'legal-1', isActive: true }),
      },
      saleDocument: {
    ...
    return {
      service: new BillingRemediationService(
        prisma as never,
        new RemediationImpactAnalyzer(),
        new SaleConsistencyValidator(),
      ),
      tx,
    };
  }
  ...
  it('assigns an active legal entity and resolves only after validation', async () => {
    ...
    expect(tx.sale.updateMany).toHaveBeenCalledWith({
      where: { id: 'sale-1', version: 4 },
      data: { legalEntityId: 'legal-1', version: { increment: 1 } },
    });
    expect(tx.billingDataRemediation.updateMany).toHaveBeenCalledWith(
      expect.objectContaining({
        where: { id: 'rem-1', version: 2, resolvedAt: null },
    ...
    expect(tx.billingAuditLog.create).toHaveBeenCalledWith({
      data: expect.objectContaining({ action: 'BILLING_REMEDIATION_RESOLVED' }),
    });
  });
  ...
  it('does not close a remediation while the inconsistency is still present', async () => {
    ...
    const { service, tx } = setup(remediation, {
      id: 'sale-1',
      version: 4,
      legalEntityId: null,
      items: [],
    ...
    await expect(
      service.resolve(
        'rem-1',
        {
          ...
          expectedSaleVersion: 4,
          expectedDocumentVersions: [],
          reason: 'Validar corrección externa',
        },
        actor as never,
        'resolve-key-2',
      ),
    ...
    await expect(
      service.resolve(
        'rem-1',
        {
          ...
          expectedSaleVersion: 4,
          expectedDocumentVersions: [],
          reason: 'Corregir total inconsistente',
          correction: {
            subtotal: '900.00',
            discount: '0.00',
            tax: '0.00',
    ...
    await expect(
      service.resolve(
        'rem-1',
        {
          ...
          expectedSaleVersion: 4,
          expectedDocumentVersions: [
            { saleDocumentId: 'document-1', expectedVersion: 2 },
          ],
          reason: 'Choose primary document',
          correction: { selectedSaleDocumentId: 'document-1' },
        },
        actor as never,
  ...
  it('replays the same idempotent resolution without applying it twice', async () => {
    ...
    tx.billingDataRemediation.findFirst.mockImplementation(() =>
    ...
    await expect(
      service.resolve(
        'rem-1',
        {
          ...
          expectedSaleVersion: 4,
          expectedDocumentVersions: [],
          reason: 'Validate external correction',
        },
        actor as never,
      ...
      response: expect.objectContaining({
        ...
        findings: expect.arrayContaining([
          expect.objectContaining({ code: 'ITEM_TOTALS_MISMATCH' }),
        ]),
      }),
    });
    expect(tx.billingDataRemediation.updateMany).not.toHaveBeenCalled();
