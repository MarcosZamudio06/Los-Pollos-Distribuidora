...
export class BillingRequestsService {
  ...
  async create(
    ...
    if (dto.documents?.length)
      return this.createFromDocuments(dto, actor, idempotencyKey?.trim());
    if (!dto.saleId)
      throw new BadRequestException('documents or legacy saleId is required');
    try {
        ...
        async (tx) => {
          const sale = await tx.sale.findUnique({
            ...
            include: {
              ...
              documents: true,
              items: {
                select: { id: true, taxableBase: true, tax: true, total: true },
              },
              deliveryOrder: true,
            },
          });
  ...
  private async createFromDocuments(
    dto: CreateBillingRequestDto,
    actor: Actor,
    idempotencyKey?: string,
  ) {
    ...
    if (new Set(ids).size !== ids.length)
      throw new BadRequestException('documents must not contain duplicates');
    const normalized = [...documents].sort((a, b) =>
      a.saleDocumentId.localeCompare(b.saleDocumentId),
    );
    const payloadHash = createHash('sha256')
      .update(
        JSON.stringify({
          customerId: dto.customerId,
          retryOfBillingRequestId: dto.retryOfBillingRequestId ?? null,
          reason: dto.reason.trim(),
          notes: this.optionalText(dto.notes),
    ...
    try {
        ...
        async (tx) => {
          ...
          if (existing) {
            if (existing.creationPayloadHash !== payloadHash)
              throw new ConflictException(
                'Idempotency-Key was already used for a different billing request payload',
              );
            return existing;
          }
          ...
          const records = await tx.saleDocument.findMany({
            ...
            include: {
                ...
                include: {
                  ...
                  items: {
                    include: {
                      invoiceApplications: {
                        include: {
                          invoiceSaleDocument: { include: { invoice: true } },
                        },
                      },
                      billingRequestItems: {
                        where: { reversedAt: null },
                        include: {
                          billingRequestSaleDocument: {
                            include: {
                              billingRequest: { select: { status: true } },
                            },
                          },
                        },
                      },
              ...
              billingRequestDocuments: {
                where: { reversedAt: null },
                include: { billingRequest: { select: { status: true } } },
              },
              invoiceDocuments: {
                where: { reversedAt: null },
          ...
          if (records.length !== ids.length)
            throw new NotFoundException(
              'One or more sale documents were not found',
            );

          const first = records[0].sale;
          if (
            records.some((record) => record.sale.customerId !== dto.customerId)
          )
            throw new BadRequestException('MIXED_CUSTOMERS');
          if (
            records.some(
              (record) => record.sale.currencyCode !== first.currencyCode,
            )
          )
            throw new BadRequestException('MIXED_CURRENCIES');
          if (
            records.some(
              (record) => record.sale.legalEntityId !== first.legalEntityId,
            )
          )
            throw new BadRequestException('MIXED_LEGAL_ENTITIES');
          if (!first.legalEntityId)
            throw new BadRequestException('MISSING_LEGAL_ENTITY');
          ...
          if (
            actor.role === 'SELLER' &&
            records.some((record) => record.sale.userId !== actor.id)
          )
            throw new ForbiddenException(
              'SELLER can only create requests for own sales',
            );

          const requestedItemsByDocument = new Map<
            string,
            Array<{
              saleItemId: string;
          ...
          for (const item of normalized) {
            const record = records.find(
              (candidate) => candidate.id === item.saleDocumentId,
            )!;
            if (!item.items?.length)
              throw new BadRequestException('SALE_ITEM_SELECTION_REQUIRED');
            if (record.sale.status !== SaleStatus.CONFIRMED)
              throw new BadRequestException('SALE_NOT_CONFIRMED');
            if (record.status === 'CANCELLED')
              throw new BadRequestException('DOCUMENT_CANCELLED');
            ...
            if (
              subtotal.greaterThan(
                record.sale.subtotal.minus(record.sale.discount),
              ) ||
              tax.greaterThan(record.sale.tax)
            )
              throw new BadRequestException('INVALID_REQUESTED_TAX_BREAKDOWN');
            if (item.items) {
              const selectedIds = item.items.map(
                (selected) => selected.saleItemId,
              );
              if (new Set(selectedIds).size !== selectedIds.length)
                throw new BadRequestException(
                  'items must not contain duplicates',
                );
              const selectedItems = record.sale.items.filter((saleItem) =>
                selectedIds.includes(saleItem.id),
              );
              if (selectedItems.length !== selectedIds.length)
                throw new BadRequestException('INVALID_SALE_ITEM_SELECTION');
              const pendingFor = (
                ...
                const applied = saleItem.invoiceApplications
                  ...
                  .reduce(
                    (sum, application) => sum.plus(application[invoiceField]),
                  ...
                  .filter(
                    (reservation) =>
                      !['REJECTED', 'CANCELLED'].includes(
                        reservation.billingRequestSaleDocument.billingRequest
                          .status,
                      ),
                  )
                  .reduce(
                    (sum, reservation) => sum.plus(reservation[requestField]),
                    new Prisma.Decimal(0),
                  );
                return Prisma.Decimal.max(
                  new Prisma.Decimal(0),
                  original.minus(applied).minus(reserved),
                );
              };
              const requestedItems = selectedItems.map((saleItem) => ({
                saleItemId: saleItem.id,
                requestedSubtotal: pendingFor(
                  saleItem,
                  'subtotalApplied',
                  'requestedSubtotal',
                  saleItem.taxableBase,
                ),
                requestedTax: pendingFor(
                  saleItem,
                  'taxApplied',
                  'requestedTax',
                  saleItem.tax,
                ),
                requestedTotal: pendingFor(
                  saleItem,
                  'totalApplied',
                  'requestedTotal',
                  saleItem.total,
              ...
              const exactSubtotal = requestedItems.reduce(
                (sum, selected) => sum.plus(selected.requestedSubtotal),
                new Prisma.Decimal(0),
              );
              const exactTax = requestedItems.reduce(
                (sum, selected) => sum.plus(selected.requestedTax),
                new Prisma.Decimal(0),
              );
              const exactTotal = requestedItems.reduce(
                (sum, selected) => sum.plus(selected.requestedTotal),
                new Prisma.Decimal(0),
              );
              if (
                !subtotal.equals(exactSubtotal) ||
                !tax.equals(exactTax) ||
                !total.equals(exactTotal)
              )
                throw new BadRequestException(
                  'INVALID_REQUESTED_ITEM_BREAKDOWN',
                );
              requestedItemsByDocument.set(item.saleDocumentId, requestedItems);
            }
            const activeRequested = record.billingRequestDocuments
              .filter(
                (entry) =>
                  !['REJECTED', 'CANCELLED'].includes(
                    entry.billingRequest.status,
                  ),
              )
              .reduce(
                (sum, entry) => sum.plus(entry.requestedTotal),
                new Prisma.Decimal(0),
              );
            const activeInvoiced = record.invoiceDocuments
              .filter((entry) => entry.invoice.status === 'ACTIVE')
              .reduce(
                (sum, entry) => sum.plus(entry.totalApplied),
                new Prisma.Decimal(0),
              );
            try {
              validateRequestedAmount(
                total,
                record.sale.total.minus(activeRequested).minus(activeInvoiced),
              );
            } catch (error) {
              if (error instanceof BillingBalanceError)
          ...
          if (dto.retryOfBillingRequestId) {
            const previous = await tx.billingRequest.findUnique({
              where: { id: dto.retryOfBillingRequestId },
              select: { status: true, customerId: true },
            });
            if (!previous)
              throw new NotFoundException('Previous billing request not found');
            if (!['REJECTED', 'CANCELLED'].includes(previous.status))
              throw new BadRequestException(
                'Only terminal rejected or cancelled requests can be retried',
              );
            if (previous.customerId !== dto.customerId)
              throw new BadRequestException('Retry must preserve customer');
          }
          ...
          const created = await tx.billingRequest.create({
            data: {
              ...
              retryOfBillingRequestId: dto.retryOfBillingRequestId,
              history: {
                create: {
                  toStatus: BillingRequestStatus.REQUESTED,
                  changedByUserId: actor.id,
                  reason: dto.reason.trim(),
                  notes: this.optionalText(dto.notes),
                  compositionSnapshot: normalized.map((item) => ({
                    saleDocumentId: item.saleDocumentId,
                    requestedSubtotal: item.requestedSubtotal,
                    requestedTax: item.requestedTax,
          ...
          for (const item of normalized)
            await tx.billingRequestSaleDocument.create({
              data: {
                billingRequestId: created.id,
                saleDocumentId: item.saleDocumentId,
                requestedSubtotal: new Prisma.Decimal(item.requestedSubtotal),
                requestedTax: new Prisma.Decimal(item.requestedTax),
                requestedTotal: new Prisma.Decimal(item.requestedTotal),
                createdByUserId: actor.id,
                requestedItems: {
                  create:
                    requestedItemsByDocument.get(item.saleDocumentId) ?? [],
                },
              },
            });
          await this.writeAudit(
      ...
      if (
        ...
        throw new ConflictException(
          'Concurrent billing request creation is still in progress; retry with the same Idempotency-Key',
        );
      }
      throw error;
    }
  ...
  async update(id: string, dto: UpdateBillingRequestDto, actor: Actor) {
    return this.prisma.$transaction(
      async (tx) => {
        ...
        if (current.creationIdempotencyKey && dto.status)
          throw new BadRequestException(
            'Use an explicit billing request command for status transitions',
          );
        this.assertCanEdit(
          current.status,
          current.sale?.userId ?? '',
  ...
  async linkInvoice(
    ...
    const normalizedApplications = [...dto.applications].sort((a, b) =>
      a.saleDocumentId.localeCompare(b.saleDocumentId),
    );
    const substitutionReason = dto.invoice?.substitutesInvoiceId
      ? dto.invoice.substitutionReason?.trim()
      : undefined;
    if (dto.invoice?.substitutesInvoiceId && !substitutionReason)
      throw new BadRequestException('SUBSTITUTION_REASON_REQUIRED');
    const payloadHash = createHash('sha256')
      .update(
        JSON.stringify({
          id,
    ...
    try {
      return await this.prisma.$transaction(
        async (tx) => {
          const replay = await tx.invoice.findUnique({
            where: { linkIdempotencyKey: idempotencyKey },
            include: { documents: { include: { itemApplications: true } } },
          });
          if (replay) {
            if (replay.linkPayloadHash !== payloadHash)
                ...
                include: {
                  invoiceApplications: {
                    ...
                    include: {
                      ...
                      itemApplications: { where: { reversedAt: null } },
                    },
                  },
          ...
          if (
            new Set(normalizedApplications.map((item) => item.saleDocumentId))
              .size !== normalizedApplications.length
          )
            throw new BadRequestException('DUPLICATE_DOCUMENT_APPLICATION');

          const requestDocuments = new Map(
            request.documents.map((item) => [item.saleDocumentId, item]),
          );
          if (
            normalizedApplications.some(
              (item) => !requestDocuments.has(item.saleDocumentId),
            )
          )
            throw new BadRequestException('DOCUMENT_NOT_IN_BILLING_REQUEST');
          const documentIds = normalizedApplications.map(
            (item) => item.saleDocumentId,
          );
          await tx.$queryRaw`SELECT "id" FROM "SaleDocument" WHERE "id" IN (${Prisma.join(documentIds)}) ORDER BY "id" FOR UPDATE`;

          const firstSale = requestDocuments.get(
            normalizedApplications[0].saleDocumentId,
          )!.saleDocument.sale;
          ...
          if (dto.invoice?.substitutesInvoiceId) {
            ...
            if (
              !originalInvoice ||
              originalInvoice.status !== InvoiceStatus.ACTIVE ||
              originalInvoice.substitutedByInvoiceId
            )
              throw new BadRequestException('INVOICE_TO_REPLACE_NOT_ACTIVE');
            if (originalInvoice.legalEntityId !== dto.invoice.legalEntityId)
              throw new BadRequestException(
                'SUBSTITUTION_LEGAL_ENTITY_MISMATCH',
              );
            if (originalInvoice.currencyCode !== dto.invoice.currencyCode)
              throw new BadRequestException('SUBSTITUTION_CURRENCY_MISMATCH');
            if (
              !this.hasEquivalentApplications(
                originalInvoice.documents,
                normalizedApplications,
              )
            )
              throw new BadRequestException(
                'SUBSTITUTION_APPLICATION_MISMATCH',
              );
          }
          ...
          for (const application of normalizedApplications) {
            ...
            if (sale.legalEntityId !== firstSale.legalEntityId)
              throw new BadRequestException('MIXED_LEGAL_ENTITIES');
            if (sale.currencyCode !== firstSale.currencyCode)
              throw new BadRequestException('MIXED_CURRENCIES');
            ...
            if (!subtotal.plus(tax).equals(total))
              throw new BadRequestException('INVALID_APPLIED_AMOUNT');
            const itemSubtotal = sum(
              application.items.map(
                (item) => new Prisma.Decimal(item.subtotalApplied),
              ),
            );
            const itemTax = sum(
              application.items.map(
                (item) => new Prisma.Decimal(item.taxApplied),
              ),
            );
            const itemTotal = sum(
              application.items.map(
            ...
            if (
              !itemSubtotal.equals(subtotal) ||
              !itemTax.equals(tax) ||
              !itemTotal.equals(total)
            )
              throw new BadRequestException('ITEM_APPLICATION_MISMATCH');
            const authorizedItems = new Map(
              relation.requestedItems.map((item) => [item.saleItemId, item]),
            );
            if (
              new Set(application.items.map((item) => item.saleItemId)).size !==
                application.items.length ||
              application.items.some(
                (item) => !authorizedItems.has(item.saleItemId),
              )
            )
              throw new BadRequestException('ITEM_NOT_IN_BILLING_REQUEST');
            for (const item of application.items) {
              ...
              const prior = (relation.invoiceApplications ?? [])
                .filter(
                  (entry) =>
                    entry.invoice.status === InvoiceStatus.ACTIVE &&
                    (!originalInvoice ||
                      entry.invoice.id !== originalInvoice.id),
                )
                .flatMap((entry) => entry.itemApplications ?? [])
                .filter((entry) => entry.saleItemId === item.saleItemId);
              const consumedSubtotal = sum(
                prior.map((entry) => entry.subtotalApplied),
              );
              const consumedTax = sum(prior.map((entry) => entry.taxApplied));
              const consumedTotal = sum(
                prior.map((entry) => entry.totalApplied),
              );
              if (
                consumedSubtotal
                  .plus(item.subtotalApplied)
                  .greaterThan(authorized.requestedSubtotal) ||
                consumedTax
                  .plus(item.taxApplied)
                  .greaterThan(authorized.requestedTax) ||
                consumedTotal
                  .plus(item.totalApplied)
                  .greaterThan(authorized.requestedTotal)
              )
                throw new ConflictException(
                  'BILLING_REQUEST_ITEM_AMOUNT_EXCEEDED',
                );
            }
            const consumedFromRequest = sum(
              (relation.invoiceApplications ?? [])
                .filter(
                  (entry) =>
                    entry.invoice.status === InvoiceStatus.ACTIVE &&
                    (!originalInvoice ||
            ...
            if (
              total.greaterThan(
                Prisma.Decimal.max(
                  new Prisma.Decimal(0),
                  relation.requestedTotal.minus(consumedFromRequest),
                ),
              )
            )
              throw new ConflictException('OVER_INVOICED');
            const activeInvoiced = sum(
              relation.saleDocument.invoiceDocuments
                .filter(
                  (entry) =>
                    entry.invoice.status === InvoiceStatus.ACTIVE &&
                    (!originalInvoice ||
            ...
            if (total.greaterThan(sale.total.minus(activeInvoiced)))
              throw new ConflictException('OVER_INVOICED');
          }

          const appliedSubtotal = sum(
            normalizedApplications.map(
              (item) => new Prisma.Decimal(item.subtotalApplied),
            ),
          );
          const appliedTax = sum(
            normalizedApplications.map(
              (item) => new Prisma.Decimal(item.taxApplied),
            ),
          );
          ...
          if (dto.invoiceId) {
            ...
            if (invoice.legalEntityId !== firstSale.legalEntityId)
              throw new BadRequestException('MIXED_LEGAL_ENTITIES');
            if (invoice.currencyCode !== firstSale.currencyCode)
              throw new BadRequestException('MIXED_CURRENCIES');
            await tx.invoice.update({
              where: { id: invoice.id },
              data: {
                linkIdempotencyKey: idempotencyKey,
                linkPayloadHash: payloadHash,
              },
            });
          } else {
            const external = dto.invoice!;
            if (external.legalEntityId !== firstSale.legalEntityId)
              throw new BadRequestException('MIXED_LEGAL_ENTITIES');
            if (external.currencyCode !== firstSale.currencyCode)
              throw new BadRequestException('MIXED_CURRENCIES');
            const invoiceSubtotal = new Prisma.Decimal(external.subtotal).minus(
              external.discount,
            );
            if (
              !appliedSubtotal.equals(invoiceSubtotal) ||
              !appliedTax.equals(external.tax) ||
              !appliedTotal.equals(external.total)
            )
              throw new BadRequestException('INVOICE_TOTAL_MISMATCH');
            invoice = await tx.invoice.create({
              data: {
                legalEntityId: external.legalEntityId,
                currencyCode: external.currencyCode,
                series: external.series,
                folio: external.folio,
                uuid: external.uuid,
                subtotal: new Prisma.Decimal(external.subtotal),
                discount: new Prisma.Decimal(external.discount),
                tax: new Prisma.Decimal(external.tax),
                total: new Prisma.Decimal(external.total),
                createdByUserId: actor.id,
                linkIdempotencyKey: idempotencyKey,
                linkPayloadHash: payloadHash,
              },
            ...
            if (originalInvoice) {
              await tx.invoice.update({
                ...
                data: {
                  status: InvoiceStatus.SUBSTITUTED,
                  substitutedByInvoiceId: invoice.id,
                  version: { increment: 1 },
                },
              });
            }
          ...
          const existingApplications = dto.invoiceId
            ? (invoice.documents ?? [])
            : [];
          const totalSubtotalApplied = sum(
            existingApplications.map((item) => item.subtotalApplied),
          ).plus(appliedSubtotal);
          const totalTaxApplied = sum(
            existingApplications.map((item) => item.taxApplied),
          ).plus(appliedTax);
          const totalApplied = sum(
            existingApplications.map((item) => item.totalApplied),
          ).plus(appliedTotal);
          if (
            !totalSubtotalApplied.equals(
              invoice.subtotal.minus(invoice.discount),
            ) ||
            !totalTaxApplied.equals(invoice.tax) ||
            !totalApplied.equals(invoice.total)
          )
            throw new BadRequestException('INVOICE_TOTAL_MISMATCH');

          for (const application of normalizedApplications) {
            const requestDocument = requestDocuments.get(
              application.saleDocumentId,
            )!;
            const documentApplication = await tx.invoiceSaleDocument.create({
              data: {
                ...
                subtotalApplied: new Prisma.Decimal(
                  application.subtotalApplied,
                ),
                taxApplied: new Prisma.Decimal(application.taxApplied),
                totalApplied: new Prisma.Decimal(application.totalApplied),
                createdByUserId: actor.id,
              },
            });
            for (const item of application.items)
              await tx.invoiceSaleItemApplication.create({
                data: {
                  invoiceSaleDocumentId: documentApplication.id,
                  saleItemId: item.saleItemId,
                  subtotalApplied: new Prisma.Decimal(item.subtotalApplied),
                  taxApplied: new Prisma.Decimal(item.taxApplied),
                  totalApplied: new Prisma.Decimal(item.totalApplied),
                  createdByUserId: actor.id,
                },
              });
          }
          ...
          if (dto.invoice?.substitutesInvoiceId) {
            await this.writeAudit(
              ...
              'INVOICE_SUBSTITUTED',
              'Invoice',
              dto.invoice.substitutesInvoiceId,
              {
                before: {
  ...
  private hasEquivalentApplications(
    ...
    const amount = (value: Prisma.Decimal | string) =>
      new Prisma.Decimal(value).toFixed(2);
    const canonicalReplacement = replacementApplications
      .map((document) => ({
        saleDocumentId: document.saleDocumentId,
        subtotalApplied: amount(document.subtotalApplied),
        taxApplied: amount(document.taxApplied),
        totalApplied: amount(document.totalApplied),
        items: [...document.items]
          .sort((a, b) => a.saleItemId.localeCompare(b.saleItemId))
          .map((item) => ({
            saleItemId: item.saleItemId,
            subtotalApplied: amount(item.subtotalApplied),
            taxApplied: amount(item.taxApplied),
            totalApplied: amount(item.totalApplied),
          })),
      }))
      .sort((a, b) => a.saleDocumentId.localeCompare(b.saleDocumentId));
    const canonicalOriginal = (originalDocuments ?? [])
      .map((value) => {
        ...
        return {
          ...
          taxApplied: amount(document.taxApplied),
          totalApplied: amount(document.totalApplied),
          items: [...document.itemApplications]
            .sort((a, b) => a.saleItemId.localeCompare(b.saleItemId))
            .map((item) => ({
              saleItemId: item.saleItemId,
              subtotalApplied: amount(item.subtotalApplied),
      ...
      .sort((a, b) => a.saleDocumentId.localeCompare(b.saleDocumentId));
    return (
      JSON.stringify(canonicalOriginal) === JSON.stringify(canonicalReplacement)
    );
  }

  private async assertDocumentsRequestable(
    ...
    `;
    if (ids.some((id) => !rows.some((row) => row.saleDocumentId === id)))
      throw new BadRequestException('BILLING_READ_MODEL_INCOMPLETE');
    ...
    const blocked = rows.find(
      (row) =>
        ids.includes(row.saleDocumentId) && !requestable.has(row.billingStatus),
    );
    if (blocked)
      throw new BadRequestException(
        blocked.blockingCodes[0] ?? 'BILLING_DOCUMENT_NOT_REQUESTABLE',
      );
  }
    ...
    return this.prisma.$transaction(
      async (tx) => {
        ...
        if (current.version !== dto.expectedVersion)
          throw new ConflictException(
            'Billing request version does not match expectedVersion',
          );
        if (!transitions[current.status].includes(nextStatus))
          throw new BadRequestException(
            `Invalid billing request transition: ${current.status} to ${nextStatus}`,
        ...
        await tx.billingRequestHistory.create({
          data: {
            ...
            reason,
            notes: this.optionalText(dto.notes),
            compositionSnapshot: current.documents.map((document) => ({
              saleDocumentId: document.saleDocumentId,
              requestedSubtotal: document.requestedSubtotal.toString(),
              requestedTax: document.requestedTax.toString(),
              requestedTotal: document.requestedTotal.toString(),
            })),
          },
        });
        ...
        await this.writeAudit(
          tx,
          actor,
          nextStatus === BillingRequestStatus.CANCELLED
            ? 'BILLING_REQUEST_CANCELLED'
            : `BILLING_REQUEST_${nextStatus}`,
          'BillingRequest',
          id,
          {
    ...
    if (actor.role === 'SELLER')
      return {
        AND: [
          ...
          {
            OR: [
              { sale: { is: { userId: actor.id } } },
              {
                documents: {
                  some: { saleDocument: { sale: { userId: actor.id } } },
                },
              },
            ],
          },
    ...
    if (actor.role === 'COLLECTIONS')
      return {
        AND: [
          ...
          {
            OR: [
              { accountReceivables: { some: {} } },
              {
                documents: {
                  some: {
                    saleDocument: {
                      sale: { accountReceivable: { isNot: null } },
                    },
                  },
                },
              },
