  ...
  const tx = {
    ...
    billingPolicy: {
      findUnique: jest.fn().mockResolvedValue({
        id: 'default',
        billableDocumentTypes: ['SIMPLE_NOTE', 'LARGE_NOTE'],
        allowInternalReceipt: false,
        requireConfirmedDelivery: false,
        deadlineDays: 30,
        deadlineBasis: 'ISSUED_AT',
...
const documentRecord = {
  ...
  sale: {
    ...
    items: [
      {
        id: 'item-1',
        taxableBase: new Prisma.Decimal(90),
        tax: new Prisma.Decimal(10),
        total: new Prisma.Decimal(100),
        invoiceApplications: [],
        billingRequestItems: [],
...
describe('BillingRequestsService', () => {
  ...
  it('links an external invoice with exact document and item applications', async () => {
    ...
    const invoice = {
      ...
      total: new Prisma.Decimal(100),
      status: 'ACTIVE',
      linkPayloadHash: null,
    };
    tx.invoice.findUnique
      .mockResolvedValueOnce(null)
      .mockResolvedValueOnce(invoice);
    tx.invoice.create.mockResolvedValue(invoice);
    tx.invoiceSaleDocument.create.mockResolvedValue({
      ...
      documents: [
        {
          ...
          saleDocumentId: 'document-1',
          requestedTotal: new Prisma.Decimal(100),
          requestedItems: [authorizedItem()],
          invoiceApplications: [],
          saleDocument: {
            id: 'document-1',
            invoiceDocuments: [],
    ...
    expect(tx.invoice.create).toHaveBeenCalledWith({
      data: expect.objectContaining({
        linkIdempotencyKey: 'link-key',
        total: new Prisma.Decimal(100),
      }),
    });
    expect(tx.invoiceSaleDocument.create).toHaveBeenCalledWith({
      data: expect.objectContaining({
    ...
    expect(tx.invoiceSaleItemApplication.create).toHaveBeenCalledWith({
      data: expect.objectContaining({
        saleItemId: 'item-1',
        totalApplied: new Prisma.Decimal(100),
      }),
  ...
  it('rejects a sale item that was not authorized by the billing request', async () => {
      ...
      documents: [
        {
          id: 'request-document-1',
          saleDocumentId: 'document-1',
          requestedSubtotal: new Prisma.Decimal(90),
          requestedTax: new Prisma.Decimal(10),
          requestedTotal: new Prisma.Decimal(100),
          requestedItems: [authorizedItem()],
          invoiceApplications: [],
          saleDocument: {
    ...
    ).rejects.toThrow('ITEM_NOT_IN_BILLING_REQUEST');
  });
  ...
  it('rejects a fiscal split that exceeds the authorized item subtotal or tax', async () => {
    ...
    ).rejects.toThrow('BILLING_REQUEST_ITEM_AMOUNT_EXCEEDED');
  });
      ...
      documents: [
        {
          ...
          saleDocument: {
            ...
            invoiceDocuments: [
              {
                totalApplied: new Prisma.Decimal(80),
                invoice: { id: 'invoice-existing', status: 'ACTIVE' },
              },
            ],
            sale: {
              legalEntityId: 'legal-1',
      ...
      documents: [
        {
          id: 'request-document-1',
          saleDocumentId: 'document-1',
          requestedTotal: new Prisma.Decimal(50),
          requestedItems: [authorizedItem(45, 5, 50)],
          invoiceApplications: [
            {
              totalApplied: new Prisma.Decimal(30),
              itemApplications: [],
  ...
  it('rejects linking an existing invoice when prior and new applications exceed its totals', async () => {
    ...
    const invoice = {
      ...
      discount: new Prisma.Decimal(0),
      tax: new Prisma.Decimal(100),
      total: new Prisma.Decimal(1000),
      status: 'ACTIVE',
      documents: [
        {
          subtotalApplied: new Prisma.Decimal(900),
          taxApplied: new Prisma.Decimal(100),
          totalApplied: new Prisma.Decimal(1000),
          reversedAt: null,
        },
      ],
    };
      ...
      documents: [
        {
          ...
          saleDocumentId: 'document-2',
          requestedTotal: new Prisma.Decimal(1000),
          requestedItems: [authorizedItem(900, 100, 1000, 'item-2')],
          invoiceApplications: [],
          saleDocument: {
            id: 'document-2',
            invoiceDocuments: [],
    ...
    await expect(
      service.linkInvoice(
        ...
        admin,
        'existing-invoice-over-total',
      ),
    ).rejects.toThrow('INVOICE_TOTAL_MISMATCH');
  ...
  it('records substitution without creating sales, payments, or inventory movements', async () => {
    ...
    const original = {
      ...
      documents: [
        {
          saleDocumentId: 'document-1',
          subtotalApplied: new Prisma.Decimal(90),
          taxApplied: new Prisma.Decimal(10),
          totalApplied: new Prisma.Decimal(100),
          itemApplications: [
            {
              saleItemId: 'item-1',
          ...
          invoiceApplications: [
            {
              ...
              invoice: { id: 'invoice-old', status: 'ACTIVE' },
            },
          ],
          saleDocument: {
            id: 'document-1',
    ...
    expect(tx.invoice.update).toHaveBeenCalledWith({
      ...
      data: {
        status: 'SUBSTITUTED',
        substitutedByInvoiceId: 'invoice-new',
        version: { increment: 1 },
      },
    });
    expect(tx.$queryRaw).toHaveBeenCalledTimes(3);
    expect(tx.billingAuditLog.create).toHaveBeenCalledWith({
      data: expect.objectContaining({
        action: 'INVOICE_SUBSTITUTED',
        entityId: 'invoice-old',
        reason: 'Correct external invoice data',
      }),
    });
  ...
  it.each([
    ...
    'rejects a replacement with a different %s',
    async (_field, originalOverride, errorCode) => {
      const { prisma, tx } = createPrisma();
      const service = new BillingRequestsService(
        prisma as unknown as PrismaService,
      );
  ...
  it('rejects a replacement whose document or item applications differ from the original', async () => {
    ...
    tx.invoice.findUnique.mockResolvedValueOnce(null).mockResolvedValueOnce({
      ...
      documents: [
        {
          saleDocumentId: 'different-document',
          subtotalApplied: new Prisma.Decimal(90),
          taxApplied: new Prisma.Decimal(10),
          totalApplied: new Prisma.Decimal(100),
          itemApplications: [],
    ...
    ).rejects.toThrow('SUBSTITUTION_APPLICATION_MISMATCH');
    expect(tx.invoice.create).not.toHaveBeenCalled();
  });
  ...
  it('creates a request only for a confirmed customer sale and links an existing receivable', async () => {
    ...
    tx.sale.findUnique.mockResolvedValue({
      ...
      documents: [
        { id: 'document-1', documentType: 'SIMPLE_NOTE', createdAt: now },
      ],
      deliveryOrder: null,
    });
    ...
    ).resolves.toMatchObject({ id: 'request-1', status: 'REQUESTED' });
    expect(tx.billingRequest.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({
          saleId: 'sale-1',
          customerId: 'customer-1',
  ...
  it('creates partial and grouped requests from compatible documents in stable lock order', async () => {
    ...
    tx.saleDocument.findMany.mockResolvedValue([
      documentRecord,
      {
        ...documentRecord,
        id: 'document-2',
        sale: { ...documentRecord.sale, id: 'sale-2' },
      },
    ]);
    tx.billingRequest.create.mockResolvedValue(request({ version: 1 }));
    tx.billingRequest.findUnique
      .mockResolvedValueOnce(null)
      .mockResolvedValue(request({ version: 1 }));

    await service.create(
      {
        customerId: 'customer-1',
        reason: 'Grouped request',
        documents: [
          {
            saleDocumentId: 'document-2',
            requestedSubtotal: '90.00',
            requestedTax: '10.00',
            requestedTotal: '100.00',
            items: [{ saleItemId: 'item-1' }],
          },
          {
            saleDocumentId: 'document-1',
            requestedSubtotal: '90.00',
    ...
    expect(tx.billingRequest.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({
          saleId: null,
          creationIdempotencyKey: 'grouped-key',
        }),
      }),
    );
  });
  ...
  it('rejects a document amount that does not equal the exact selected item breakdown', async () => {
    ...
    await expect(
      service.create(
        {
          customerId: 'customer-1',
          reason: 'Incorrect tax split',
          documents: [
            {
              saleDocumentId: 'document-1',
              items: [{ saleItemId: 'item-1' }],
              requestedSubtotal: '80.00',
              requestedTax: '10.00',
              requestedTotal: '90.00',
            },
          ],
        },
        seller,
    ...
    ).rejects.toThrow('INVALID_REQUESTED_ITEM_BREAKDOWN');
  ...
  it('uses the canonical read model for internal receipt, delivery, and deadline rules', async () => {
    ...
    tx.billingPolicy.findUnique.mockResolvedValue({
      id: 'default',
      billableDocumentTypes: ['INTERNAL_RECEIPT'],
      allowInternalReceipt: true,
      requireConfirmedDelivery: false,
      deadlineDays: null,
      deadlineBasis: 'DELIVERED_AT',
      timezone: 'America/Cancun',
    });
    tx.saleDocument.findMany.mockResolvedValue([
      { ...documentRecord, documentType: 'INTERNAL_RECEIPT' },
    ]);
    tx.billingRequest.findUnique
      .mockResolvedValueOnce(null)
    ...
    expect(
      tx.$queryRaw.mock.calls.some(([query]) =>
        JSON.stringify(query).includes('BillingReportableNoteReadModel'),
      ),
    ).toBe(true);
  });
  ...
  it('rejects mixed issuers and incomplete fiscal profiles', async () => {
    ...
    tx.saleDocument.findMany.mockResolvedValue([
      documentRecord,
      {
        ...documentRecord,
        id: 'document-2',
        sale: { ...documentRecord.sale, legalEntityId: 'legal-2' },
      },
    ]);
    await expect(
      service.create(
    ...
    tx.saleDocument.findMany.mockResolvedValue([
      {
        ...
        sale: {
          ...documentRecord.sale,
          customer: { ...documentRecord.sale.customer, fiscalRegime: null },
        },
      },
    ]);
    await expect(
  ...
  it('returns the existing request for an idempotent retry and rejects payload drift', async () => {
    ...
    const payloadHash = createHash('sha256')
      ...
      .digest('hex');
    tx.billingRequest.findUnique.mockResolvedValue(
      request({ creationPayloadHash: payloadHash }),
    );
    await expect(
      service.create(dto, seller, 'retry-key'),
    ).resolves.toMatchObject({ id: 'request-1' });
    tx.billingRequest.findUnique.mockResolvedValue(
      request({ creationPayloadHash: 'different' }),
    );
    await expect(
      service.create(dto, seller, 'retry-key'),
  ...
  it('approves, rejects and cancels with optimistic versioning and terminal states', async () => {
    ...
    tx.billingRequest.findUnique.mockResolvedValue(
      request({ version: 2, status: BillingRequestStatus.IN_REVIEW }),
    );
    tx.billingRequest.update.mockResolvedValue(
      request({ version: 3, status: BillingRequestStatus.APPROVED }),
    );
    await service.approve(
      'request-1',
      { expectedVersion: 2, reason: 'Validated' },
      admin,
    );
    expect(tx.billingRequest.update).toHaveBeenCalledWith(
      expect.objectContaining({
    ...
    await expect(
      service.cancel(
        'request-1',
        { expectedVersion: 3, reason: 'Too late' },
        admin,
      ),
    ).rejects.toBeInstanceOf(BadRequestException);
    tx.billingRequest.findUnique.mockResolvedValue(
      request({ version: 4, status: BillingRequestStatus.REQUESTED }),
    );
    await expect(
      service.reject(
        'request-1',
        { expectedVersion: 3, reason: 'Invalid' },
        admin,
      ),
    ).rejects.toBeInstanceOf(ConflictException);
  });
  ...
  it('rejects duplicate, cancelled, customerless and mismatched-customer sales', async () => {
    ...
    tx.sale.findUnique.mockResolvedValueOnce({
      id: 'sale-1',
      customerId: 'customer-1',
      userId: 'seller-1',
      status: SaleStatus.CONFIRMED,
      billingRequests: [{ id: 'existing' }],
      accountReceivable: null,
      documents: [],
    });
    await expect(
      service.create(
        { customerId: 'customer-1', saleId: 'sale-1', reason: 'Razón' },
        seller,
      ),
    ).rejects.toBeInstanceOf(ConflictException);
    tx.sale.findUnique.mockResolvedValueOnce({
  ...
  it('enforces state transitions, audit history and terminal states', async () => {
    ...
    await service.update(
      ...
      {
        status: BillingRequestStatus.IN_REVIEW,
        reason: 'Revisión iniciada',
        notes: 'Validar datos',
      },
      admin,
    );
    expect(tx.billingRequestHistory.create).toHaveBeenCalledWith({
      data: expect.objectContaining({
    ...
    tx.billingRequest.findUnique.mockResolvedValue(
      request({ status: BillingRequestStatus.APPROVED }),
    );
    await expect(
      service.update(
        'request-1',
    ...
    tx.billingRequest.findUnique.mockResolvedValue(
      request({ status: BillingRequestStatus.REQUESTED }),
    );
    await expect(
      service.update(
        'request-1',
  ...
  it('limits sellers to their own requests and REQUESTED text edits', async () => {
    ...
    await expect(
      service.update('request-1', { notes: 'Nota' }, seller),
    ).rejects.toBeInstanceOf(ForbiddenException);
    tx.billingRequest.findUnique.mockResolvedValue(
      request({ status: BillingRequestStatus.IN_REVIEW }),
    );
    await expect(
      service.update('request-1', { notes: 'Nota' }, seller),
    ).rejects.toBeInstanceOf(ForbiddenException);
  ...
  it('starts review through the explicit versioned and audited command', async () => {
    ...
    tx.billingRequest.findUnique.mockResolvedValue(
      request({
        version: 3,
        status: BillingRequestStatus.REQUESTED,
        creationIdempotencyKey: 'created-from-report',
        documents: [],
      }),
    );
    tx.billingRequest.update.mockResolvedValue(
      request({ version: 4, status: BillingRequestStatus.IN_REVIEW }),
    );

    await service.startReview(
      'request-1',
      { expectedVersion: 3, reason: 'Validar expediente' },
      admin,
    );

    expect(tx.billingRequest.update).toHaveBeenCalledWith(
      expect.objectContaining({
        where: { id: 'request-1', version: 3 },
        data: expect.objectContaining({
          status: BillingRequestStatus.IN_REVIEW,
          version: { increment: 1 },
        }),
    ...
    expect(tx.billingAuditLog.create).toHaveBeenCalledWith({
      data: expect.objectContaining({
        action: 'BILLING_REQUEST_IN_REVIEW',
        actorUserId: 'admin-1',
      }),
    });
  });
