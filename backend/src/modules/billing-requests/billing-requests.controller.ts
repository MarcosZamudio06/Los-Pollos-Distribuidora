...
export class BillingRequestsController {
  ...
  async findAll(
    @Query() query: ListBillingRequestsQueryDto,
    @CurrentUser() user: AuthenticatedUser,
  ) {
    return {
      success: true,
      message: 'Billing requests retrieved successfully',
      data: await this.service.findAll(query, user),
    };
  }
  ...
  async findOne(
    ...
    return {
      success: true,
      message: 'Billing request retrieved successfully',
  ...
  @Roles('ADMIN', 'SELLER')
  async create(
    @Body() body: CreateBillingRequestDto,
    @CurrentUser() user: AuthenticatedUser,
    @Headers('Idempotency-Key') idempotencyKey?: string,
  ) {
    if (body.documents?.length && !idempotencyKey?.trim())
      throw new BadRequestException('Idempotency-Key header is required');
    return {
      success: true,
      message: 'Billing request created successfully',
      data: await this.service.create(body, user, idempotencyKey),
    };
  }
  ...
  async startReview(
    @Param('id') id: string,
    @Body() body: ReviewBillingRequestDto,
    @CurrentUser() user: AuthenticatedUser,
  ) {
    return {
      success: true,
      message: 'Billing request review started successfully',
      data: await this.service.startReview(id, body, user),
    };
  }
  ...
  async approve(
    ...
    return {
      success: true,
      message: 'Billing request approved successfully',
  ...
  async reject(
    ...
    return {
      success: true,
      message: 'Billing request rejected successfully',
      data: await this.service.reject(id, body, user),
    };
  }
  ...
  async update(
    @Param('id') id: string,
    @Body() body: UpdateBillingRequestDto,
    @CurrentUser() user: AuthenticatedUser,
  ) {
    return {
      success: true,
      message: 'Billing request updated successfully',
      data: await this.service.update(id, body, user),
    };
  }
  ...
  async cancel(
    @Param('id') id: string,
    @Body() body: CancelBillingRequestDto,
    @CurrentUser() user: AuthenticatedUser,
  ) {
    return {
      success: true,
      message: 'Billing request cancelled successfully',
      data: await this.service.cancel(id, body, user),
    };
  }
  ...
  async linkInvoice(
    ...
    if (!!body.invoiceId === !!body.invoice)
      throw new BadRequestException(
        'Provide exactly one of invoiceId or invoice',
      );
    return {
      success: true,
      message: 'Invoice linked successfully',
      data: await this.service.linkInvoice(
        id,
        body,
        user,
        idempotencyKey.trim(),
