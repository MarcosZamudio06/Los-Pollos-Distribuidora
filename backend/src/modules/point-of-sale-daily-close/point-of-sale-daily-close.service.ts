...
import {
  ...
  DailyCloseDifferenceType,
  DailyCloseDifferenceUnit,
  DailyCloseEventType,
  DailyCloseSnapshotType,
...
const dailyCloseTransitions: Record<
  PointOfSaleDailyCloseStatus,
  readonly PointOfSaleDailyCloseStatus[]
> = {
...
const decreaseMovementTypes = new Set([
  'OUT',
  'SALE',
  'CANCEL_PURCHASE',
  'TRANSFER_OUT',
...
export class PointOfSaleDailyCloseService {
  ...
  async list(query: ListDailyCloseQueryDto, user: AuthenticatedUser) {
    ...
    if (
      ...
      query.operationalLocationId !== locationId
    ) {
      ...
      where: {
        ...(locationId
          ...
          : query.operationalLocationId
            ...
            : {}),
        ...(query.businessDate
          ? { businessDate: this.date(query.businessDate) }
          : {}),
      },
      include: {
  ...
  private async findClose(id: string, client: DailyCloseClient = this.prisma) {
    const close = await client.pointOfSaleDailyClose.findUnique({
      where: { id },
  ...
  async open(dto: OpenDailyCloseDto, user: AuthenticatedUser) {
    ...
    try {
      created = await this.prisma.$transaction(async (tx) => {
        const close = await tx.pointOfSaleDailyClose.create({
          data: {
            operationalLocationId: dto.operationalLocationId,
            businessDate,
  ...
  async addExpense(
    ...
    if (!dto.reason.trim())
      throw new BadRequestException('EXPENSE_REASON_REQUIRED');
    const shift = await this.prisma.cashShift.findUnique({
      where: { id: dto.cashShiftId },
      include: { terminal: true },
    });
    if (!shift || shift.pointOfSaleDailyCloseId !== id)
    ...
    if (occurredAt < from || occurredAt >= to)
      throw new BadRequestException('EXPENSE_OUTSIDE_OPERATIONAL_DAY');
    ...
    try {
      const updated = await this.prisma.$transaction(async (tx) => {
        ...
        await this.createEvent(
          tx,
          id,
          DailyCloseEventType.EXPENSE_RECORDED,
          user.id,
          { cashMovementId: movement.id, payloadHash },
          idempotencyKey,
        );
        return recalculated;
      });
      ...
      if (this.isIdempotencyRaceError(error))
        return this.resolveExpenseReplay(id, idempotencyKey, payloadHash, user);
      throw error;
    }
  }

  async addScaleTicket(
    id: string,
    dto: CreateScaleTicketDto,
    user: AuthenticatedUser,
    idempotencyKey: string,
  ) {
    const close = await this.requireDraft(id, user);
    ...
    if (
      ...
      throw new BadRequestException('SCALE_TICKET_QUANTITY_REQUIRED');
    const capturedDate = this.date(dto.capturedDate);
    if (capturedDate.getTime() !== close.businessDate.getTime())
      throw new BadRequestException('SCALE_TICKET_DATE_MISMATCH');
    let saleId = dto.saleId?.trim() || null;
    const saleDocumentId = dto.saleDocumentId?.trim() || null;
    ...
    if (saleDocumentId) {
      ...
      if (!saleDocument)
        throw new BadRequestException('SCALE_TICKET_DOCUMENT_NOT_FOUND');
      if (saleId && saleId !== saleDocument.saleId)
        throw new BadRequestException('SCALE_TICKET_SALE_DOCUMENT_MISMATCH');
    ...
    const payloadHash = this.hashPayload({
      ...
      weightKg: netWeightKg ?? null,
      grossWeightKg: dto.grossWeightKg ?? null,
      tareWeightKg: dto.tareWeightKg ?? null,
      pieceCount: dto.pieceCount ?? null,
      unitPrice: dto.unitPrice ?? null,
      amount: dto.amount ?? null,
      scaleDeviceId: dto.scaleDeviceId?.trim() || null,
      notes: dto.notes?.trim() || null,
      userId: user.id,
    });
    try {
      const updated = await this.prisma.$transaction(async (tx) => {
        const existing = await tx.scaleTicketReference.findUnique({
          where: { idempotencyKey },
        });
        if (existing) {
          this.assertSameIdempotencyPayload(
        ...
        await this.createEvent(
          tx,
          id,
          DailyCloseEventType.SCALE_TICKET_RECORDED,
          user.id,
          { scaleTicketReferenceId: ticket.id, payloadHash },
          idempotencyKey,
        );
        return recalculated;
      });
      ...
      if (
        ...
        const existing = await this.prisma.scaleTicketReference.findUnique({
          where: { idempotencyKey },
        });
        if (existing)
          return this.resolveScaleTicketReplay(
            id,
            idempotencyKey,
            payloadHash,
            user,
  ...
  async recordCashCount(
    ...
    const updated = await this.prisma.$transaction(async (tx) => {
      ...
      await this.createEvent(
        tx,
        id,
        DailyCloseEventType.CASH_COUNT_RECORDED,
        user.id,
        { cashCountedTotal: dto.cashCountedTotal },
      );
      return recalculated;
  ...
  async justifyDifference(
    ...
    if (!reason)
      throw new BadRequestException('DAILY_CLOSE_DIFFERENCE_REASON_REQUIRED');
    if (!evidence)
    ...
    const updated = await this.prisma.$transaction(async (tx) => {
      ...
      if (!difference)
        throw new NotFoundException('DAILY_CLOSE_DIFFERENCE_NOT_FOUND');
      if (Number(difference.differenceValue) === 0)
      ...
      await this.createEvent(
        tx,
        id,
        DailyCloseEventType.DIFFERENCE_JUSTIFIED,
        user.id,
        { differenceId, reason, evidence, sourceVersion: dto.version },
      );
      return this.findClose(id, tx);
    });
    return this.projectForRole(updated, user);
  ...
  async authorizeDifference(
    ...
    const updated = await this.prisma.$transaction(async (tx) => {
      ...
      if (!difference)
        throw new NotFoundException('DAILY_CLOSE_DIFFERENCE_NOT_FOUND');
      if (
        difference.status !== DailyCloseDifferenceStatus.PENDING_AUTHORIZATION
      )
        throw new BadRequestException(
      ...
      await this.createEvent(
        tx,
        id,
        DailyCloseEventType.DIFFERENCE_AUTHORIZED,
        user.id,
        { differenceId, sourceVersion: dto.version },
      );
      return this.findClose(id, tx);
    });
    return this.projectForRole(updated, user);
  ...
  async createInventoryCount(
    id: string,
    dto: CreateDailyCloseInventoryCountDto,
    user: AuthenticatedUser,
    idempotencyKey: string,
  ) {
    await this.requireDraft(id, user);
    ...
    if (!dto.reason.trim())
      throw new BadRequestException(
        'DAILY_CLOSE_INVENTORY_COUNT_REASON_REQUIRED',
      );
    ...
    try {
      const updated = await this.prisma.$transaction(async (tx) => {
        const existing = await tx.dailyCloseInventoryCount.findUnique({
          where: { idempotencyKey },
        });
        if (existing) {
          this.assertSameIdempotencyPayload(
        ...
        await this.createEvent(
          tx,
          id,
          DailyCloseEventType.INVENTORY_COUNT_CREATED,
          user.id,
          { inventoryCountId: count.id, payloadHash },
          idempotencyKey,
        );
        return recalculated;
      });
      ...
      if (
        ...
        const existing = await this.prisma.dailyCloseInventoryCount.findUnique({
          where: { idempotencyKey },
        });
        if (existing)
          return this.resolveInventoryCountReplay(
            id,
            idempotencyKey,
            payloadHash,
            user,
        ...
        throw new ConflictException(
          'DAILY_CLOSE_INVENTORY_COUNT_ALREADY_EXISTS',
        );
      }
      if (this.isIdempotencyRaceError(error))
        return this.resolveInventoryCountReplay(
  ...
  async updateInventoryCount(
    ...
    if (
      ...
      throw new BadRequestException(
        'DAILY_CLOSE_INVENTORY_COUNT_UPDATE_REQUIRED',
      );
    this.assertCountQuantities(
      dto.physicalQuantityKg,
      dto.physicalQuantityPieces,
    ...
    const updated = await this.prisma.$transaction(async (tx) => {
      await tx.dailyCloseInventoryCount.update({
        ...
        data: {
          ...(dto.physicalQuantityKg === undefined
            ? {}
            : { physicalQuantityKg: dto.physicalQuantityKg }),
          ...(dto.physicalQuantityPieces === undefined
            ? {}
            : { physicalQuantityPieces: dto.physicalQuantityPieces }),
      ...
      await this.createEvent(
        tx,
        id,
        DailyCloseEventType.INVENTORY_COUNT_UPDATED,
        user.id,
  ...
  async deleteInventoryCount(
    ...
    const updated = await this.prisma.$transaction(async (tx) => {
      ...
      await this.createEvent(
        tx,
        id,
        DailyCloseEventType.INVENTORY_COUNT_DELETED,
        user.id,
        { inventoryCountId: countId },
      );
      return recalculated;
  ...
  private async validateWithin(
    ...
    if (
      updated.lines.some(
        (line) => line.operationalLocationId !== close.operationalLocationId,
      )
    )
      errors.push({
    ...
    const differences =
      storedDifferences.length > 0
        ? storedDifferences.map((difference) => ({
            ...
            recordedValue:
              difference.recordedValue === null
                ? null
                : Number(difference.recordedValue),
    ...
    return {
      ...
      differences:
        user.role === 'WAREHOUSE'
          ? differences.filter(
              (item) => item.unit === 'KG' || item.unit === 'kg',
            )
          : user.role === 'COLLECTIONS'
            ? differences.filter((item) => item.unit === 'MXN')
            : differences,
    };
  }
  ...
  async close(
    ...
    return this.transition(
      ...
      {
        status: 'CLOSED',
        cashSessionStatus: CashSessionStatus.CLOSED,
        cashSessionClosedAt: closedAt,
  ...
  async cancel(
    ...
    return this.transition(
      ...
      'CANCELLED',
      {
        status: 'CANCELLED',
        cashSessionStatus: CashSessionStatus.CLOSED,
        cashSessionClosedAt: cancelledAt,
        cancelledByUserId: user.id,
        cancelledAt,
        notes: `Cancelación: ${dto.reason.trim()}`,
      },
      user,
    );
  }
  ...
  private async recalculate(
    ...
    const byConcept = (concept: string, field: 'quantityKg' | 'amount') =>
      sum(
        close.lines
          .filter((line) => line.conceptType === concept)
          .map((line) => line[field]),
      );
    const scaleReportedKg = sum(
      close.scaleTicketReferences.map((ticket) => ticket.weightKg),
    );
    const kilos = calculateDailyCloseKilos({
      ...
      manualSoldKg:
        byConcept('SALE_NOTE', 'quantityKg') +
        byConcept('SALE_SCALE_TICKET', 'quantityKg'),
      sales: close.sales,
    });
    ...
    const appliedPayments = close.payments.filter(
    ...
    const { purchaseCostTotal, costQuality } = calculateDailyCloseCost(
      close.sales,
    );
    const cashExpenseTotal = sum(
      close.cashMovements
        .filter(
          (movement) =>
            movement.type === 'EXPENSE' && movement.movementChannel === 'CASH',
        )
        .map((movement) => movement.amount),
    );
    const openingCash =
    ...
    const data = {
      ...
      totalRemainingKg: reconciliation.items.reduce(
        (total, item) => total + item.theoreticalQuantityKg,
        0,
      ),
      totalShortageKg: reconciliation.items.reduce(
        (total, item) => total + item.shortageQuantityKg,
        0,
      ),
      totalSurplusKg: reconciliation.items.reduce(
        (total, item) => total + item.surplusQuantityKg,
        0,
      ),
      scaleReportedKg,
      scaleDifferenceKg: scaleReportedKg - kilos.totalSoldKg,
      cashTotal,
      cardVoucherTotal,
      transferTotal,
      expenseTotal,
    ...
    for (const item of input.inventory) {
      ...
      if (item.physicalQuantityPieces !== null) {
        add({
          ...
          differenceValue:
            item.physicalQuantityPieces - item.theoreticalQuantityPieces,
          productId: item.product.id,
        });
      }
    }
  ...
  private async syncDifferences(
    ...
    closeId: string,
    definitions: DifferenceDefinition[],
  ) {
    ...
    for (const definition of definitions) {
      const existing = previous.find(
        (difference) =>
          difference.scope === definition.scope &&
          difference.referenceKey === definition.referenceKey,
      );
      const valuesChanged =
        existing !== undefined &&
  ...
  private async reconciliationForClose(
    ...
    const [movements, counts] = await Promise.all([
      client.inventoryMovement.findMany({
        ...
        select: {
          ...
          quantityKg: true,
          quantityPieces: true,
          previousQuantityKg: true,
          previousQuantityPieces: true,
          newQuantityKg: true,
          newQuantityPieces: true,
          createdAt: true,
          product: { select: { id: true, name: true, sku: true, unit: true } },
        },
      }),
    ...
    type Entry = {
      ...
      openingQuantityKg: number;
      openingQuantityPieces: number;
      entriesQuantityKg: number;
      entriesQuantityPieces: number;
      soldQuantityKg: number;
      soldQuantityPieces: number;
      otherOutputsQuantityKg: number;
      otherOutputsQuantityPieces: number;
    ...
    const entryFor = (product: Entry['product']) => {
      ...
      const entry: Entry = {
        product,
        openingQuantityKg: 0,
        openingQuantityPieces: 0,
        entriesQuantityKg: 0,
        entriesQuantityPieces: 0,
        soldQuantityKg: 0,
        soldQuantityPieces: 0,
        otherOutputsQuantityKg: 0,
        otherOutputsQuantityPieces: 0,
      };
    ...
    for (const movement of movements) {
      ...
      const deltaKg = this.movementDelta(
        movement.previousQuantityKg,
        movement.newQuantityKg,
        movement.quantityKg,
        movement.type,
      );
      const deltaPieces = this.movementDelta(
        movement.previousQuantityPieces,
        movement.newQuantityPieces,
      ...
      if (movement.createdAt < from) {
        ...
        if (deltaKg >= 0) entry.entriesQuantityKg += deltaKg;
        else entry.otherOutputsQuantityKg -= deltaKg;
        if (deltaPieces >= 0) entry.entriesQuantityPieces += deltaPieces;
        else entry.otherOutputsQuantityPieces -= deltaPieces;
      }
    }
    for (const sale of close.sales)
      for (const item of sale.items) {
        const product =
          movements.find((movement) => movement.productId === item.productId)
            ?.product ??
          counts.find((count) => count.productId === item.productId)?.product;
        if (!product) continue;
    ...
    return {
      ...
      items: [...entries.values()]
        .map((entry) => {
          const theoreticalQuantityKg =
            entry.openingQuantityKg +
            entry.entriesQuantityKg -
            entry.soldQuantityKg -
            entry.otherOutputsQuantityKg;
          const theoreticalQuantityPieces =
            entry.openingQuantityPieces +
            entry.entriesQuantityPieces -
            entry.soldQuantityPieces -
            entry.otherOutputsQuantityPieces;
          const physicalQuantityKg = entry.count
            ? Number(entry.count.physicalQuantityKg)
            : null;
          const physicalQuantityPieces = entry.count
            ? entry.count.physicalQuantityPieces
            : null;
          return {
            ...entry,
            ...
            surplusQuantityKg:
              physicalQuantityKg === null
                ? 0
                : Math.max(physicalQuantityKg - theoreticalQuantityKg, 0),
            shortageQuantityKg:
              physicalQuantityKg === null
                ? 0
                : Math.max(theoreticalQuantityKg - physicalQuantityKg, 0),
            surplusQuantityPieces:
              physicalQuantityPieces === null
                ? 0
                : Math.max(
                    physicalQuantityPieces - theoreticalQuantityPieces,
                    0,
                  ),
            shortageQuantityPieces:
              physicalQuantityPieces === null
                ? 0
                : Math.max(
                    theoreticalQuantityPieces - physicalQuantityPieces,
                    0,
                  ),
          };
        })
  ...
  private assertCountQuantities(
    ...
    if (
      ...
      throw new BadRequestException(
        'DAILY_CLOSE_INVENTORY_COUNT_QUANTITY_REQUIRED',
      );
    if (
      (quantityKg !== undefined && quantityKg < 0) ||
      (quantityPieces !== undefined && quantityPieces < 0)
    )
      throw new BadRequestException(
        'DAILY_CLOSE_INVENTORY_COUNT_QUANTITY_INVALID',
      );
  }
  private assertProductCountUnit(
    unit: string,
    quantityKg?: number,
    quantityPieces?: number,
    allowPartial = false,
    ...
    if (
      unit === 'KG' &&
      (quantityPieces !== undefined ||
        (!allowPartial && quantityKg === undefined))
    )
      throw new BadRequestException(
        'DAILY_CLOSE_INVENTORY_COUNT_UNIT_MISMATCH',
      );
    if (
      unit === 'PIECE' &&
      (quantityKg !== undefined ||
        (!allowPartial && quantityPieces === undefined))
    )
      throw new BadRequestException(
        'DAILY_CLOSE_INVENTORY_COUNT_UNIT_MISMATCH',
      );
  ...
  private async requireDraft(id: string, user: AuthenticatedUser) {
    const close = await this.requireCloseAccess(id, user);
    if (close.status !== 'DRAFT')
      throw new BadRequestException('DAILY_CLOSE_NOT_EDITABLE');
    return close;
  }
  private async syncOperations(
    ...
    await tx.inventoryMovement.updateMany({
      where: {
        locationId,
        createdAt: { gte: from, lt: to },
        type: { in: ['IN', 'PURCHASE', 'TRANSFER_IN'] },
        pointOfSaleDailyCloseId: null,
      },
      data: { pointOfSaleDailyCloseId: closeId },
    });
  ...
  private async transitionWithin(
    ...
    const snapshotType =
      target === 'REVIEWED'
        ...
        : target === 'CLOSED'
          ...
          : target === 'DRAFT' &&
              (current.status === 'REVIEWED' || current.status === 'CLOSED')
            ? DailyCloseSnapshotType.REOPENED
            : null;
    if (snapshotType)
      await this.createSnapshot(
        tx,
        snapshotType === DailyCloseSnapshotType.REOPENED
          ? current
          : transitioned,
        current.version,
        snapshotType,
    ...
    await this.createEvent(
      tx,
      id,
      DailyCloseEventType.STATUS_CHANGED,
      user.id,
      {
  ...
  private async createSnapshot(
    ...
    await tx.dailyCloseSnapshot.create({
      data: {
        pointOfSaleDailyCloseId: (close as { id: string }).id,
        sourceVersion,
        snapshotType,
        payload,
  ...
  private async resolveExpenseReplay(
    closeId: string,
    idempotencyKey: string,
    payloadHash: string,
    user: AuthenticatedUser,
  ...
  private async resolveScaleTicketReplay(
    closeId: string,
    idempotencyKey: string,
    payloadHash: string,
    user: AuthenticatedUser,
  ...
  private async resolveInventoryCountReplay(
    closeId: string,
    idempotencyKey: string,
    payloadHash: string,
    user: AuthenticatedUser,
  ...
  private money(value?: number) {
    ...
    if (!Number.isFinite(amount) || amount < 0)
      throw new BadRequestException('INITIAL_CASH_AMOUNT_INVALID');
  ...
  private async locationScope(user: AuthenticatedUser): Promise<string | null> {
    ...
    if (!actor?.isActive || !actor.operationalLocationId)
      throw new ForbiddenException('LOCATION_NOT_AUTHORIZED');
  ...
  private async projectDetailForRole<
    T extends {
      id: string;
      operationalLocationId: string;
      businessDate?: Date;
    },
  >(close: T, user: AuthenticatedUser) {
    if (!close.businessDate) return this.projectForRole(close, user);
    return this.projectForRole(
      await this.withExcludedOperations(close as T & { businessDate: Date }),
      user,
    );
  }
  private async withExcludedOperations<
    T extends { id: string; operationalLocationId: string; businessDate: Date },
  >(close: T) {
    const { from, to } = this.operationalDay(close.businessDate);
    ...
    return {
      ...close,
      excludedOperations: [
        ...routePayments.map((payment) => ({
          id: payment.id,
          type: 'PAYMENT' as const,
          reference: payment.referenceNumber || payment.id,
          amount: payment.amount,
          reason: 'Pago asociado a una ruta pendiente de liquidación.',
          occurredAt: payment.paidAt,
        })),
        ...unconfirmedSales.map((sale) => ({
          id: sale.id,
          type: 'SALE' as const,
  ...
  private projectForRole(close: object, user: AuthenticatedUser) {
    ...
    if (user.role !== 'ADMIN' && Array.isArray(result.sales)) {
      result.sales = (result.sales as Array<Record<string, unknown>>).map(
        (sale) => {
    ...
    if (Array.isArray(result.differences)) {
      ...
      const visibleDifferences =
        user.role === 'WAREHOUSE'
          ? differences.filter(
              (difference) =>
                difference.scope === DailyCloseDifferenceScope.INVENTORY,
            )
          : user.role === 'COLLECTIONS'
            ? differences.filter(
                (difference) =>
                  difference.scope === DailyCloseDifferenceScope.CASH,
              )
            : differences;
      result.differences = visibleDifferences;
      result.unresolvedDifferenceCount = visibleDifferences.filter(
        (difference) =>
          Number(difference.differenceValue) !== 0 &&
    ...
    if (user.role === 'SELLER') {
      ...
      if (Array.isArray(result.lines)) {
        result.lines = (result.lines as Array<Record<string, unknown>>).filter(
          (line) =>
            line.section !== 'PROFIT' && line.conceptType !== 'NET_PROFIT',
        );
      }
    }
  ...
  >(close: T) {
    const estimated = close.sales.some((sale) =>
      sale.items.some((item) => item.costSnapshotSource === 'LEGACY_BACKFILL'),
    );
    return {
      ...close,
      costQuality: estimated ? ('ESTIMATED' as const) : ('EXACT' as const),
      dataAsOf: close.updatedAt,
    };
  }
}
