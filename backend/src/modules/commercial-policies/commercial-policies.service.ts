...
import {
  CreateCommercialPolicyDto,
  CreateDiscountAuthorizationDto,
  ListCommercialPoliciesQueryDto,
  UpdateCommercialPolicyDto,
} from './dto';

type CommercialPolicyRecord = Prisma.CommercialPolicyGetPayload<
  Record<string, never>
>;
...
export class CommercialPoliciesService {
  ...
  async create(dto: CreateCommercialPolicyDto, currentUser: AuthenticatedUser) {
    const data = this.normalizeMutationData(
      ...
      currentUser.id,
      true,
    ) as Prisma.CommercialPolicyUncheckedCreateInput;
    const policy = await this.prisma.commercialPolicy.create({
      data,
    });
  ...
  async update(
    ...
    const data = this.normalizeMutationData(
      ...
      currentUser.id,
      false,
    ) as Prisma.CommercialPolicyUncheckedUpdateInput;
  ...
  async authorizeDiscount(
    policyId: string,
    dto: CreateDiscountAuthorizationDto,
    currentUser: AuthenticatedUser,
  ) {
    ...
    if (dto.maximumPercentage > maximumDiscountPercentage) {
      throw new BadRequestException(
        'Discount authorization exceeds the commercial policy maximum',
      );
    }

    if (dto.authorizedForUserId) {
      const user = await this.prisma.user.findUnique({
        where: { id: dto.authorizedForUserId },
        select: { id: true, isActive: true },
      });
      if (!user?.isActive)
        throw new NotFoundException('Authorized seller not found');
    }
  ...
  private buildListWhere(
    query: ListCommercialPoliciesQueryDto,
  ): Prisma.CommercialPolicyWhereInput {
  ...
  private buildPagination(query: ListCommercialPoliciesQueryDto): {
    skip?: number;
    take?: number;
  } {
  ...
  private normalizeMutationData(
    ...
    if (dto.defaultCreditDays !== undefined && dto.defaultCreditDays < 0)
      throw new BadRequestException(
        'Credit days must be greater than or equal to zero',
      );
    if (
      dto.maximumDiscountPercentage !== undefined &&
      dto.maximumDiscountPercentage > 100
    )
      throw new BadRequestException(
        'Maximum discount percentage must not exceed 100',
      );

    const overdueBlockingMode = dto.overdueBlockingMode?.trim();
    const creditLimitBlockingMode = dto.creditLimitBlockingMode?.trim();
    if (
      dto.defaultCreditDays !== undefined &&
      dto.defaultCreditDays > 0 &&
      !overdueBlockingMode
    ) {
      throw new BadRequestException(
        'overdueBlockingMode is required when credit days are configured',
      );
    }
    if (
      dto.defaultCreditLimit !== undefined &&
      dto.defaultCreditLimit > 0 &&
      !creditLimitBlockingMode
    ) {
      throw new BadRequestException(
        'creditLimitBlockingMode is required when credit limit is configured',
      );
    }
    ...
    if (isActive === true && !dto.effectiveFrom)
      throw new BadRequestException(
        'effectiveFrom is required for active policies',
      );
    ...
    return {
      ...
      ...(dto.priceListId !== undefined
        ? { priceListId: dto.priceListId?.trim() || null }
        : {}),
      ...(dto.defaultCreditLimit !== undefined
        ? { defaultCreditLimit: dto.defaultCreditLimit }
        : {}),
      ...(dto.defaultCreditDays !== undefined
        ? { defaultCreditDays: dto.defaultCreditDays }
        : {}),
      ...(dto.overdueBlockingMode !== undefined
        ? { overdueBlockingMode: overdueBlockingMode || null }
        : {}),
      ...(dto.creditLimitBlockingMode !== undefined
        ? { creditLimitBlockingMode: creditLimitBlockingMode || null }
        : {}),
      ...(dto.allowAdministrativeOverride !== undefined
        ? { allowAdministrativeOverride: dto.allowAdministrativeOverride }
        : {}),
      ...(dto.maximumDiscountPercentage !== undefined
        ? { maximumDiscountPercentage: dto.maximumDiscountPercentage }
        : {}),
  ...
  private async findActivePolicy(id: string): Promise<CommercialPolicyRecord> {
    const policy = await this.prisma.commercialPolicy.findFirst({
  ...
  private assertPolicyIsEffective(policy: CommercialPolicyRecord): void {
    ...
    if (
      ...
      throw new BadRequestException(
        'Commercial policy is not currently effective',
      );
    }
  }

  private async assertHistoricalConditionsAreNotOverwritten(
    id: string,
    dto: UpdateCommercialPolicyDto,
  ): Promise<void> {
    const changesHistoricalConditions = HISTORICAL_POLICY_FIELDS.some(
    ...
    if (sale || accountReceivable) {
      throw new BadRequestException(
        'Cannot overwrite commercial conditions already applied to sales or accounts receivable',
      );
    }
  }

  private async assertNoOpenPolicyDependencies(id: string): Promise<void> {
    const openSale = await this.prisma.sale.findFirst({
      where: { commercialPolicyId: id, status: { in: ['DRAFT', 'CONFIRMED'] } },
      select: { id: true },
    });
    if (openSale)
      throw new BadRequestException(
        'Cannot deactivate commercial policy with open sales',
      );

    const openReceivable = await this.prisma.accountReceivable.findFirst({
      where: {
    ...
    if (openReceivable)
      throw new BadRequestException(
        'Cannot deactivate commercial policy with open receivables',
      );
  }
    ...
    return {
      ...
      defaultCreditLimit:
        policy.defaultCreditLimit === null
          ? undefined
          : Number(policy.defaultCreditLimit),
  ...
  private toResponse(policy: CommercialPolicyRecord) {
    return {
      ...
      allowAdministrativeOverride: policy.allowAdministrativeOverride,
      maximumDiscountPercentage:
        policy.maximumDiscountPercentage?.toString() ?? '0',
      isActive: policy.isActive,
