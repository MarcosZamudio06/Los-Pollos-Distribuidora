...
describe('CommercialPoliciesService', () => {
  it('creates auditable policies with non-negative credit terms and blocking modes', async () => {
    ...
    const service = new CommercialPoliciesService(
      prisma as unknown as PrismaService,
    );
    prisma.commercialPolicy.create.mockImplementation(({ data }) =>
  ...
  it('rejects invalid credit terms, missing blocking modes, and invalid vigency before writing', async () => {
    ...
    await expect(
      service.create(
        { name: 'Bad', defaultCreditLimit: -1, effectiveFrom: '2026-06-19' },
        user,
      ),
    ).rejects.toBeInstanceOf(BadRequestException);
    await expect(
      service.create(
        { name: 'Bad', defaultCreditDays: -1, effectiveFrom: '2026-06-19' },
        user,
      ),
    ).rejects.toBeInstanceOf(BadRequestException);
    await expect(
    ...
    await expect(
      service.create({ name: 'Bad', isActive: true }, user),
    ).rejects.toBeInstanceOf(BadRequestException);
    await expect(
      service.create({ name: 'Bad', effectiveFrom: 'bad-date' }, user),
    ).rejects.toBeInstanceOf(BadRequestException);
    await expect(
      service.create(
        { name: 'Bad', effectiveFrom: '2026-06-20', effectiveTo: '2026-06-19' },
        user,
      ),
    ).rejects.toBeInstanceOf(BadRequestException);
  ...
  it('creates an ADMIN-only discount authorization within the commercial policy cap', async () => {
    ...
    prisma.discountAuthorization.create.mockImplementation(({ data }) =>
      Promise.resolve({ id: 'discount-auth-1', ...data }),
    );

    await expect(
      service.authorizeDiscount(
        'policy-1',
        {
  ...
  it('preserves historical commercial conditions already applied to sales or receivables', async () => {
    ...
    await expect(
      service.update('policy-1', { defaultCreditDays: 20 }, user),
    ).rejects.toBeInstanceOf(BadRequestException);
  ...
  it('updates audit user and soft-deactivates without physical delete', async () => {
    ...
    prisma.accountReceivable.findFirst.mockResolvedValueOnce(null);
    prisma.commercialPolicy.update.mockResolvedValueOnce(
      policy({ name: 'Updated', updatedByUserId: 'admin-1' }),
    );
    await expect(
      service.update('policy-1', { name: 'Updated' }, user),
    ).resolves.toMatchObject({ name: 'Updated', updatedByUserId: 'admin-1' });
    expect(prisma.commercialPolicy.update).toHaveBeenCalledWith({
      where: { id: 'policy-1' },
      data: expect.objectContaining({
        name: 'Updated',
        updatedByUserId: 'admin-1',
    ...
    await expect(
      service.update('missing', { name: 'Nope' }, user),
    ...
    await expect(service.deactivate('policy-1', user)).resolves.toMatchObject({
      isActive: false,
    });
    expect(prisma.commercialPolicy.update).toHaveBeenLastCalledWith({
      where: { id: 'policy-1' },
      data: { isActive: false, updatedByUserId: 'admin-1' },
    });
  });
});
