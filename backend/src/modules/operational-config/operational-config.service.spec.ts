...
describe('OperationalConfigService', () => {
  it('creates auditable allowed operational config and enforces location scope', async () => {
    ...
    const service = new OperationalConfigService(
    ...
    prisma.operationalConfig.create.mockImplementation(({ data }) =>
    ...
    await expect(
      service.create(
        {
          key: 'REQUIRED_DELIVERY_EVIDENCE',
          value: 'PHOTO',
          valueType: 'STRING',
          scope: 'LOCATION',
          locationId: 'loc-1',
    ...
    expect(prisma.operationalConfig.create).toHaveBeenCalledWith({
  ...
  it('rejects structural invariant changes and invalid operational values before writing', async () => {
    ...
    await expect(
      service.create(
        {
          key: 'DISABLE_LOCATION_INVENTORY',
          value: 'true',
          valueType: 'BOOLEAN',
          scope: 'GLOBAL',
          effectiveFrom: '2026-06-19',
    ...
    await expect(
      service.create(
        {
          key: 'DISABLE_ACCOUNTS_RECEIVABLE_FOR_CREDIT',
          value: 'true',
          valueType: 'BOOLEAN',
          scope: 'GLOBAL',
          effectiveFrom: '2026-06-19',
    ...
    await expect(
      service.create(
        {
          key: 'DISABLE_INVENTORY_TRANSFERS',
          value: 'true',
          valueType: 'BOOLEAN',
          scope: 'GLOBAL',
          effectiveFrom: '2026-06-19',
    ...
    await expect(
      service.create(
        {
          key: 'USE_INTERNAL_TICKET_AS_ONLY_MVP_DOCUMENT',
          value: 'true',
          valueType: 'BOOLEAN',
          scope: 'GLOBAL',
          effectiveFrom: '2026-06-19',
  ...
  it('updates audit user and soft-deactivates without physical delete', async () => {
    ...
    prisma.operationalConfig.update.mockResolvedValueOnce(
      config({ value: '45', updatedByUserId: 'admin-1' }),
    );

    await expect(
      service.update('config-1', { value: '45' }, user),
    ).resolves.toMatchObject({ value: '45', updatedByUserId: 'admin-1' });
    expect(prisma.operationalConfig.update).toHaveBeenCalledWith({
      where: { id: 'config-1' },
      data: expect.objectContaining({
        value: '45',
        updatedByUserId: 'admin-1',
    ...
    await expect(
      service.update('missing', { value: '30' }, user),
    ...
    await expect(service.deactivate('config-1', user)).resolves.toMatchObject({
      isActive: false,
    });
    expect(prisma.operationalConfig.update).toHaveBeenLastCalledWith({
      where: { id: 'config-1' },
