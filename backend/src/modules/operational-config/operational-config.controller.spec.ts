...
describe('OperationalConfigController API', () => {
  ...
  let service: jest.Mocked<
    Pick<
      ...
      'findAll' | 'create' | 'update' | 'deactivate'
    >
  >;
  ...
  beforeEach(async () => {
    ...
    service = {
      findAll: jest.fn().mockResolvedValue({ items: [configResponse] }),
      create: jest.fn().mockResolvedValue(configResponse),
      update: jest.fn().mockResolvedValue(configResponse),
      deactivate: jest
        .fn()
        .mockResolvedValue({ ...configResponse, isActive: false }),
    };
  ...
  it('exposes exact operational config routes with admin mutations and audit user', async () => {
    await request(app.getHttpServer())
      .get(
        '/api/operational-config?page=1&limit=10&key=REPORT_REFRESH_INTERVAL_SECONDS&scope=GLOBAL&isActive=true',
      )
    ...
    expect(service.create).toHaveBeenCalledWith(
      expect.objectContaining({ key: 'REPORT_REFRESH_INTERVAL_SECONDS' }),
      adminUser,
    );

    await request(app.getHttpServer())
      .patch('/api/operational-config/config-1')
      .set('Authorization', 'Bearer admin-token')
      .send({ value: '45' })
      .expect(200);
    ...
    await request(app.getHttpServer())
      .delete('/api/operational-config/config-1')
      .set('Authorization', 'Bearer admin-token')
      .expect(200);
  ...
  it('rejects non-admin access to operational config mutations', async () => {
    ...
    await request(app.getHttpServer())
      ...
      .set('Authorization', 'Bearer seller-token')
      .expect(403);
    expect(service.findAll).not.toHaveBeenCalledWith(
      expect.objectContaining({}),
    );
  });
});
