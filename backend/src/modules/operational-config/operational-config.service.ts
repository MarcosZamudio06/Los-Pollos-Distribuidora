...
import {
  ...
  ListOperationalConfigQueryDto,
  UpdateOperationalConfigDto,
} from './dto';

type OperationalConfigRecord = Prisma.OperationalConfigGetPayload<
  Record<string, never>
>;
...
const SAFE_STOCK_LOCATION_STRATEGIES = new Set([
  'REQUIRE_EXPLICIT_LOCATION',
  'USER_SELECTED_LOCATION',
  'CUSTOMER_ROUTE_LOCATION',
]);
...
export class OperationalConfigService {
  ...
  async create(
    dto: CreateOperationalConfigDto,
    currentUser: AuthenticatedUser,
  ) {
    const data = (await this.normalizeMutationData(
      dto,
      currentUser.id,
      true,
    )) as Prisma.OperationalConfigUncheckedCreateInput;
    const config = await this.prisma.operationalConfig.create({
      data,
    });
  ...
  async update(
    id: string,
    dto: UpdateOperationalConfigDto,
    currentUser: AuthenticatedUser,
  ) {
    ...
    const data = (await this.normalizeMutationData(
      ...
      currentUser.id,
      false,
    )) as Prisma.OperationalConfigUncheckedUpdateInput;
  ...
  private buildListWhere(
    query: ListOperationalConfigQueryDto,
  ): Prisma.OperationalConfigWhereInput {
    return {
  ...
  private buildPagination(query: ListOperationalConfigQueryDto): {
    skip?: number;
    take?: number;
  } {
  ...
  private async normalizeMutationData(
    ...
    return {
      ...
      ...(dto.effectiveFrom !== undefined
        ? { effectiveFrom: this.parseDate(dto.effectiveFrom, 'effectiveFrom') }
        : {}),
  ...
  private async validateConfig(dto: Partial<ConfigMutationDto>): Promise<void> {
    ...
    if (valueType !== undefined && valueType.length === 0)
      throw new BadRequestException('valueType is required');
    ...
    if (scope === 'LOCATION' && !dto.locationId)
      throw new BadRequestException(
        'locationId is required for LOCATION scope',
      );
    ...
    if (key === 'DRIVER_OFFLINE_POLICY')
      throw new BadRequestException(
        'DRIVER_OFFLINE_POLICY is blocked until business approval',
      );
    if (STRUCTURAL_INVARIANT_KEYS.has(key))
      throw new BadRequestException(
        'Operational config cannot modify structural MVP invariants',
      );
    if (!ALLOWED_KEYS.has(key))
      throw new BadRequestException(
        'Operational config key is not allowed for MVP',
      );
    ...
    if (key === 'REPORT_REFRESH_INTERVAL_SECONDS' && value !== undefined) {
      ...
      if (
        ...
        throw new BadRequestException(
          'REPORT_REFRESH_INTERVAL_SECONDS must be an integer between 1 and 60',
        );
      }
    }

    if (
      key === 'DEFAULT_SALE_STOCK_LOCATION_STRATEGY' &&
      value !== undefined &&
      !SAFE_STOCK_LOCATION_STRATEGIES.has(value)
    ) {
      throw new BadRequestException(
        'DEFAULT_SALE_STOCK_LOCATION_STRATEGY must preserve sale locationId',
      );
    }
  }
  ...
  private async assertActiveLocation(locationId: string): Promise<void> {
    ...
    if (!location)
      throw new BadRequestException(
        'LOCATION scope requires an active location',
      );
  }

  private async findActiveConfig(id: string): Promise<OperationalConfigRecord> {
    const config = await this.prisma.operationalConfig.findFirst({
  ...
  private toMutationSnapshot(
    config: OperationalConfigRecord,
  ): CreateOperationalConfigDto {
    return {
      key: config.key,
      value: config.value,
