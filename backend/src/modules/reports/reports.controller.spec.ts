...
describe('ReportsController', () => {
  ...
  function createController() {
    const service = {
      ...
      getDeliveryOperations: jest
        .fn()
        .mockResolvedValue({ deliverySummary: {} }),
  ...
  it('exposes report endpoints with expected role metadata', () => {
    ...
    expect(
      Object.getOwnPropertyNames(ReportsController.prototype)
  ...
  it('wraps dashboard response and passes query/current user to service', async () => {
    ...
    await expect(controller.getDashboard(query, user)).resolves.toEqual({
      success: true,
  ...
  it('wraps sales-daily response and passes query/current user to service', async () => {
    ...
    await expect(controller.getSalesDaily(query, user)).resolves.toEqual({
      success: true,
  ...
  it('wraps inventory low stock response and passes query/current user to service', async () => {
    ...
    await expect(controller.getInventoryLowStock(query, user)).resolves.toEqual(
      {
        success: true,
  ...
  it('wraps cash closing response and passes query/current user to service', async () => {
    ...
    await expect(controller.getCashClosing(query, user)).resolves.toEqual({
      success: true,
  ...
  it('wraps inventory-by-location response and passes query/current user to service', async () => {
    ...
    await expect(
      controller.getInventoryByLocation(query, user),
    ).resolves.toEqual({
      success: true,
  ...
  it('wraps accounts-receivable response and passes query/current user to service', async () => {
    ...
    await expect(
      controller.getAccountsReceivable(query, user),
    ).resolves.toEqual({
      success: true,
  ...
  it('wraps delivery-operations response and passes query/current user to service', async () => {
    ...
    await expect(
      controller.getDeliveryOperations(query, user),
    ).resolves.toEqual({
      success: true,
