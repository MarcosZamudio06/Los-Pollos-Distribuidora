...
describe('Delivery controllers', () => {
  ...
  it('passes route order assignment to the service with the current user', async () => {
    ...
    await expect(
      controller.assignOrders('route-1', body, user),
    ).resolves.toEqual({
      success: true,
  ...
  it('passes evidence, collection, incident, and settlement commands to the service with the current user', async () => {
    const service = {
      ...
      registerCollection: jest
        .fn()
        .mockResolvedValue({ payment: { id: 'payment-1' } }),
      registerIncident: jest
        .fn()
        .mockResolvedValue({ deliveryOrder: { id: 'order-1' } }),
      openSettlement: jest.fn().mockResolvedValue({ id: 'settlement-1' }),
      closeSettlement: jest
        .fn()
        .mockResolvedValue({ id: 'settlement-1', status: 'CLOSED' }),
      reopenSettlement: jest
        .fn()
    ...
    await expect(
      orderController.captureEvidence('order-1', evidenceDto, user),
    ).resolves.toEqual({
      success: true,
