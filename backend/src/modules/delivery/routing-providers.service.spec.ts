...
describe('RoutingProvidersService', () => {
          ...
          features: [
            {
              geometry: { coordinates: [-96.1342, 19.1738] },
              properties: {
                name: 'Avenida Salvador Díaz Mirón',
                city: 'Veracruz',
                country: 'México',
              },
            },
  ...
  it('sends a closed single-vehicle problem to VROOM and rejects unassigned jobs', async () => {
    ...
    await expect(
      service.optimizeStops(
        [-96.1421, 19.1802],
        [
          { saleId: 'sale-1', longitude: -96.1342, latitude: 19.1738 },
          { saleId: 'sale-2', longitude: -96.12, latitude: 19.16 },
        ],
      ),
  ...
  it('requests the final OSRM route as full GeoJSON', async () => {
    ...
    await expect(
      ...
      expect.objectContaining({ distanceMeters: 8600, durationSeconds: 1440 }),
    );
    const url = new URL((fetch as jest.Mock).mock.calls[0][0]);
