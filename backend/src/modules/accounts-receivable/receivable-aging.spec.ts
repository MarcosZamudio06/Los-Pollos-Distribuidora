...
describe('calculateReceivableAging', () => {
  ...
  it.each([
    [
      'more than seven calendar days away',
      '2026-07-24T06:00:00.000Z',
      AgingStatus.CURRENT,
      0,
    ],
    [
      'exactly seven calendar days away',
      '2026-07-23T06:00:00.000Z',
      AgingStatus.DUE_SOON,
      0,
    ],
    [
      'on the due calendar day',
    ...
    expect(calculateReceivableAging(new Date(dueDate), 100, asOf)).toEqual({
      agingStatus,
      daysOverdue,
    });
  });
  ...
  it('resets aging when no balance remains', () => {
    expect(
      calculateReceivableAging(new Date('2026-07-01T06:00:00.000Z'), 0, asOf),
    ).toEqual({
  ...
  it('uses the configured operational timezone for calendar boundaries', () => {
    ...
    ).toEqual({ agingStatus: AgingStatus.OVERDUE, daysOverdue: 1 });
    expect(
      calculateReceivableAging(
        boundaryDueDate,
        100,
    ...
    ).toEqual({ agingStatus: AgingStatus.DUE_SOON, daysOverdue: 0 });
  });
});
