...
export class AccountsReceivableAgingJob implements OnApplicationBootstrap {
  ...
  async reconcile(asOf = new Date()): Promise<{
    skipped: boolean;
    examined?: number;
    updated?: number;
    partial?: boolean;
    ...
    this.logger.log({
      event: 'receivable-aging.started',
    ...
    try {
      while (true) {
        const batch = await this.prisma.$transaction(
          async (tx) => {
            const [{ acquired } = { acquired: false }] =
              await tx.$queryRawUnsafe<Array<{ acquired: boolean }>>(
                'SELECT pg_try_advisory_xact_lock($1) AS acquired',
                LOCK_ID,
              );
            if (!acquired)
              return { acquired: false as const, examined: 0, updated: 0 };
            ...
            for (const receivable of batch) {
              const aging = calculateReceivableAging(
                receivable.dueDate,
                Number(receivable.outstandingAmount.toString()),
                asOf,
              );
              if (
                aging.agingStatus !== receivable.agingStatus ||
                aging.daysOverdue !== receivable.daysOverdue
              ) {
        ...
        if (!batch.acquired) {
          ...
          this.logger.log({
            event: 'receivable-aging.skipped',
