...
describe('billing reportable notes phase 1 data contract', () => {
  it('introduces the external invoice aggregate without PaymentAllocation', () => {
    ...
    expect(model('LegalEntityOperationalLocation')).toMatch(
      /operationalLocationId\s+String/,
    );
    ...
    expect(model('BillingRequestSaleItem')).toMatch(
      /requestedSubtotal\s+Decimal/,
    );
    ...
    expect(model('InvoiceSaleDocument')).toContain(
      'billingRequestSaleDocumentId',
    );
  ...
  it('expands legacy links and backfills before contracting uniqueness', () => {
    ...
    expect(model('AccountReceivable')).not.toMatch(
      /billingRequestId\s+String\?\s+@unique/,
    );
