...
describe('UsersController API', () => {
    ...
    ])('rejects invalid %s query params before calling the service', async (_case, path, assertService) => {
        await (0, supertest_1.default)(app.getHttpServer())
            .get(path)
            .set('Authorization', 'Bearer admin-token')
            .expect(400);
    ...
    it('deactivates users through DELETE without physical deletion semantics', async () => {
        ...
        expect(usersService.deactivate).toHaveBeenCalledWith('user-1', adminUser.id, {
            reason: 'Left company',
        });
    });
});
