...
describe("role navigation", () => {
  ...
  it("declara roles explícitos en cada item de navegación", () => {
    ...
    expect(NAVIGATION_ITEMS.every((item) => item.allowedRoles.length > 0)).toBe(
      true,
    );
  });
  ...
  it("mantiene navegación enlazada a la matriz central de rutas", () => {
    expect(
      NAVIGATION_ITEMS.every(
        (item) => item.allowedRoles === ROUTE_ACCESS_ROLES[item.routeAccessKey],
      ),
    ).toBe(true);
    ...
    expect(ROUTE_ACCESS_ROLES.billingRemediations).toEqual([
      "ADMIN",
      "BILLING",
    ]);
    expect(ROUTE_ACCESS_ROLES.deliveryRouteDetail).toEqual([
      "ADMIN",
      "COLLECTIONS",
      "DRIVER",
    ]);
    expect(ROUTE_ACCESS_ROLES.deliveryRoutes).toEqual([
      "ADMIN",
      "COLLECTIONS",
      "WAREHOUSE",
    ]);
  ...
  it("limita accesos por rol y deja fallback mínimo para roles desconocidos", () => {
    ...
    expect(getSidebarNavForRole("DRIVER").map((item) => item.to)).toEqual([
      "/",
      "/my-routes",
      "/reports",
    ]);
    expect(getSidebarNavForRole("BILLING").map((item) => item.to)).toEqual([
      "/",
      "/billing-requests",
      "/billing/reportable-notes",
      "/billing/remediations",
    ...
    expect(
      getSidebarNavForRole("DRIVER").find((item) => item.to === "/my-routes"),
    ).toEqual(
      expect.objectContaining({
  ...
  it("marca rutas de detalle en el acceso principal correcto", () => {
    ...
    expect(getActiveSidebarItemKey("/billing-requests/request-1")).toBe(
      "billing-requests",
    );
    expect(getActiveSidebarItemKey("/purchases/purchase-1")).toBe("purchases");
    expect(getActiveSidebarItemKey("/purchases/suppliers")).toBe(
      "purchase-suppliers",
    );
    expect(getActiveSidebarItemKey("/delivery-routes/route-1/evidence")).toBe(
      "delivery-routes",
    );
  ...
  it("renderiza cerrar sesión abajo sin botón interno de cerrar", () => {
    ...
    expect(html.lastIndexOf("Cerrar sesión")).toBeGreaterThan(
      html.lastIndexOf("Reportes"),
    );
  });
});
