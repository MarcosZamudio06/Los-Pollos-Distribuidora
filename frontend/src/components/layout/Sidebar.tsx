...
function getGroupedNavigation(items: NavigationItem[]) {
  return items.reduce<
    Array<{ section: NavigationItem["section"]; items: NavigationItem[] }>
  >((groups, item) => {
...
export function Sidebar({
  ...
  return (
    ...
    >
      ...
      {expanded && user && (
        <section className="relative mx-4 mt-4 rounded-2xl border border-white/10 bg-white/6 p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.08)]">
          ...
          <div className="mt-3 flex items-center gap-2">
            <Badge
              tone="gold"
              className="border-white/10 bg-white/10 text-[0.68rem] tracking-[0.16em] text-[var(--erp-brand-gold-soft)]"
            >
              {roleLabel}
            </Badge>
      ...
      <ScrollArea
        className={cn("relative mt-4 flex-1 px-3 pb-4", !expanded && "px-2")}
      >
                  ...
                  >
                    <span
                      ...
                      className={cn(
                        ...
                        isActive
                          ? "bg-[var(--erp-brand-gold)] text-[var(--erp-charcoal)]"
                          : "bg-white/8 text-white/80 group-hover:bg-white/12 group-hover:text-white",
                      )}
                    >
      ...
      <div
        className={cn(
          "relative mt-auto border-t border-white/10 p-4",
          !expanded && "px-2",
        )}
      >
        <NavLink
