  ...
  return (
    <span className="grid gap-1">
      ...
      {Boolean(error) && (
        <span
          role="alert"
          className="text-xs normal-case tracking-normal text-[var(--erp-danger)]"
        >
          No se pudo cargar el catálogo de{" "}
          {label?.toLocaleLowerCase("es-MX") ?? "opciones"}.
        </span>
      )}
      {error == null && !isLoading && options?.length === 0 && (
        <span className="text-xs normal-case tracking-normal text-[var(--erp-muted-foreground)]">
...
export function MiniAjaxSelect({
  className,
  endpoint,
  label,
  onChange,
  ...
  const results = useOperationalCatalog(
    `${endpoint}${separator}search=${encodeURIComponent(debounced)}&limit=20`,
    debounced.length >= 2,
  );
  ...
  return (
    <div className="grid gap-1">
      <input
        ...
        value={selected && selected.id === value ? display(selected) : search}
      />
      <datalist id={listId}>
        {results.data?.map((item) => (
          <option key={item.id} value={display(item)} />
        ))}
      </datalist>
      {results.isFetching && (
        <span className="text-xs normal-case tracking-normal text-[var(--erp-muted-foreground)]">
      ...
      {results.error && (
        <span
          ...
          className="text-xs normal-case tracking-normal text-[var(--erp-danger)]"
        >
          No se pudieron consultar las coincidencias.
        </span>
      )}
    </div>
  );
