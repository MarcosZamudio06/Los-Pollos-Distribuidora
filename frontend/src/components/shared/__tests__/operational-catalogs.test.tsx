...
describe("catálogos operativos", () => {
  ...
  it("muestra loading, vacío y error sin convertir el error en una opción", async () => {
    ...
    expect(
      [...container.querySelectorAll("option")].some(
        (item) => item.textContent === "No disponible",
      ),
    ).toBe(false);
  });
  ...
  it("espera el debounce, selecciona un ID inequívoco y conserva la etiqueta seleccionada", async () => {
    ...
    expect(fetch).toHaveBeenCalledWith(
      expect.stringContaining("search=Com"),
      expect.anything(),
    );
    await act(async () => {
    ...
    expect(
      container.querySelector("datalist option")?.getAttribute("value"),
    ).toBe("Comercial Centro · customer-1");
