      ...
      <section className="mx-auto grid max-w-7xl gap-5">
          ...
          <div className="relative grid gap-5 lg:grid-cols-[1fr_auto] lg:items-end">
            <div className="min-w-0">
              ...
              <h1 className="mt-4 max-w-4xl text-3xl font-black tracking-[-0.06em] text-[var(--erp-foreground)] sm:text-4xl">
                Recepción de mercancía por ubicación
              </h1>
              <p className="mt-3 max-w-3xl text-sm leading-6 text-[var(--erp-muted-foreground)]">
                Consulta entradas confirmadas, proveedor, ubicación receptora y
            ...
            <div className="grid gap-3 sm:grid-cols-3 lg:min-w-96">
              ...
              <Link
                className="inline-flex h-full min-h-16 items-center justify-center gap-2 rounded-2xl border border-[color:var(--erp-border)] bg-white px-4 text-sm font-black text-[var(--erp-brand-red)] transition hover:border-[var(--erp-brand-red)] hover:bg-[rgba(182,42,34,0.04)] focus-visible:ring-4 focus-visible:ring-[var(--erp-ring)] sm:col-span-1"
                to="/purchases/new"
              >
        ...
        <div className="grid gap-4 md:grid-cols-3">
          <Card className="p-5">
            ...
            <p className="mt-1 text-sm text-[var(--erp-muted-foreground)]">
              Suma de la consulta actual
            </p>
          </Card>
          <Card className="p-5">
            <p className="flex items-center gap-2 text-xs font-black uppercase tracking-[0.16em] text-[var(--erp-muted-foreground)]">
              <Truck className="h-4 w-4 text-[var(--erp-info)]" />
              Recepciones
            </p>
            <p className="mt-3 text-2xl font-black tracking-[-0.05em] tabular-nums">
              {confirmedCount}
            ...
            <p className="mt-1 text-sm text-[var(--erp-muted-foreground)]">
              Entradas confirmadas visibles
            </p>
          </Card>
          <Card className="p-5">
            <p className="flex items-center gap-2 text-xs font-black uppercase tracking-[0.16em] text-[var(--erp-muted-foreground)]">
              <FilterX className="h-4 w-4 text-[var(--erp-danger)]" />
              Canceladas
            </p>
            <p className="mt-3 text-2xl font-black tracking-[-0.05em] tabular-nums">
              {cancelledCount}
          ...
          <CardHeader className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <CardDescription className="mt-2">
                Refina proveedor, ubicación, estado y rango de recepción
                manteniendo la lógica actual.
              </CardDescription>
            </div>
            <Button
              disabled={!hasFilters}
        ...
        <Card className="overflow-hidden p-0">
          ...
          <div className="p-5">
            ...
            {!purchases.isLoading && !purchases.error && items.length === 0 && (
              ...
              <>
                <div className="grid gap-3 md:hidden">
                  {pagination.pageItems.map((purchase) => (
                    <Link
                      className="rounded-2xl border border-[color:var(--erp-border)] bg-white p-4 transition hover:border-[var(--erp-brand-gold)]"
                      key={purchase.id}
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                          ...
                          <p className="mt-1 truncate text-sm text-[var(--erp-muted-foreground)]">
                            {purchase.supplierName ??
                              supplierNameById.get(purchase.supplierId) ??
                              purchase.supplierId}
                          </p>
                        </div>
                      ...
                      <div className="mt-4 grid gap-2 text-sm text-[var(--erp-muted-foreground)]">
                        <span className="flex items-center gap-2">
                          ...
                          {purchase.locationName ??
                            locationNameById.get(purchase.locationId) ??
                            purchase.locationId}
                        </span>
                        <span className="flex items-center gap-2">
                      ...
                      <p className="mt-4 text-xl font-black tabular-nums">
