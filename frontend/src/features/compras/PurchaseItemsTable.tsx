...
function productPresentation(product: Product): ProductPresentation {
  return (product.presentationType ??
    product.presentation ??
    "KG") as ProductPresentation;
}
...
function buildItem(product: Product): PurchaseFormItem {
  return {
    ...
    appliedEquivalentFactor:
      product.pieceWeightEquivalent ?? product.equivalentWeightKg ?? null,
    ...
    <Card className="overflow-hidden p-0">
      <CardHeader className="flex flex-col gap-4 border-b border-[color:var(--erp-border)] bg-white/70 p-5 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <CardTitle className="mt-1">
            Captura por kilo, pieza o ambas
          </CardTitle>
          <CardDescription className="mt-2">
            Selecciona equivalencia oficial cuando captures kilos y piezas; la
            confirmación final queda en backend.
          </CardDescription>
        </div>
          ...
          <Select onChange={handleAdd} defaultValue="">
            ...
            {products.map((product) => (
              <option
                ...
                value={product.id}
              >
                {product.name} · {unitLabel(productUnit(product))}
              </option>
            ))}
          </Select>
        </label>
      ...
      <div className="p-5">
        {errors.map((error) => (
          <p
            role="alert"
            className="mt-3 rounded-2xl border border-[rgba(157,45,36,0.20)] bg-[rgba(157,45,36,0.08)] p-4 text-sm font-bold text-[var(--erp-danger)] first:mt-0"
            key={error}
          >
            {error}
          </p>
            ...
            <div className="grid gap-3 lg:hidden">
              {items.map((item) => {
                const subtotal =
                  (item.quantityKg > 0
                    ? item.quantityKg
                    : item.quantityPieces) * item.unitCost;
                return (
                  <div
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        ...
                        <p className="mt-1 text-sm text-[var(--erp-muted-foreground)]">
                          {presentationLabel(item.presentationType)}
                        </p>
                      </div>
                    ...
                    <div className="mt-4 grid gap-3 sm:grid-cols-2">
                      <label className="grid gap-2 text-xs font-black uppercase tracking-[0.14em] text-[var(--erp-muted-foreground)]">
                        Kilos
                        <Input
                          aria-label={`Kilos de ${item.productName}`}
                          disabled={item.unit === "PIECE"}
                          min="0"
                          onChange={(event) =>
                            onUpdateItem(item.productId, {
                              quantityKg: toNumberInput(event.target.value),
                            })
                          }
                          step="0.001"
                          type="number"
                      ...
                      <label className="grid gap-2 text-xs font-black uppercase tracking-[0.14em] text-[var(--erp-muted-foreground)]">
                        Piezas
                        <Input
                          aria-label={`Piezas de ${item.productName}`}
                          disabled={item.unit === "KG"}
                          min="0"
                          onChange={(event) =>
                            onUpdateItem(item.productId, {
                              quantityPieces: Math.trunc(
                      ...
                      <label className="grid gap-2 text-xs font-black uppercase tracking-[0.14em] text-[var(--erp-muted-foreground)]">
                        Costo unitario
                        <Input
                          aria-label={`Costo de ${item.productName}`}
                          min="0"
                          onChange={(event) =>
                            onUpdateItem(item.productId, {
                              unitCost: toNumberInput(event.target.value),
                            })
                          }
                          step="0.01"
                          type="number"
                      ...
                      <div className="rounded-xl bg-[var(--erp-surface)] p-3">
                        <span className="text-xs font-black uppercase tracking-[0.14em] text-[var(--erp-muted-foreground)]">
                          Subtotal
                        </span>
                    ...
                    item.availableEquivalences &&
                    item.availableEquivalences.length > 0 ? (
                      <label className="mt-3 grid gap-2 text-xs font-black uppercase tracking-[0.14em] text-[var(--erp-muted-foreground)]">
                        Equivalencia
                        <Select
                          aria-label={`Equivalencia de ${item.productName}`}
                          onChange={(event) => {
                            const equivalent = item.availableEquivalences?.find(
                              (candidate) =>
                                candidate.id === event.target.value,
                            );
                            onUpdateItem(item.productId, {
                              appliedEquivalentFactor:
                                equivalent?.factor ?? null,
                        ...
                        >
                          ...
                          {item.availableEquivalences.map((equivalent) => (
                            <option key={equivalent.id} value={equivalent.id}>
                              {decimal(equivalent.factor)} kg/pza
                            </option>
                          ))}
                        </Select>
                      </label>
                        ...
                        <Td className="text-[var(--erp-muted-foreground)]">
                          ...
                          item.availableEquivalences.length > 0 ? (
                            <Select
                              ...
                              onChange={(event) => {
                                const equivalent =
                                  item.availableEquivalences?.find(
                                    (candidate) =>
                                      candidate.id === event.target.value,
                                  );
                                onUpdateItem(item.productId, {
        ...
        <div className="mt-5 flex flex-col gap-1 rounded-2xl bg-[var(--erp-charcoal)] px-5 py-4 text-white sm:flex-row sm:items-center sm:justify-end">
          <span className="text-sm font-bold text-white/65">
            Total de vista previa
          </span>
          <strong className="text-xl tabular-nums">{money(total)}</strong>
        </div>
      </div>
    </Card>
  );
