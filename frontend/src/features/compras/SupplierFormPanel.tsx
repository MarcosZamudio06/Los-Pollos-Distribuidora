...
export function SupplierFormPanel({
  ...
  onUpdate,
  supplier,
}: SupplierFormPanelProps) {
  const originalDraft = useMemo(
    () => createSupplierFormDraft(supplier),
    [supplier],
  );
        ...
        <CardHeader className="flex flex-row items-start justify-between gap-4 border-b border-[color:var(--erp-border)] p-5">
          <div>
            ...
            <p className="mt-2 text-sm text-[var(--erp-muted-foreground)]">
              Completa datos fiscales operativos mínimos: nombre, teléfono,
              email y dirección.
            </p>
          </div>
          <button
            aria-label="Cerrar panel de proveedor"
            className="rounded-xl border border-[color:var(--erp-border)] bg-white p-2 text-[var(--erp-muted-foreground)]"
            onClick={onClose}
            type="button"
          >
            <X className="h-4 w-4" />
        ...
        <CardContent className="p-5">
          <form className="grid gap-4" onSubmit={handleSubmit}>
            <label className={fieldClass}>
              ...
              />
              {errors.name && (
                <span className="text-[var(--erp-danger)]">{errors.name}</span>
              )}
            </label>
            <label className={fieldClass}>
              Teléfono
              ...
              />
              {errors.phone && (
                <span className="text-[var(--erp-danger)]">{errors.phone}</span>
              )}
            </label>
            <label className={fieldClass}>
              Email
              ...
              />
              {errors.email && (
                <span className="text-[var(--erp-danger)]">{errors.email}</span>
              )}
            </label>
            <label className={fieldClass}>
              Dirección
              ...
              />
              {errors.address && (
                <span className="text-[var(--erp-danger)]">
                  {errors.address}
                </span>
              )}
            </label>
            <div className="mt-2 flex justify-end gap-2">
              ...
              <Button
                disabled={isSaving || hasSupplierFormErrors(errors)}
                type="submit"
              >
