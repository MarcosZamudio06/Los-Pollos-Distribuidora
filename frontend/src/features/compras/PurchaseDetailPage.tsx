...
function InfoCard({
  ...
  return (
    <Card className="p-5">
      ...
      <div className="mt-3 text-lg font-black tracking-[-0.03em] text-[var(--erp-foreground)]">
        {value}
      </div>
    </Card>
  );
  ...
  return (
    <main className="min-h-screen bg-[var(--erp-background)] px-4 py-6 text-[var(--erp-foreground)] sm:px-6 lg:px-8">
      <section className="mx-auto grid max-w-7xl gap-5">
        <header className="relative overflow-hidden rounded-[2rem] border border-black/10 bg-[var(--erp-charcoal)] p-6 text-white shadow-[0_24px_80px_rgba(17,24,21,0.18)] sm:p-7">
          ...
          <div className="relative grid gap-5 lg:grid-cols-[1fr_auto] lg:items-end">
            <div className="min-w-0">
              ...
              <h1 className="mt-4 max-w-4xl text-3xl font-black tracking-[-0.06em] text-white sm:text-4xl">
                Trazabilidad de entrada y movimientos
              </h1>
              <p className="mt-3 max-w-3xl text-sm leading-6 text-white/72">
                Revisa proveedor, ubicación receptora, partidas, totales y
                movimientos generados por la compra.
              </p>
            </div>
            <Link
        ...
        {!purchase.isLoading && !purchase.error && !detail && (
          <p className="rounded-2xl border border-dashed border-[color:var(--erp-border)] bg-white p-6 text-sm text-[var(--erp-muted-foreground)]">
        ...
        {detail && (
          <>
            <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              <InfoCard
                icon={
                  <ClipboardList className="h-4 w-4 text-[var(--erp-info)]" />
                }
                label="Número"
                value={detail.purchaseNumber ?? detail.id}
              />
              <InfoCard
                icon={
              ...
              <InfoCard
                ...
                label="Ubicación receptora"
                value={detail.locationName ?? detail.locationId}
              />
              <InfoCard
                label="Estado"
                value={
                  <Badge tone={purchaseStatusTone(detail.status)}>
              ...
              <div className="grid gap-4 md:grid-cols-[1fr_1fr_auto] md:items-end">
                ...
                <div className="grid gap-2 text-sm text-[var(--erp-muted-foreground)]">
                  ...
                  <span className="flex items-center gap-2">
                    ...
                    {detail.userName ?? detail.userId ?? "Sin usuario"}
                  </span>
                </div>
                <Button
                  disabled={!canCancel}
                    ...
                    <div className="mt-3 grid grid-cols-2 gap-3 text-sm">
                      <span className="text-[var(--erp-muted-foreground)]">
                        ...
                        <strong className="block text-[var(--erp-foreground)]">
                          {decimal(item.quantityKg)}
                        </strong>
                      </span>
                      <span className="text-[var(--erp-muted-foreground)]">
                        Piezas{" "}
                        <strong className="block text-[var(--erp-foreground)]">
                          {decimal(item.quantityPieces, 0)}
                        </strong>
                      </span>
                      <span className="text-[var(--erp-muted-foreground)]">
                        Costo{" "}
                        ...
                        >
                          ...
                          <Td className="text-[var(--erp-muted-foreground)]">
                            {item.appliedEquivalentFactor
                              ? `${decimal(item.appliedEquivalentFactor)} kg/pza`
                              : "Sin equivalencia aplicada"}
                          </Td>
            ...
            <Card className="overflow-hidden p-0">
              ...
              {(detail.inventoryMovements ?? []).length === 0 ? (
                <p className="m-5 rounded-2xl border border-dashed border-[color:var(--erp-border)] bg-[var(--erp-surface)] p-6 text-sm text-[var(--erp-muted-foreground)]">
                  Sin movimientos asociados en la respuesta.
                </p>
              ) : (
                      ...
                      <tbody>
                        {(detail.inventoryMovements ?? []).map((movement) => (
                          ...
                          >
                            ...
                            <Td>{movement.type}</Td>
                            <Td>
                              {movement.locationName ?? movement.locationId}
                            </Td>
                            <Td className="text-right tabular-nums">
                              {decimal(movement.quantityKg)}
                            </Td>
                            <Td className="text-right tabular-nums">
                              {decimal(movement.quantityPieces, 0)}
                            </Td>
                            <Td>
                              {decimal(movement.newQuantityKg)} kg ·{" "}
                              {decimal(movement.newQuantityPieces, 0)} pzas
                            </Td>
                            <Td className="text-[var(--erp-muted-foreground)]">
