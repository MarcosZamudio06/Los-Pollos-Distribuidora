...
export const purchasesService = {
  async listPurchases(
    ...
    const response = await apiClient.get<ListEnvelope<PurchaseListItem>>(
      withParams("/purchases", filters),
      {
  ...
  async getPurchase(id: string, accessToken?: string | null) {
    const response = await apiClient.get<ItemEnvelope<PurchaseDetail>>(
      `/purchases/${id}`,
      {
        headers: authHeaders(accessToken),
      },
  ...
  async listSuppliers(
    ...
    const response = await apiClient.get<ListEnvelope<Supplier>>(
      withParams("/suppliers", filters),
      {
        headers: authHeaders(accessToken),
      },
    );
  ...
  async deactivateSupplier(id: string, accessToken?: string | null) {
    const response = await apiClient.delete<ItemEnvelope<Supplier>>(
      `/suppliers/${id}`,
      {
        headers: authHeaders(accessToken, crypto.randomUUID()),
      },
  ...
  async listLocations(
    ...
    const response = await apiClient.get<ListEnvelope<OperationalLocation>>(
