...
const mockState = vi.hoisted(() => ({
  ...
  products: {
    data: [
      {
        activeEquivalences: [
          { factor: 2.4, id: "eq-1", unitFrom: "PIECE", unitTo: "KG" },
        ],
        id: "prod-1",
        name: "Pollo entero",
  ...
  purchase: {
    data: null as PurchaseDetail | null,
    error: null,
    isLoading: false,
  },
  purchases: {
    data: { items: [] as PurchaseDetail[] },
    error: null,
    isLoading: false,
  },
  suppliers: {
    data: [{ id: "sup-1", name: "Granja Norte", isActive: true }] as Supplier[],
...
describe("TASK-062 purchase UI behavior", () => {
  ...
  it("muestra listado de compras con filtros y columnas requeridas", () => {
    mockState.purchases = {
      data: { items: [{ ...purchase, locationName: undefined }] },
      error: null,
      isLoading: false,
    };
  ...
  it("muestra selector de proveedor activo", () => {
    const html = renderToStaticMarkup(
      <SupplierSelector onChange={() => undefined} value="" />,
    );
  ...
  it("deja vacíos los inputs numéricos cuyo valor inicial es cero", () => {
    ...
    const piecesInputs = Array.from(
      container.querySelectorAll<HTMLInputElement>(
        '[aria-label="Piezas de Pollo entero"]',
      ),
    );
    const costInputs = Array.from(
      container.querySelectorAll<HTMLInputElement>(
        '[aria-label="Costo de Pollo entero"]',
      ),
    );
    ...
    expect(
      piecesInputs.every((input) => input.value === "" && input.disabled),
    ).toBe(true);
  ...
  it("expone detalle con items, movimientos y acción de cancelación", () => {
    ...
    const html = renderToStaticMarkup(
      <MemoryRouter initialEntries={["/purchases/pur-1"]}>
        <Routes>
          <Route
  ...
  it("requiere motivo para cancelar y advierte inventario negativo", () => {
    const html = renderToStaticMarkup(
      <CancelPurchaseDialog onClose={() => undefined} purchase={purchase} />,
    );
  ...
  it("muestra acción de desactivación solo para ADMIN", () => {
    ...
    const warehouseHtml = renderToStaticMarkup(
      <MemoryRouter>
        <SuppliersPage />
      </MemoryRouter>,
    );
  ...
  it("solicita confirmación antes de desactivar un proveedor", async () => {
    mockState.deactivateSupplier.mutateAsync.mockResolvedValue({
      id: "sup-1",
      isActive: false,
      name: "Granja Norte",
    });
    ...
    try {
      ...
      if (!deactivateButton)
        throw new Error("Deactivate supplier button not found");
      ...
      const confirmButton = Array.from(
        ...
        button.textContent?.includes("Confirmar desactivación"),
      );
      if (!confirmButton)
        throw new Error("Confirm deactivation button not found");
      await act(async () => confirmButton.click());

      expect(mockState.deactivateSupplier.mutateAsync).toHaveBeenCalledTimes(1);
      expect(mockState.deactivateSupplier.mutateAsync).toHaveBeenCalledWith(
        "sup-1",
      );
    } finally {
  ...
  it("muestra estados vacío, carga y error para proveedores", () => {
    ...
    ).toContain("No hay proveedores para estos filtros.");
    ...
    ).toContain("Cargando proveedores...");
    ...
    ).toContain("No se pudieron cargar los proveedores.");
  });
});
