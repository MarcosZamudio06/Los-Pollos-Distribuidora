...
describe("supplier form utilities — normalization", () => {
  it("trims and collapses repeated spaces in text fields", () => {
    expect(collapseSupplierSpaces("   Granja   del   Norte   ")).toBe(
      "Granja del Norte",
    );
    expect(normalizeSupplierTextInput("  Carretera  Xalapa-Veracruz  ")).toBe(
      "Carretera Xalapa-Veracruz",
    );
  });

  it("normalizes email to lowercase without spaces for backend normalization parity", () => {
    expect(cleanSupplierEmail("  Ventas@GranjNorte.com.mx  ")).toBe(
      "ventas@granjnorte.com.mx",
    );
    expect(cleanSupplierEmail("ventas @granjanorte.com.mx")).toBe(
      "ventas@granjanorte.com.mx",
    );
  });
...
describe("supplier form utilities — map draft to backend payload", () => {
  ...
  it("maps only changed fields to a partial update payload", () => {
    const payload = toUpdateSupplierPayload(
      { ...validDraft, email: "  nuevo@granjanorte.com.mx  " },
      validDraft,
    );
...
describe("supplier form utilities — validation", () => {
  ...
  it("rejects an email missing the domain dot", () => {
    expect(
      validateSupplierField("email", {
        ...validDraft,
        email: "ventas@granjanorte",
      }),
    ).toBe("El email del proveedor debe tener un formato válido.");
  });
  ...
  it("accepts a well-formed email with subdomain", () => {
    expect(
      validateSupplierField("email", {
        ...validDraft,
        email: "Ventas@Corpo.GranjaNorte.com.mx",
      }),
    ).toBeNull();
  });
...
describe("supplier form utilities — legacy null edit completion", () => {
  ...
  it("edit-completes a valid mix of null and populated fields from legacy records", () => {
    ...
    const payload = toUpdateSupplierPayload(
      { ...draft, email: "  Contacto@GranjaMítica.com.mx  " },
      draft,
    );
