...
describe("supplier service contracts", () => {
  ...
  it("lists suppliers with search and active status filters", async () => {
    vi.mocked(fetch).mockResolvedValueOnce(
      okJson({ items: [{ id: "sup-1", name: "Granja Norte" }] }),
    );

    const suppliers = await purchasesService.listSuppliers(
      { isActive: false, limit: 25, page: 1, search: "granja" },
      "access-token",
    );
    ...
    expect(request.url).toBe(
      "/api/suppliers?isActive=false&limit=25&page=1&search=granja",
    );
  ...
  it("creates suppliers through POST /api/suppliers with idempotency", async () => {
    vi.mocked(fetch).mockResolvedValueOnce(
      okJson({ id: "sup-1", name: "Granja Norte", isActive: true }),
    );
    ...
    expect(new Headers(request.init?.headers).get("idempotency-key")).toBe(
      "supplier-idempotency-key",
    );
  });

  it("updates suppliers through PATCH /api/suppliers/:id with partial fields", async () => {
    vi.mocked(fetch).mockResolvedValueOnce(
      okJson({ id: "sup-1", name: "Granja Norte Renovada" }),
    );

    await purchasesService.updateSupplier(
      "sup-1",
      { name: "Granja Norte Renovada" },
      "access-token",
    );
  ...
  it("deactivates suppliers through DELETE /api/suppliers/:id", async () => {
    vi.mocked(fetch).mockResolvedValueOnce(
      okJson({ id: "sup-1", isActive: false, name: "Granja Norte" }),
    );

    const supplier = await purchasesService.deactivateSupplier(
      "sup-1",
      "access-token",
    );
