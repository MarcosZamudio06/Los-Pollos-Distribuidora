...
export function SuppliersPage() {
  ...
  const activeCount = useMemo(
    () => items.filter((supplier) => supplier.isActive !== false).length,
    [items],
  );
    ...
    <main className="min-h-screen bg-[var(--erp-background)] px-4 py-6 text-[var(--erp-foreground)] sm:px-6 lg:px-8">
      <section className="mx-auto grid max-w-7xl gap-5">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          ...
          <Card className="p-5">
            ...
            <p className="mt-3 text-2xl font-black">{activeCount}</p>
          </Card>
          <Card className="p-5">
            <p className="text-xs font-black uppercase tracking-[0.16em] text-[var(--erp-muted-foreground)]">
              Acceso
            ...
            <p className="mt-3 flex items-center gap-2 text-sm font-bold">
              <ShieldAlert className="h-4 w-4 text-[var(--erp-brand-gold-deep)]" />{" "}
              {isAdmin
                ? "ADMIN con desactivación"
                : "WAREHOUSE sin desactivación"}
            </p>
          </Card>
        </div>
          ...
          <CardHeader className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <p className="mt-2 text-sm text-[var(--erp-muted-foreground)]">
                Buscar proveedor por nombre, teléfono o email y filtrar por
                estado.
              </p>
            </div>
            <Button
              disabled={!hasFilters}
              onClick={() =>
                setFilters({ isActive: "", limit: 50, page: 1, search: "" })
              }
              variant="outline"
            >
        ...
        <Card className="overflow-hidden p-0">
          <div className="flex flex-col gap-3 border-b border-[color:var(--erp-border)] bg-white/70 p-5 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <CardTitle className="mt-1">Proveedores registrados</CardTitle>
            </div>
          ...
          <div className="p-5">
            ...
            {!suppliers.isLoading && !suppliers.error && items.length === 0 && (
              <p className="rounded-2xl border border-dashed border-[color:var(--erp-border)] bg-[var(--erp-surface)] p-6 text-sm text-[var(--erp-muted-foreground)]">
                No hay proveedores para estos filtros.
              </p>
            )}
            {items.length > 0 && (
              <>
                        ...
                        >
                          <Td>
                            <p className="flex items-center gap-2 font-black">
                              <Building2 className="h-4 w-4 text-[var(--erp-brand-gold-deep)]" />
                              {supplier.name}
                            </p>
                          </Td>
                          <Td>
                          ...
                          <Td>
                            <Badge tone={supplierStatusTone(supplier.isActive)}>
                          ...
                          <Td className="text-right">
                            <div className="flex justify-end gap-2">
                              ...
                              {isAdmin && supplier.isActive !== false && (
                                <Button
                                  onClick={() => {
                                    setDeactivationError(null);
                                    setPendingDeactivation(supplier);
                                  }}
                                  size="sm"
                                  variant="outline"
                                >
      ...
      {editingSupplier !== undefined && (
        <SupplierFormPanel
          ...
          onUpdate={async (_supplierId, draft) => {
            await updateSupplier.mutateAsync(draft);
            setEditingSupplier(undefined);
          }}
        />
      )}
      <ConfirmationDialog
        ...
        description="Revise el proveedor antes de retirarlo de las operaciones activas."
      ...
      >
        <p>
          <strong>Proveedor:</strong> {pendingDeactivation?.name}
        </p>
        <p>
          <strong>Contacto:</strong>{" "}
          {pendingDeactivation?.phone ??
            pendingDeactivation?.email ??
            "Sin contacto registrado"}
        </p>
        <p className="text-[var(--erp-muted-foreground)]">
          El proveedor dejará de estar disponible para nuevas compras. Su
          historial permanecerá intacto.
        </p>
        {deactivationError && (
