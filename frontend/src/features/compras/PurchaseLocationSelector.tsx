...
export function PurchaseLocationSelector({
  ...
  const receiverLocations = useMemo(
    () =>
      (locations.data ?? []).filter((location) =>
        receiverTypes.has(location.type),
      ),
    [locations.data],
  );
  const selected = useMemo(
    () => receiverLocations.find((location) => location.id === value) ?? null,
  ...
  return (
    <Card className="p-5">
      ...
      {locations.isLoading && (
        <p className="mt-3 text-sm font-bold text-[var(--erp-info)]">
      ...
      {locations.error && (
        <p
          role="alert"
          className="mt-3 rounded-2xl border border-[rgba(157,45,36,0.20)] bg-[rgba(157,45,36,0.08)] p-4 text-sm font-bold text-[var(--erp-danger)]"
        >
          No se pudieron cargar ubicaciones operativas.
        </p>
      )}
      {error && (
        <p
      ...
      {selected && (
        <p className="mt-3 rounded-2xl border border-[rgba(214,155,45,0.28)] bg-[rgba(214,155,45,0.12)] p-4 text-sm text-[var(--erp-brand-gold-deep)]">
          Recepción en <strong>{selected.name}</strong>. Tipo:{" "}
          {locationTypeLabel(selected.type)}.
        </p>
      )}
    </Card>
  );
