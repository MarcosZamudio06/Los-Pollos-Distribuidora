...
export function createSupplierFormDraft(
  ...
  return {
    ...
    address: collapseSupplierSpaces(
      (supplier?.address as string | null | undefined) ?? "",
    ),
  };
}
...
export function toUpdateSupplierPayload(
  draft: SupplierFormDraft,
  original?: SupplierFormDraft | Supplier | null,
): UpdateSupplierPayload {
  const payload: UpdateSupplierPayload = {};
  const normalized = toCreateSupplierPayload(draft);
  const baseline = original
    ? toCreateSupplierPayload(createSupplierFormDraft(original as Supplier))
    : null;
  if (normalized.name && normalized.name !== baseline?.name)
    payload.name = normalized.name;
  if (normalized.phone && normalized.phone !== baseline?.phone)
    payload.phone = normalized.phone;
  if (normalized.email && normalized.email !== baseline?.email)
    payload.email = normalized.email;
  if (normalized.address && normalized.address !== baseline?.address)
