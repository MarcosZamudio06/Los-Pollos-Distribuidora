...
export function usePurchase(purchaseId?: string) {
  ...
  return useQuery({
    ...
    queryFn: () =>
      purchasesService.getPurchase(purchaseId as string, accessToken),
  });
}
...
export function useSuppliers(filters: SupplierListFilters | string = "") {
  ...
  const resolvedFilters =
    typeof filters === "string"
      ? { isActive: true, limit: 50, page: 1, search: filters }
      : filters;
  return useQuery({
...
export function useCreateSupplier() {
  ...
  return useMutation({
    mutationFn: (payload: CreateSupplierPayload) =>
      purchasesService.createSupplier(payload, accessToken),
    onSuccess: () =>
...
export function useUpdateSupplier(supplierId: string) {
  ...
  return useMutation({
    mutationFn: (payload: UpdateSupplierPayload) =>
      purchasesService.updateSupplier(supplierId, payload, accessToken),
    onSuccess: () =>
      void queryClient.invalidateQueries({ queryKey: ["suppliers"] }),
  });
}
...
export function useDeactivateSupplier() {
  ...
  return useMutation({
    mutationFn: (supplierId: string) =>
      purchasesService.deactivateSupplier(supplierId, accessToken),
    onSuccess: () =>
      void queryClient.invalidateQueries({ queryKey: ["suppliers"] }),
  });
}
...
export function usePurchaseLocations(search = "") {
  ...
  return useQuery({
    ...
    queryFn: () =>
      purchasesService.listLocations(
        { isActive: true, limit: 50, page: 1, search },
        accessToken,
      ),
  });
}
