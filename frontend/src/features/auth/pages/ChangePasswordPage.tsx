...
export function ChangePasswordPage() {
  ...
  const nextPath =
    state?.from?.pathname && state.from.pathname !== "/change-password"
      ? state.from.pathname
      : "/";
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setFormError(
        "No fue posible actualizar la contraseña. Verifica tu contraseña actual e inténtalo de nuevo.",
      );
    } finally {
    ...
    <main className="relative flex min-h-screen items-center justify-center overflow-hidden bg-[var(--erp-background)] px-5 py-10 text-[var(--erp-foreground)]">
      ...
      <Card className="relative w-full max-w-xl overflow-hidden border-[color:var(--erp-border)] bg-white/94 p-0 shadow-[var(--erp-shadow-elevated)] backdrop-blur">
        ...
        <CardContent className="p-6 sm:p-8">
          ...
          <p className="mt-6 text-xs font-black uppercase tracking-[0.2em] text-[var(--erp-danger)]">
            Acción obligatoria
          </p>
          ...
          <p className="mt-3 text-sm leading-6 text-[var(--erp-muted-foreground)]">
            Debes reemplazar la contraseña temporal antes de acceder a los
            módulos operativos.
          </p>
          ...
          >
            ...
            >
              ...
              <span
                ...
                id="password-help"
              >
                Mínimo 10 caracteres y diferente a la contraseña actual.
              </span>
            </label>
            <label
              className="grid gap-2 text-sm font-bold"
            ...
            <Button
              className="mt-2 h-12 w-full rounded-2xl font-black uppercase tracking-[0.12em]"
