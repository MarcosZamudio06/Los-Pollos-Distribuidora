...
describe("branch orders service", () => {
  ...
  it("requests orders for exactly one operational location", async () => {
    ...
    await ordersService.listBranchOrders(
      { locationId: "loc-1", paymentType: "CASH_SALE" },
      "access-token",
    );

    expect(fetch).toHaveBeenCalledWith(
      "/api/sales/orders?locationId=loc-1&paymentType=CASH_SALE",
  ...
  it("requests two persisted orders when the periodic cleanup runs", async () => {
    ...
    await ordersService.listBranchOrders(
      { locationId: "loc-1", limit: 2 },
      "access-token",
    );

    expect(fetch).toHaveBeenCalledWith(
      "/api/sales/orders?locationId=loc-1&limit=2",
      expect.any(Object),
    );
  });
  ...
  it("accumulates every socket sale until the scheduled cleanup", () => {
    ...
    expect(mergeOrders([firstOrder, secondSale], [latestSale])).toEqual([
      latestSale,
      secondSale,
      firstOrder,
    ]);
  ...
  it("uses the two persisted orders returned by REST during cleanup", async () => {
    ...
    vi.mocked(fetch).mockResolvedValueOnce(
      okJson({ items: [latestSale, secondSale] }),
    );

    await expect(
      ordersService.listBranchOrders(
        { locationId: "loc-1", limit: 2 },
        "access-token",
      ),
    ).resolves.toEqual({
