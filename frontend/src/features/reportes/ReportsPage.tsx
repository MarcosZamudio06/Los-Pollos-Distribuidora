...
type ReportKey =
  "sales" | "cash" | "low-stock" | "inventory" | "receivables" | "delivery";
...
const tableShellClass =
  "mt-4 overflow-x-auto rounded-[1.2rem] border border-[color:var(--erp-border)] bg-white/70";
...
function AdminUserFilter({
  ...
  return (
    <label className={filterLabelClass}>
      Usuario
      <MiniAjaxSelect
        className="h-10 rounded-xl border border-[color:var(--erp-border)] bg-[var(--erp-surface-elevated)] px-3.5 text-sm font-semibold normal-case tracking-normal"
        endpoint="/users?status=active"
        label="Usuario"
        onChange={onChange}
...
function CatalogFilter({
  ...
  return (
    <label className={`${filterLabelClass} ${constrainWidth ? "min-w-0" : ""}`}>
      {label}
      <CatalogSelect
        className={`h-10 rounded-xl border border-[color:var(--erp-border)] bg-[var(--erp-surface-elevated)] px-3.5 text-sm font-semibold normal-case tracking-normal ${constrainWidth ? "w-full min-w-0 max-w-full" : ""}`}
        error={catalog.error}
...
function FreshnessNotice({ data }: { data?: ReportFreshness }) {
  ...
  const isStale = Boolean(
    data.isStale || Number(data.freshnessSeconds ?? 0) > 60,
  );
    ...
    <Alert tone={isStale ? "warning" : "info"}>
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        ...
        <p className="text-sm leading-6">
          Datos al: {formatDateTime(data.dataAsOf)} · Frescura:{" "}
          {formatNumber(data.freshnessSeconds)}s{" "}
          {isStale
            ? "· Datos fuera del objetivo de 60s"
            : "· Dentro del objetivo de 60s"}
        </p>
      </div>
    </Alert>
  );
...
function UnauthorizedState() {
  return (
    <Card className="p-8 text-center">
      ...
      <Badge className="mt-5" tone="red">
        Sin autorización
      </Badge>
      <h2 className="mx-auto mt-4 max-w-2xl text-2xl font-black tracking-[-0.05em] text-[var(--erp-foreground)] sm:text-3xl">
        No tienes permisos para este reporte
      </h2>
      <p className="mx-auto mt-3 max-w-xl text-sm leading-6 text-[var(--erp-muted-foreground)]">
        Revisa tu rol o vuelve a iniciar sesión si el acceso cambió.
      </p>
      <Link
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm">
            Verifica los filtros y la conexión con la API de reportes.
          </p>
        </div>
...
function EmptyState({ children }: { children: ReactNode }) {
  return (
    <Card className="p-8 text-center">
      ...
      <Badge className="mt-5" tone="amber">
        Sin resultados
      </Badge>
      <h2 className="mx-auto mt-4 max-w-2xl text-2xl font-black tracking-[-0.05em] text-[var(--erp-foreground)] sm:text-3xl">
...
function StatCard({
  ...
  icon?: ReactNode;
  label: string;
  tone?: "danger" | "gold" | "info" | "success";
  value: string;
}) {
  const toneClass = {
    danger:
      "text-[var(--erp-danger)] bg-[rgba(157,45,36,0.08)] border-[rgba(157,45,36,0.18)]",
    ...
    success:
      "text-[var(--erp-success)] bg-[rgba(63,123,65,0.08)] border-[rgba(63,123,65,0.20)]",
  }[tone];
  ...
  return (
    <Card className="p-5 transition duration-200 hover:-translate-y-0.5 hover:shadow-[var(--erp-shadow-elevated)]">
      ...
      <p className="mt-4 break-words text-2xl font-black tracking-[-0.06em] text-[var(--erp-foreground)] sm:text-3xl">
        {value}
      </p>
...
export function CashClosingSummary({ data }: { data: CashClosingReport }) {
  return (
    <div className="grid gap-5 xl:grid-cols-2">
      <MoneyGroupsTable
        emptyLabel="Ventas de contado por método."
        groups={data.cashSales}
        title="Ventas de contado"
      />
      <CreditSalesSummary creditSales={data.creditSales} />
      <MoneyGroupsTable
        emptyLabel="Pagos directos de cuentas por cobrar."
        groups={data.accountsReceivablePayments}
        title="Cobranza en caja"
      />
      <MoneyGroupsTable
        emptyLabel="Cobros pendientes o liquidados de rutas."
        groups={data.routeCollections}
        title="Cobros en ruta"
      />
      <MoneyGroupsTable
        emptyLabel="Transferencias y depósitos confirmados."
        groups={data.bankTransfersAndDeposits}
        title="Transferencias y depósitos"
      />
      <MoneyGroupsTable
        emptyLabel="Resumen agrupado por banco."
        groups={data.paymentsByBank}
        title="Pagos por banco"
      />
    </div>
  );
}

function MoneyGroupsTable({
  emptyLabel,
  groups,
  title,
}: {
            ...
            {(groups ?? []).length === 0 ? (
              ...
              groups?.map((group, index) => (
                <tr
                  ...
                  key={`${group.paymentMethod ?? group.method ?? group.bankName ?? "grupo"}-${index}`}
                >
...
function ReportSection<TData extends ReportFreshness>({
  children,
  emptyMessage,
  query,
}: {
  ...
  return (
    <div className="space-y-5">
      ...
      {query.isFetching && (
        <Alert tone="info">Actualizando datos del reporte…</Alert>
      )}
...
function SalesDailyReport({ isAdmin }: { isAdmin: boolean }) {
  ...
  return (
    <ReportPanel
      filters={
        <>
          ...
          <CatalogFilter
            constrainWidth
            endpoint="/locations?isActive=true&limit=100"
            label="Ubicación"
          ...
          <NativeSelect
            label="Tipo de venta"
            onChange={(paymentType) => setFilters({ ...filters, paymentType })}
          ...
          <NativeSelect
            ...
            options={[
              { label: "Todos", value: "" },
              { label: "Ticket de báscula", value: "SCALE_TICKET" },
              { label: "Nota sencilla", value: "SIMPLE_NOTE" },
              { label: "Nota grande", value: "LARGE_NOTE" },
              { label: "Comprobante interno", value: "INTERNAL_RECEIPT" },
            ]}
    ...
    >
      <ReportSection<SalesDailyReport>
        emptyMessage="Cambia la fecha, ubicación o vendedor para consultar operaciones confirmadas."
        query={query}
      >
        {(data) => (
          <>
            <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
              ...
              />
              <StatCard
                detail="Solo ventas de contado"
                icon={<Banknote className="h-5 w-5" />}
                label="Contado"
                tone="success"
                value={formatMoney(data.summary?.cash)}
              />
              <StatCard
                detail="No se trata como efectivo"
                icon={<CreditCard className="h-5 w-5" />}
                label="Crédito"
                value={formatMoney(data.summary?.credit)}
              />
              <StatCard
                detail="Notas separadas"
                icon={<FileText className="h-5 w-5" />}
                label="Cancelado"
                tone="danger"
                value={formatMoney(data.summary?.canceled)}
              />
            </div>
            <MoneyGroupsTable
              emptyLabel="Importes derivados de pagos no cancelados."
              groups={data.byPaymentMethod}
            ...
            <Card className="overflow-hidden p-5">
              <CardHeader>
                ...
                <CardDescription>
                  Cliente, vendedor, ubicación, documento, método y total.
                </CardDescription>
              </CardHeader>
              <CardContent className={tableShellClass}>
                <Table className="min-w-[1080px]">
                  ...
                  <tbody>
                    {(data.items ?? []).map((item, index) => (
                      <tr
                        className={tableRowClass}
                        key={item.saleId ?? item.saleNumber ?? index}
                      >
                        <Td className="font-black">
                          {item.saleNumber ?? "Sin folio"}
                        </Td>
                        <Td>
                          {item.customerName ??
                            item.clientName ??
...
function CashClosingReport({ isAdmin }: { isAdmin: boolean }) {
  ...
  return (
    ...
    >
      <ReportSection<CashClosingReport>
        emptyMessage="No hay ventas, pagos o cobros de ruta para los filtros seleccionados."
        query={query}
      >
  ...
  return (
    <ReportPanel
      filters={
        <>
          ...
          />
          <CatalogFilter
            endpoint="/products?isActive=true&limit=100"
            label="Producto"
            onChange={(productId) => onChange({ ...filters, productId })}
            value={filters.productId}
          />
          <CatalogFilter
            endpoint="/categories?isActive=true&limit=100"
            label="Categoría"
    ...
    >
      <ReportSection<InventoryReport>
        emptyMessage="No hay saldos para los filtros seleccionados."
        query={query}
      >
        {(data) => (
          <Card className="overflow-hidden p-5">
            <CardHeader>
              <CardTitle>{title}</CardTitle>
              <CardDescription>
                Kilos, piezas, mínimos y último movimiento.
              </CardDescription>
            </CardHeader>
            <CardContent className={tableShellClass}>
              <Table className="min-w-[1080px]">
                <thead>
                  <tr>
                    ...
                    <Th className="text-right">Mínimo kg</Th>
                    <Th className="text-right">Mínimo piezas</Th>
                    <Th>Estado</Th>
                ...
                <tbody>
                  {(data.items ?? []).map((item) => (
                    ...
                    >
                      ...
                      <Td className={numericCellClass}>
                        {formatNumber(item.quantityPieces)}
                      </Td>
                      <Td className={numericCellClass}>
                        {formatNumber(item.minQuantityKg)}
                      </Td>
                      <Td className={numericCellClass}>
                        {formatNumber(item.minQuantityPieces)}
                      </Td>
                      <Td>
                        <Badge tone={item.isLowStock ? "red" : "green"}>
                          {item.isLowStock ? "Bajo stock" : "Suficiente"}
                        </Badge>
                      </Td>
...
function AccountsReceivableReport() {
  ...
  return (
    <ReportPanel
      filters={
        <>
          ...
          <NativeSelect
            label="Antigüedad"
            onChange={(agingStatus) => setFilters({ ...filters, agingStatus })}
            options={agingStatuses}
            value={filters.agingStatus}
          />
          <DateFilter
            label="Vence desde"
            onChange={(dueDateFrom) => setFilters({ ...filters, dueDateFrom })}
            value={filters.dueDateFrom}
          />
          <DateFilter
            label="Vence hasta"
            onChange={(dueDateTo) => setFilters({ ...filters, dueDateTo })}
    ...
    >
      <ReportSection<AccountsReceivableReport>
        emptyMessage="No hay cuentas por cobrar para los filtros seleccionados."
        query={query}
      >
        {(data) => (
          <>
            <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
              <StatCard
                detail="Saldo de origen"
                icon={<ClipboardList className="h-5 w-5" />}
                label="Original"
                value={formatMoney(data.summary?.originalBalance)}
              />
              <StatCard
                ...
                label="Pendiente"
                tone="gold"
                value={formatMoney(data.summary?.pendingBalance)}
              />
              <StatCard
                detail="Crédito atrasado"
                icon={<AlertTriangle className="h-5 w-5" />}
                label="Vencido"
                tone="danger"
                value={formatMoney(data.summary?.overdueBalance)}
              />
              <StatCard
                detail="Pagos del periodo"
                icon={<WalletCards className="h-5 w-5" />}
                label="Pagado"
                tone="success"
                value={formatMoney(data.summary?.paymentsInPeriod)}
              />
            </div>
            <Card className="overflow-hidden p-5">
              <CardHeader>
                ...
                <CardDescription>
                  Cliente, venta, vencimiento, folio físico, saldo y estado.
                </CardDescription>
              </CardHeader>
              <CardContent className={tableShellClass}>
                <Table className="min-w-[960px]">
                  ...
                  <tbody>
                    {(data.items ?? []).map((item, index) => (
                      <tr
                        className={tableRowClass}
                        key={item.accountReceivableId ?? index}
                      >
                        <Td className="font-bold">
                          {item.customerName ??
                            item.clientName ??
                        ...
                        <Td>{item.saleNumber ?? item.saleId ?? "Sin venta"}</Td>
                        <Td>{formatDateTime(item.dueDate)}</Td>
                        <Td>{item.physicalFolio ?? "Sin folio"}</Td>
                        <Td className={numericCellClass}>
                          {formatMoney(item.balance)}
                        </Td>
                        <Td>{item.status ?? "Sin estado"}</Td>
                        <Td>{item.agingStatus ?? "Sin antigüedad"}</Td>
                      </tr>
                    ))}
                  </tbody>
                </Table>
            ...
            <div className="grid gap-5 xl:grid-cols-2">
              <MoneyGroupsTable
                emptyLabel="Pagos por método."
                groups={data.paymentsByMethod}
                title="Pagos por método"
              />
              <MoneyGroupsTable
                emptyLabel="Pagos agrupados por banco."
                groups={data.paymentsByBank}
                title="Pagos por banco"
              />
            </div>
...
function DeliveryOperationsReport() {
  ...
  return (
    <ReportPanel
      filters={
        <>
          ...
          <TextFilter
            label="Ruta"
            onChange={(routeId) => setFilters({ ...filters, routeId })}
          ...
          <NativeSelect
            ...
            options={[
              {
                label: catalog.drivers.error
                  ? "Catálogo no disponible"
                  : catalog.drivers.isLoading
                    ? "Cargando…"
                    : "Todos",
                value: "",
              },
    ...
    >
      <ReportSection<DeliveryOperationsReport>
        emptyMessage="No hay pedidos, cobros o incidencias para el rango seleccionado."
        query={query}
      >
        {(data) => (
          <>
            <div className="grid gap-5 xl:grid-cols-2">
              <SummaryMapCard
                entries={data.deliverySummary}
                title="Pedidos por estado"
              />
              <SummaryMapCard
                entries={data.evidenceSummary}
                title="Evidencias por tipo"
              />
              <SummaryMapCard
                entries={data.settlementsSummary}
                title="Liquidaciones"
              />
              <MoneyGroupsTable
                emptyLabel="Cobros por ruta, método y vuelta."
                groups={data.collectionsSummary}
                title="Cobros en ruta"
              />
            </div>
            <Card className="overflow-hidden p-5">
              <CardHeader>
                ...
                <CardDescription>
                  Devoluciones, rechazos parciales, no entregas y créditos
                  atrasados.
                </CardDescription>
              </CardHeader>
              <CardContent className={tableShellClass}>
                <Table className="min-w-[760px]">
                  ...
                  <tbody>
                    {(data.incidents ?? []).map((incident, index) => (
                      <tr
                        className={tableRowClass}
                        key={`${incident.type ?? "incidencia"}-${index}`}
                      >
                        <Td className="font-bold">
                          {incident.routeName ?? "Sin ruta"}
                        </Td>
                        <Td>{incident.type ?? "Sin tipo"}</Td>
                        <Td>{incident.status ?? "Sin estado"}</Td>
                        <Td>{incident.severity ?? "Sin severidad"}</Td>
...
function SummaryMapCard({
  ...
  return (
    <Card className="overflow-hidden p-5">
      <CardHeader>
        <CardTitle>{title}</CardTitle>
        <CardDescription>Conteo operativo confirmado.</CardDescription>
      </CardHeader>
      <CardContent className="mt-4 space-y-3">
        {rows.length === 0 ? (
          <p className="rounded-2xl bg-[var(--erp-surface)] p-4 text-sm text-[var(--erp-muted-foreground)]">
            Sin datos registrados.
          </p>
        ) : (
          rows.map(([label, value]) => (
            <div
              className="flex items-center justify-between gap-4 rounded-2xl border border-[color:var(--erp-border)] bg-[var(--erp-surface)] px-4 py-3"
              key={label}
            >
              <span className="font-bold text-[var(--erp-foreground)]">
                {label}
              </span>
              <span className="text-2xl font-black tracking-[-0.05em] text-[var(--erp-info)] tabular-nums">
                {formatNumber(value)}
              </span>
            </div>
          ))
  ...
  return (
    <section className="space-y-5">
      <Card className="overflow-hidden">
        ...
        <CardContent className="grid gap-3 p-5 sm:p-6 md:grid-cols-2 xl:grid-cols-4">
          {filters}
        </CardContent>
      </Card>
      {children}
...
export function ReportsPage() {
  ...
  const visibleTabs = useMemo(
    () => reportTabs.filter((tab) => hasAccess(role, tab.roles)),
    [role],
  );
  const selectedReport = visibleTabs.some((tab) => tab.key === activeReport)
    ? activeReport
    : visibleTabs[0]?.key;
        ...
        <div className="relative overflow-hidden rounded-[2rem] border border-[color:var(--erp-border)] bg-white text-[var(--erp-foreground)] shadow-[var(--erp-shadow-elevated)]">
          ...
          <div className="relative grid gap-6 p-6 lg:grid-cols-[1.2fr_0.8fr] lg:p-8">
            <div className="max-w-4xl">
              ...
              <h1 className="mt-4 max-w-3xl text-3xl font-black tracking-[-0.07em] sm:text-5xl">
                Mesa ejecutiva para ventas, caja, inventario, cobranza y
                reparto.
              </h1>
              <p className="mt-4 max-w-2xl text-sm leading-7 text-[var(--erp-muted-foreground)]">
                Reportes casi en tiempo real basados en operaciones confirmadas.
                La vista separa caja, crédito, inventario y reparto sin cambiar
                cálculos ni permisos existentes.
              </p>
              <div className="mt-6 flex flex-wrap gap-2">
                <Badge
                  className="border-[color:var(--erp-border)] bg-[var(--erp-surface-muted)] text-[var(--erp-foreground)]"
                  tone="slate"
                >
                  Trazabilidad
                </Badge>
            ...
            <div className="grid gap-3 sm:grid-cols-3 lg:grid-cols-1">
              <div className="rounded-[1.5rem] border border-[color:var(--erp-border)] bg-[var(--erp-surface-muted)]/90 p-5 backdrop-blur">
                <p className="flex items-center gap-2 text-xs font-black uppercase tracking-[0.18em] text-[var(--erp-muted-foreground)]">
                  <RefreshCw className="h-4 w-4" />
                  Frescura operativa
                </p>
                <p className="mt-3 text-4xl font-black tracking-[-0.08em] text-[var(--erp-brand-gold-deep)] sm:text-5xl">
                  60s
                </p>
                <p className="mt-2 text-sm leading-6 text-[var(--erp-muted-foreground)]">
                  Cada reporte conserva su indicador de generación y datos
                  incluidos.
                </p>
              </div>
              <div className="rounded-[1.5rem] border border-[color:var(--erp-border)] bg-[var(--erp-surface-muted)]/90 p-5 backdrop-blur sm:col-span-2 lg:col-span-1">
                ...
                <p className="mt-3 text-xl font-black tracking-[-0.04em] text-[var(--erp-foreground)]">
                  {visibleTabs.length} reporte(s) autorizados
                </p>
              </div>
            </div>
          </div>
        ...
        {visibleTabs.length === 0 ? (
          ...
          <>
            <nav
              ...
              className="flex gap-2 overflow-x-auto rounded-[1.35rem] border border-[color:var(--erp-border)] bg-white/85 p-2 shadow-[var(--erp-shadow)] backdrop-blur"
            >
              {visibleTabs.map((tab) => (
                <Button
            ...
            {selectedReport === "sales" && (
              <SalesDailyReport isAdmin={isAdmin} />
            )}
            {selectedReport === "cash" && (
              <CashClosingReport isAdmin={isAdmin} />
            )}
