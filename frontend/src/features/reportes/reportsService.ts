...
async function getReport<TReport>(
  path: string,
  filters: ReportFilters,
  accessToken?: string | null,
) {
  const response = await apiClient.get<ApiEnvelope<TReport>>(
    withParams(path, filters),
    {
...
export const reportsService = {
  getAccountsReceivable(
    filters: AccountsReceivableReportFilters,
    accessToken?: string | null,
  ) {
    return getReport<AccountsReceivableReport>(
  ...
  getCashClosing(
    filters: CashClosingReportFilters,
    accessToken?: string | null,
  ) {
    return getReport<CashClosingReport>(
  ...
  getDashboard(filters: DashboardReportFilters, accessToken?: string | null) {
    return getReport<DashboardReport>(
      "/reports/dashboard",
      filters,
      accessToken,
    );
  ...
  getDeliveryOperations(
    filters: DeliveryOperationsReportFilters,
    accessToken?: string | null,
  ) {
    return getReport<DeliveryOperationsReport>(
  ...
  getSalesDaily(filters: SalesDailyReportFilters, accessToken?: string | null) {
    return getReport<SalesDailyReport>(
      "/reports/sales-daily",
      filters,
      accessToken,
    );
