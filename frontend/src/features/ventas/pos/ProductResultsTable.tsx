...
export function ProductResultsTable({
  ...
  const sourceProducts = useMemo(
    ...
    [activeView, frequentProducts, products, search],
  );
  ...
  const renderedRowCount =
    Math.ceil((viewportHeight || 520) / PRODUCT_ROW_HEIGHT) +
    VIRTUALIZATION_OVERSCAN * 2;
  const startIndex = virtualized
    ? Math.max(
        0,
        Math.floor(scrollTop / PRODUCT_ROW_HEIGHT) - VIRTUALIZATION_OVERSCAN,
      )
    : 0;
  const endIndex = virtualized
    ? Math.min(visibleProducts.length, startIndex + renderedRowCount)
    : visibleProducts.length;
  const renderedProducts = virtualized
      ...
      <div className="grid shrink-0 gap-2 border-b border-[var(--pos-steel)] p-3">
        ...
        >
          <button
            ...
            className={`inline-flex h-11 items-center text-xs font-bold transition ${activeView === "frequent" ? "text-[var(--pos-green)]" : "text-[var(--pos-muted)] hover:text-[var(--pos-ink)]"}`}
            onClick={() => {
          ...
          <button
            ...
            className={`inline-flex h-11 items-center text-xs font-bold transition ${activeView === "all" ? "text-[var(--pos-green)]" : "text-[var(--pos-muted)] hover:text-[var(--pos-ink)]"}`}
          ...
          {categoryOptions.map((category) => (
            <button
              ...
              className={`inline-flex h-11 items-center border-l pl-3 text-xs font-bold transition ${activeCategory === category ? "border-[var(--pos-green)] text-[var(--pos-green)]" : "border-[var(--pos-steel)] text-[var(--pos-muted)] hover:text-[var(--pos-ink)]"}`}
              key={category}
              onClick={() => {
                setActiveView("all");
        ...
        <table className="w-full min-w-[24rem] border-collapse text-left xl:min-w-[34rem]">
          ...
          <tbody>
            {virtualized && startIndex > 0 && (
              <tr aria-hidden="true">
                <td
                  colSpan={6}
                  style={{
                    height: `${startIndex * PRODUCT_ROW_HEIGHT}px`,
                    padding: 0,
                  }}
                />
              </tr>
            ...
            {renderedProducts.map((product) => {
              ...
              const addDisabled =
                !locationId || product.locationId !== locationId || hasNoStock;
              const stockStatusId = `product-stock-${product.id}`;
              return (
                <tr
                ...
                >
                  ...
                  <td className="hidden font-[var(--pos-mono)] text-[0.65rem] text-[var(--pos-muted)] xl:table-cell">
                  ...
                  <td className="px-3 text-right">
                    <button
                      aria-describedby={hasNoStock ? stockStatusId : undefined}
                      className={`h-11 border px-2 text-xs font-bold disabled:pointer-events-none ${addDisabled ? "cursor-not-allowed border-[var(--pos-steel)] bg-[var(--pos-surface-secondary)] text-[var(--pos-muted)]" : "border-[var(--pos-ink)] text-[var(--pos-ink)] transition hover:bg-[var(--pos-ink)] hover:text-white focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-[-2px] focus-visible:outline-[var(--pos-focus)]"}`}
                      disabled={addDisabled}
                      onClick={() => onAdd(product)}
                      tabIndex={addDisabled ? -1 : undefined}
                      type="button"
                    >
                      Agregar
                    </button>
            ...
            {virtualized && endIndex < visibleProducts.length && (
              <tr aria-hidden="true">
                <td
                  colSpan={6}
                  style={{
                    height: `${(visibleProducts.length - endIndex) * PRODUCT_ROW_HEIGHT}px`,
                    padding: 0,
                  }}
                />
              </tr>
