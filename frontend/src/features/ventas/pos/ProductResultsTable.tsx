      ...
      <div className="min-h-0 flex-1 overflow-auto" onScroll={(event) => { if (virtualized) setScrollTop(event.currentTarget.scrollTop) }} ref={resultsRef}>
        <table className="w-full min-w-[24rem] border-collapse text-left xl:min-w-[34rem]">
          ...
          <tbody>{virtualized && startIndex > 0 && <tr aria-hidden="true"><td colSpan={6} style={{ height: `${startIndex * PRODUCT_ROW_HEIGHT}px`, padding: 0 }} /></tr>}{renderedProducts.map((product) => {
            ...
            const addDisabled = !locationId || product.locationId !== locationId || hasNoStock
            ...
            return <tr className="h-[52px] border-b border-[var(--pos-steel)] transition hover:bg-[var(--pos-porcelain)]" key={product.id}>
              ...
              <td className={`text-right font-[var(--pos-mono)] text-xs font-bold ${hasNoStock ? 'text-[var(--pos-red)]' : product.isLowStock ? 'text-[#7d5a12]' : 'text-[var(--pos-muted)]'}`} id={stockStatusId}>{hasNoStock ? 'Sin stock' : `${product.availableKg} kg · ${product.availablePieces} pz`}</td>
              <td className="px-3 text-right"><button aria-describedby={hasNoStock ? stockStatusId : undefined} className={`h-11 border px-2 text-xs font-bold disabled:pointer-events-none ${addDisabled ? 'cursor-not-allowed border-[var(--pos-steel)] bg-[var(--pos-surface-secondary)] text-[var(--pos-muted)]' : 'border-[var(--pos-ink)] text-[var(--pos-ink)] transition hover:bg-[var(--pos-ink)] hover:text-white focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-[-2px] focus-visible:outline-[var(--pos-focus)]'}`} disabled={addDisabled} onClick={() => onAdd(product)} tabIndex={addDisabled ? -1 : undefined} type="button">Agregar</button></td>
            </tr>
