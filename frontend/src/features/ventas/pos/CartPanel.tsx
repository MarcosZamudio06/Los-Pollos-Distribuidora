      ...
      <table className="w-full min-w-[34rem] border-collapse text-left xl:min-w-[42rem]">
        ...
        <tbody>
          {items.map((item) => {
            ...
            return (
              <tr
                aria-selected={activeItemId === item.productId}
                className={`min-h-16 border-b border-[var(--pos-steel)] transition ${highlightedItemId === item.productId ? "pos-cart-row-added" : ""} ${activeItemId === item.productId ? "bg-[rgba(35,113,90,0.08)] shadow-[inset_3px_0_0_var(--pos-green)]" : "hover:bg-[var(--pos-porcelain)]"}`}
                key={item.productId}
              ...
              >
                ...
                <td className="align-middle">
                    ...
                    {(item.unit === "KG" || item.unit === "KG_AND_PIECE") && (
                      <label className="grid gap-0.5 text-[0.6rem] font-bold text-[var(--pos-muted)]">
                        ...
                        <input
                          ...
                          className="h-11 w-20 border border-[var(--pos-steel)] bg-white px-2 font-[var(--pos-mono)] text-sm text-[var(--pos-ink)] outline-none focus:border-[var(--pos-focus)]"
                          min="0"
                          onChange={(event) =>
                            onQuantityChange(
                              item.productId,
                          ...
                          onKeyDown={(event) => {
                            if (
                              ...
                              !getQuantityValidationError({
                                ...item,
                                quantityKg: Number(event.currentTarget.value),
                              })
                            ) {
                  ...
                  {!validation && item.quantityKg > 0 && (
                    <p
                      className="mt-1 max-w-48 text-[0.62rem] font-bold text-[var(--pos-success)]"
                      role="status"
                    >
                      Peso capturado
                    </p>
                ...
                <td className="px-2 text-right align-middle">
                  <button
                    ...
                    className="inline-grid h-11 w-11 place-items-center text-[var(--pos-red)] transition hover:bg-[rgba(182,42,34,0.08)]"
