...
import { Check, ChevronDown, ChevronRight, LoaderCircle, Minus, ReceiptText, Search, ShoppingCart, UserRound, WalletCards } from 'lucide-react'
...
function PosPrimaryAction({ confirmButtonRef, customerSearchRef, isSubmitting, onConfirm, paymentPanelRef, pendingAmount, visualState }: PosPrimaryActionProps) {
  ...
  return (
    <button aria-atomic="true" aria-busy={isBusy || undefined} aria-describedby={reason ? 'pos-primary-action-reason' : undefined} aria-keyshortcuts={isExecutable ? 'F8 Enter' : undefined} aria-live="polite" className="flex h-full min-h-14 w-full min-w-0 items-center justify-center gap-4 rounded-[14px] bg-[var(--pos-action)] px-4 text-left text-white shadow-[0_10px_22px_rgba(18,61,50,0.18)] transition-[background-color,color,opacity,transform,box-shadow] duration-150 hover:bg-[#1a4e40] hover:shadow-[0_12px_26px_rgba(18,61,50,0.24)] active:scale-95 focus-visible:relative focus-visible:z-10 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-[-2px] focus-visible:outline-[var(--pos-focus)] disabled:cursor-not-allowed disabled:bg-[rgba(96,112,107,0.72)] disabled:opacity-50 disabled:shadow-none min-[1680px]:gap-6 min-[1680px]:px-6" data-pos-primary-action disabled={!isExecutable || isBusy} onClick={activate} ref={confirmButtonRef} type="button">
       <span aria-hidden="true" className="relative grid size-14 shrink-0 place-items-center min-[1680px]:size-16">{isBusy ? <LoaderCircle className="size-9 animate-spin min-[1680px]:size-10" /> : isExecutable ? <><ShoppingCart className="size-14 stroke-[2.25] min-[1680px]:size-16" /><span className="absolute right-0 top-0 grid size-6 place-items-center rounded-full bg-white text-[var(--pos-action)] min-[1680px]:size-7"><Check className="size-4 stroke-[3] min-[1680px]:size-5" /></span></> : <Minus className="size-8" />}</span>
       <span className="min-w-0"><span className="block truncate whitespace-nowrap text-[clamp(1rem,1.6vw,1.6rem)] font-black leading-none tabular-nums">{action.label}</span>{reason && <span className="mt-2 block truncate text-[0.65rem] font-semibold leading-3 text-white/75" id="pos-primary-action-reason">{reason}</span>}{isExecutable && <span className="mt-2 block font-mono text-base font-bold leading-none text-white/80">F8</span>}</span>
    </button>
  )
}
  ...
  return (
    <footer className="z-20 h-60 shrink-0 overflow-visible border-t border-[var(--pos-steel)] bg-[var(--pos-surface)] min-[1024px]:h-60 min-[1280px]:h-36" aria-label="Confirmación de venta">
      <div className="grid h-full grid-cols-2 grid-rows-[repeat(3,minmax(0,1fr))] gap-px bg-[var(--pos-steel)] min-[1024px]:grid-cols-[20fr_13fr_22fr] min-[1024px]:grid-rows-[minmax(0,1fr)_minmax(0,1fr)] min-[1280px]:grid-cols-[20fr_13fr_22fr_20fr_25fr] min-[1280px]:grid-rows-1">
        ...
        <div className="col-span-2 row-start-3 flex min-w-0 justify-end bg-[var(--pos-surface)] p-3 min-[1024px]:col-start-3 min-[1024px]:col-span-1 min-[1024px]:row-start-2 min-[1280px]:col-start-5 min-[1280px]:col-span-1 min-[1280px]:row-start-1"><PosPrimaryAction confirmButtonRef={confirmButtonRef} customerSearchRef={customerSearchRef} isSubmitting={isSubmitting} onConfirm={onConfirm} paymentPanelRef={paymentPanelRef} pendingAmount={pendingAmount} total={exactTotal} visualState={visualState} /></div>
      </div>
    </footer>
  )
}
