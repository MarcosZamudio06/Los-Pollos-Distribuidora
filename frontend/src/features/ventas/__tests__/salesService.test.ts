...
describe("salesService TASK-055 contracts", () => {
  ...
  it("consulta el historial con los filtros del contrato GET /api/sales", async () => {
    ...
    expect(request.url).toBe(
      "/api/sales?collectionStatus=UNPAID&dateFrom=2026-07-01&dateTo=2026-07-03&documentType=SIMPLE_NOTE&limit=50&locationId=loc-counter&page=1&paymentType=CASH_SALE&physicalFolio=A-1024&saleChannel=COUNTER&status=CONFIRMED",
    );
  ...
  it("carga el detalle de venta desde GET /api/sales/:id", async () => {
    vi.mocked(fetch).mockResolvedValueOnce(
      okJson({ id: "sale-1", items: [], status: "CONFIRMED" }),
    );
  ...
  it("carga el documento exacto para reimpresión", async () => {
    vi.mocked(fetch).mockResolvedValueOnce(
      okJson({ saleNumber: "V-1", ticketNumber: "T-1" }),
    );

    const ticket = await salesService.getTicket(
      "sale-1",
      "doc-1",
      "access-token",
    );
  ...
  it("consulta documentos internos desde GET /api/sales/:saleId/documents", async () => {
    ...
    const documents = await salesService.getSaleDocuments(
      "sale-1",
      "access-token",
    );

    expect(documents.items).toEqual([
      { id: "doc-1", documentType: "SIMPLE_NOTE" },
    ]);
  ...
  it("envía cancelación con payload { reason, expectedVersion } e idempotencia", async () => {
    vi.mocked(fetch).mockResolvedValueOnce(
      okJson({ sale: { id: "sale-1", status: "CANCELLED" } }),
    );

    await salesService.cancelSale(
      "sale-1",
      { expectedVersion: 4, reason: "Cliente canceló pedido" },
      "cancel-attempt-key",
      "access-token",
    );
  ...
  it("envía la anulación administrativa con una clave idempotente única", async () => {
    vi.mocked(fetch).mockResolvedValueOnce(
      okJson({
        sale: { id: "sale-1", status: "CANCELLED" },
        payments: [],
        documents: [],
      }),
    );
    ...
    await salesService.voidSale(
      "sale-1",
      { expectedVersion: 4, reason: "Cliente devolvió pedido" },
      "void-attempt-key",
      "access-token",
    );
    ...
    expect(JSON.parse(String(request.init?.body))).toEqual({
      expectedVersion: 4,
      reason: "Cliente devolvió pedido",
    });
    expect(new Headers(request.init?.headers).get("idempotency-key")).toBe(
      "void-attempt-key",
    );
  ...
  it("expone estado no autorizado cuando la API responde 401", async () => {
    ...
    await expect(
      salesService.getSale("sale-1", "access-token"),
