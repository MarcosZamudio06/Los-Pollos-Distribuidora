...
describe("POS cart calculations and validation", () => {
  it("derives fixed POS channels from the operational location type", () => {
    expect(getSaleChannelsForLocation("BRANCH")).toEqual([
      "COUNTER",
      "INSTITUTIONAL",
      "WHOLESALE",
    ]);
    expect(getSaleChannelsForLocation("MIXED")).toEqual([
      "COUNTER",
      "INSTITUTIONAL",
      "WHOLESALE",
    ]);
    expect(getSaleChannelsForLocation("EXTERNAL_POINT_OF_SALE")).toEqual([
  ...
  it("keeps SELLER location options restricted to the assigned fixed POS location", () => {
    const locations = [
      ...
      },
      {
        id: "loc-ext",
        name: "Punto externo",
        type: "EXTERNAL_POINT_OF_SALE" as const,
        isActive: true,
      },
      {
        id: "loc-route",
        name: "Ruta Norte",
        type: "ROUTE_STOCK" as const,
        isActive: true,
      },
    ];

    expect(getPosLocationOptions(locations, "SELLER", "loc-counter")).toEqual([
      locations[0],
    ]);
    expect(getPosLocationOptions(locations, "SELLER", undefined)).toEqual([]);
    expect(getPosLocationOptions(locations, "ADMIN")).toEqual(
      locations.slice(0, 2),
    );
  });
  ...
  it("rejects empty, non-positive, over-stock, and fractional piece quantities per operational stock", () => {
    expect(getQuantityValidationError({ ...kgItem, quantityKg: 0 })).toBe(
      "Ingresa una cantidad mayor que cero.",
    );
    expect(getQuantityValidationError({ ...kgItem, quantityKg: 12.5 })).toBe(
      "La cantidad no puede exceder 12.25 kg disponibles en Counter cooler.",
    );
    expect(
      getQuantityValidationError({ ...pieceItem, quantityPieces: 5.5 }),
    ).toBe("Las piezas deben ser un número entero.");
    expect(
      getQuantityValidationError({ ...pieceItem, quantityPieces: 6 }),
    ).toBe(
      "La cantidad no puede exceder 5 piezas disponibles en Counter cooler.",
    );
  ...
  it("allows kg-and-piece products to sell kilos, pieces, or both, requiring equivalence only for pieces", () => {
    expect(
      getQuantityValidationError({
        ...
        equivalentFactor: undefined,
        equivalentUnitFrom: undefined,
        equivalentUnitTo: undefined,
      }),
    ).toBeNull();
    expect(
      getQuantityValidationError({
    ...
    expect(
      ...
      "El producto requiere una equivalencia activa entre kilos y piezas.",
    );
    expect(
      getQuantityValidationError({
        ...kgAndPieceItem,
    ...
    ).toBe("Ingresa kilos, piezas o ambas cantidades.");
    expect(
      getQuantityValidationError({
        ...kgAndPieceItem,
        quantityKg: 2,
  ...
  it("requires payment evidence only for non-cash methods that need reconciliation", () => {
    ...
    expect(
      getPaymentReferenceValidationError("CASH", emptyReference),
    ).toBeNull();
    expect(getPaymentReferenceValidationError("TRANSFER", emptyReference)).toBe(
      "Captura el banco y la referencia del pago.",
    );
    expect(
      getPaymentReferenceValidationError("CARD", {
        ...emptyReference,
  ...
  it("validates every split payment amount, evidence, and combined amount before submission", () => {
    expect(
      getPaymentsValidationError([{ amount: 0, paymentMethod: "CASH" }], 100),
    ).toBe("Captura un método y un monto mayor que cero para cada pago.");
    expect(
      getPaymentsValidationError([{ amount: 101, paymentMethod: "CASH" }], 100),
    ).toBe("El total recibido no puede exceder el total de la venta.");
    expect(
      getPaymentsValidationError(
        [
          {
  ...
  it("rejects split payments whose individual money rounding changes their combined amount before submission", () => {
    ...
    expect(getPaymentsValidationError(payments, 100)).toBe(
      "Los montos de pago no pueden alterar el total al redondearse a centavos.",
    );
  });
  ...
  it("validates cash tendered only for cash and calculates change from its individual applied amount", () => {
    ...
    expect(
      getPaymentsValidationError(
        [{ amount: 187.5, paymentMethod: "CASH", cashTendered: 180 } as never],
        187.5,
      ),
    ).toBe("El efectivo entregado no puede ser menor al monto aplicado.");
    expect(
      getPaymentsValidationError(
        [
          {
    ...
    expect(
      getPaymentsValidationError(
        [{ amount: "100.00", paymentMethod: "CASH", cashTendered: "100.00" }],
        100,
      ),
    ).toBeNull();
    expect(
    ...
    ).toBe("El efectivo entregado solo aplica a pagos en efectivo.");
  });

  it("rejects cart items whose displayed stock belongs to another operational location", () => {
    expect(
      getLocationValidationError(
        [{ ...kgItem, locationId: "loc-old", locationName: "Old counter" }],
        "loc-new",
      ),
    ).toBe(
      "El carrito contiene stock de otra ubicación operativa. Actualiza el carrito para esta ubicación.",
    );
...
describe("POS credit restrictions", () => {
  it("requires a registered customer for credit sales", () => {
    expect(getCreditRestriction("CREDIT_SALE", null, 100)).toBe(
      "Selecciona un cliente activo para una venta a crédito.",
    );
  });
  ...
  it("blocks credit when the customer is blocked or the sale exceeds available credit", () => {
    expect(
      getCreditRestriction(
        "CREDIT_SALE",
        { ...activeCustomer, isBlockedForCredit: true },
        100,
      ),
    ).toBe("El crédito del cliente está bloqueado.");
    expect(
      getCreditRestriction(
        "CREDIT_SALE",
        {
    ...
    ).toBe("La venta excede el crédito disponible de $0.00.");
    expect(getCreditRestriction("CREDIT_SALE", activeCustomer, 2700)).toBe(
      "La venta excede el crédito disponible de $2,600.00.",
    );
    expect(
      getCreditRestriction("CREDIT_SALE", activeCustomer, 2500),
    ).toBeNull();
  });
  ...
  it("permits warnings and only bypasses an eligible block with an intentional admin override", () => {
    ...
    expect(
      getCreditRestriction("CREDIT_SALE", warningCustomer, 100),
    ).toBeNull();
    expect(
      getCreditRestriction("CREDIT_SALE", blockedCustomer, 100, {
        isAdmin: true,
        overrideEnabled: true,
        overrideReason: "",
      }),
    ).toBe("Captura el motivo de la autorización administrativa.");
    expect(
      getCreditRestriction("CREDIT_SALE", blockedCustomer, 100, {
        isAdmin: true,
        overrideEnabled: true,
        overrideReason: "Autorizado por dirección",
      }),
    ).toBeNull();
    expect(
      getCreditRestriction("CREDIT_SALE", blockedCustomer, 100, {
...
describe("POS sale-level restrictions", () => {
  it("requires cash sales to be fully paid and directs partial payments to credit", () => {
    ...
    expect(getSaleRestriction("CASH_SALE", activeCustomer, 100, 50)).toBe(
      message,
    );
    expect(getSaleRestriction("CASH_SALE", null, 100, 100)).toBeNull();
    expect(
      getSaleRestriction("CASH_SALE", activeCustomer, 100, 100),
    ).toBeNull();
  });
});
...
describe("POS sale payload", () => {
  ...
  it("sends cashTendered without a client-provided changeGiven", () => {
    const payload = buildCreateSalePayload({
      ...
      locationId: "loc-counter",
      payments: [
        { amount: 187.5, paymentMethod: "CASH", cashTendered: 200 } as never,
      ],
      paymentType: "CASH_SALE",
      physicalFolio: "",
      requiresAdministrativeInvoice: false,
    ...
    expect(payload.payments).toEqual([
      { amount: "187.50", paymentMethod: "CASH", cashTendered: "200.00" },
    ]);
  ...
  it("only sends a trimmed administrative override reason when it was intentionally captured", () => {
    ...
    expect(payload.administrativeOverrideReason).toBe(
...
describe("POS backend credit errors", () => {
  it("maps stable credit codes to actionable Spanish messages", () => {
    expect(
      getSaleErrorMessage({ payload: { code: "CREDIT_OVERDUE_BLOCKED" } }),
    ).toBe(
      "El cliente tiene saldo vencido y su política bloquea nuevas ventas a crédito.",
    );
    expect(
      getSaleErrorMessage({ payload: { code: "CREDIT_POLICY_MISMATCH" } }),
    ).toBe(
