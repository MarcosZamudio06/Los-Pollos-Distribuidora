type PosOperation = 'cart-update' | 'checkout' | 'checkout-registration' | 'print' | 'scan-feedback' | 'search'
...
export function finishPosMeasurement(operation: PosOperation, targetMs?: number) {
  if (typeof performance === 'undefined') return
  const name = metricName(operation)
  ...
  const duration = performance.getEntriesByName(name, 'measure').at(-1)?.duration
  if (targetMs && duration && duration > targetMs && import.meta.env.DEV) {
    console.warn(`POS ${operation} exceeded ${targetMs} ms: ${duration.toFixed(1)} ms`)
  }
}
