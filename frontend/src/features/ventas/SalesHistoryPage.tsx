      ...
      <section className="mx-auto grid max-w-7xl gap-5">
          ...
          <div className="relative grid gap-5 lg:grid-cols-[1fr_auto] lg:items-end">
            <div className="min-w-0">
              ...
              <h1 className="mt-4 max-w-4xl text-3xl font-black tracking-[-0.06em] text-[var(--erp-foreground)] sm:text-4xl">
                Bitácora comercial operativa
              </h1>
              <p className="mt-3 max-w-3xl text-sm leading-6 text-[var(--erp-muted-foreground)]">
                Consulta ventas de mostrador, ruta y mayoreo con filtros
                operativos, estados de cobranza y documentos internos sin
                mezclar comprobantes fiscales.
              </p>
            </div>
            <div className="flex flex-col gap-3 sm:flex-row lg:flex-col lg:items-end">
              <div className="rounded-2xl border border-[color:var(--erp-border)] bg-[var(--erp-surface-muted)]/90 px-4 py-3 text-sm">
                <span className="block text-xs font-black uppercase tracking-[0.16em] text-[var(--erp-muted-foreground)]">
                  Resultados
                </span>
                <span className="mt-1 block text-2xl font-black tracking-[-0.05em] text-[var(--erp-foreground)]">
                  {items.length}
                </span>
              </div>
              <Link
        ...
        <Card className="p-5">
          <CardHeader className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <CardDescription className="mt-2">
                Ajusta el rango, canal, documento y estado conservando la
                consulta actual de ventas.
              </CardDescription>
            </div>
            <Button
              disabled={!hasFilters}
          ...
          <CardContent className="mt-5 grid gap-3 md:grid-cols-4">
            ...
            <label className={filterLabelClass}>
              ...
              <Input
                onChange={(event) =>
                  setFilters({ ...filters, physicalFolio: event.target.value })
                }
                placeholder="Ej. A-1024"
                value={filters.physicalFolio}
              />
            </label>
            <label className={filterLabelClass}>
              Estado
            ...
            <label className={filterLabelClass}>
              ...
              <Select
                onChange={(event) =>
                  setFilters({
                    ...filters,
                    collectionStatus: event.target.value as
                      CollectionStatus | "",
                  })
                }
            ...
            <label className={filterLabelClass}>
              ...
              >
                <option value="">Todas</option>
                <option value="CASH_SALE">Contado</option>
                <option value="CREDIT_SALE">Crédito</option>
              </Select>
            </label>
            <label className={filterLabelClass}>
              Canal
              <Select
                onChange={(event) =>
                  setFilters({
                    ...filters,
                    saleChannel: event.target.value as SaleChannel | "",
                  })
                }
              ...
              >
                ...
                <option value="EXTERNAL_POINT_OF_SALE">Punto externo</option>
                <option value="ROUTE">Ruta</option>
                <option value="INSTITUTIONAL">Institucional</option>
                <option value="WHOLESALE">Mayoreo</option>
              </Select>
            </label>
            <label className={cn(filterLabelClass, "md:col-span-2")}>
              Documento
              <Select
                onChange={(event) =>
                  setFilters({
        ...
        <Card className="overflow-hidden p-0">
          <div className="flex flex-col gap-3 border-b border-[color:var(--erp-border)] bg-white/70 p-5 sm:flex-row sm:items-center sm:justify-between">
            ...
            <Badge tone={hasFilters ? "blue" : "slate"}>
          ...
          <div className="p-5">
            ...
            {!sales.isLoading && !sales.error && items.length === 0 && (
              <p className="rounded-2xl border border-dashed border-[color:var(--erp-border)] bg-[var(--erp-surface)] p-6 text-sm text-[var(--erp-muted-foreground)]">
                      ...
                      >
                        <Td>
                          <div className="flex items-start gap-3">
                            <span className="mt-0.5 grid size-9 shrink-0 place-items-center rounded-xl bg-[var(--erp-surface-muted)] text-[var(--erp-info)]">
                              <ReceiptText className="h-4 w-4" />
                            </span>
                            <div>
                              ...
                              <p className="mt-1 text-xs text-[var(--erp-muted-foreground)]">
                                {saleChannelLabel(sale.saleChannel)} ·{" "}
                        ...
                        <Td>
                          ...
                          <p className="mt-1 flex items-center gap-1 text-xs text-[var(--erp-muted-foreground)]">
                            ...
                            {sale.locationId ?? "Sin ubicación"}
                          </p>
                        </Td>
                        <Td>
                          <p className="font-semibold">
                            {documentTypeLabel(sale.documentType)}
                          </p>
                          {sale.physicalFolio ? (
                            <p className="mt-1 flex items-center gap-1 text-xs text-[var(--erp-muted-foreground)]">
                              <FileText className="h-3.5 w-3.5" />
                              Folio {sale.physicalFolio}
                            </p>
                          ) : null}
                        </Td>
                        <Td className="text-right text-base font-black tabular-nums">
                          {money(sale.total)}
                        </Td>
                        <Td>
                        ...
                        <Td>
                          ...
                          sale.billingRequestStatus ? (
                            <Link
                              className="inline-flex"
                              to={`/billing-requests/${sale.billingRequestId}`}
                            >
                              <BillingRequestStatusBadge
                                status={
                                  sale.billingRequestStatus as BillingRequestStatus
                                }
                              />
                            </Link>
                          ) : (
                        ...
                        <Td>
                          <Badge tone={sale.routeId ? "blue" : "amber"}>
                            {sale.routeId
                              ? "Ruta asignada"
                              : "Sin ruta asignada"}
                          </Badge>
                        ...
                        <Td>
                          <Badge
                            tone={collectionStatusTone(sale.collectionStatus)}
                          >
                            {collectionStatusLabel(sale.collectionStatus)}
                          </Badge>
                        </Td>
                        <Td>
                          <p className="flex items-center gap-2 text-sm text-[var(--erp-muted-foreground)]">
                        ...
                        <Td className="text-right">
                          <Link
                            className="inline-flex h-9 items-center justify-center gap-2 rounded-xl border border-[color:var(--erp-border)] bg-white px-3 text-sm font-black text-[var(--erp-danger)] transition hover:border-[var(--erp-danger)] hover:bg-[rgba(157,45,36,0.06)]"
                            to={`/sales/${sale.id}`}
                          >
