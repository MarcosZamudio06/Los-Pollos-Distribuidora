...
const mockState = vi.hoisted(() => ({
  ...
  categories: {
    data: [{ id: "cat-1", name: "Pollo fresco" }],
...
vi.mock("../components/InventoryByLocationView", () => ({
  InventoryByLocationView: () => null,
}));
vi.mock("../components/InventoryMovementsView", () => ({
  InventoryMovementsView: () => null,
}));
...
describe("Filtros del inventario", () => {
  ...
  it("envía los IDs seleccionados y limpia stock bajo al quitar la ubicación", async () => {
    ...
    const lowStock = container.querySelector<HTMLInputElement>(
    ...
    expect(mockState.productFilters).toMatchObject({
      categoryId: "cat-1",
      locationId: "loc-1",
      lowStock: true,
    });
  ...
  it("muestra errores de catálogos y permite reintentar ambas cargas", async () => {
    mockState.categories = {
      data: [],
      error: new Error("categorías caídas"),
      isLoading: false,
      refetch: vi.fn(),
    };
    mockState.locations = {
      data: [],
      error: new Error("ubicaciones caídas"),
      isLoading: false,
      refetch: vi.fn(),
    };
    const container = await renderPage();
    ...
    ].filter((button) => button.textContent === "Reintentar");
