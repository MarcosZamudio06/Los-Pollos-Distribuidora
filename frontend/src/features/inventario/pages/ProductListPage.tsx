...
function productEquivalence(product: Product) {
  ...
  if (activeEquivalence)
    return `${activeEquivalence.factor} ${activeEquivalence.unitTo} por ${activeEquivalence.unitFrom}`;
  return (
...
export function ProductListPage() {
  ...
  const [activeSection, setActiveSection] =
    useState<InventorySectionKey>(defaultSection);
  const canViewPurchaseCost =
  ...
  return (
    <main className="min-h-screen bg-[var(--erp-shell-bg)] px-4 py-6 text-[var(--erp-foreground)] sm:px-6 lg:px-8">
      <section className="mx-auto grid max-w-7xl gap-6">
        <header className="overflow-hidden rounded-2xl border border-[var(--erp-border)] bg-[var(--erp-surface-elevated)] shadow-[0_24px_80px_rgba(16,24,32,0.08)]">
          <div className="grid gap-6 border-b border-[var(--erp-border)] p-5 md:grid-cols-[1.2fr_0.8fr] md:items-end lg:p-6">
            <div>
              ...
              <p className="mt-3 max-w-2xl text-sm leading-6 text-[var(--erp-muted-foreground)]">
                La disponibilidad operativa se muestra solo por ubicación. Los
                productos nunca capturan ni editan stock global.
              </p>
            </div>
            {canManage ? (
              <button
              ...
              <div className="rounded-xl border border-[var(--erp-border)] bg-[var(--erp-surface-muted)] p-4 text-sm font-semibold text-[var(--erp-muted-foreground)]">
                Acceso de solo lectura para SELLER. Los cambios de productos,
                ajustes y traspasos requieren ADMIN o WAREHOUSE.
              </div>
            )}
          </div>
          <div className="grid gap-3 bg-[var(--erp-surface-muted)]/70 p-4 text-sm text-[var(--erp-muted-foreground)] sm:grid-cols-3">
            <div className="flex items-center gap-3 rounded-xl bg-[var(--erp-surface-elevated)] px-4 py-3">
              <MapPin
                className="h-4 w-4 text-[var(--erp-brand-gold-deep)]"
                aria-hidden="true"
              />
            ...
            <div className="flex items-center gap-3 rounded-xl bg-[var(--erp-surface-elevated)] px-4 py-3">
              <Boxes
                className="h-4 w-4 text-[var(--erp-info)]"
                aria-hidden="true"
              />
            ...
            <div className="flex items-center gap-3 rounded-xl bg-[var(--erp-surface-elevated)] px-4 py-3">
              <ShieldCheck
                className="h-4 w-4 text-[var(--erp-success)]"
                aria-hidden="true"
              />
        ...
        <div
          aria-labelledby={getInventorySectionTabId("products")}
          className="grid gap-6"
          hidden={selectedSection !== "products"}
          id={getInventorySectionPanelId("products")}
          role="tabpanel"
          tabIndex={0}
        >
          ...
          <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-xs text-[var(--erp-muted-foreground)]">
                Conservan la búsqueda y los filtros existentes.
              </p>
            </div>
          </div>
          <div className="grid gap-3 md:grid-cols-7">
            ...
            <select
              className={fieldClass}
              onChange={(event) =>
                setFilters({ ...filters, presentationType: event.target.value })
              }
            >
            ...
            >
              ...
              <option value="KG_AND_PIECE">KG_AND_PIECE</option>
            </select>
            <select
              ...
              onChange={(event) =>
                setFilters({
                  ...
                  lowStock: event.target.value ? filters.lowStock : undefined,
                })
              }
            >
              <option value="">
            ...
            <label className="flex h-11 items-center gap-2 rounded-xl border border-[var(--erp-border)] bg-[var(--erp-surface-muted)] px-3 text-sm font-semibold text-[var(--erp-muted-foreground)]">
              <input
                ...
                className="h-4 w-4 accent-[var(--erp-brand-red)]"
          ...
          {(categories.error || locations.error) && (
            <div className="mt-3 grid gap-2 md:grid-cols-2" role="status">
              {categories.error && (
                <div className="flex items-center justify-between gap-3 rounded-xl border border-[rgba(157,45,36,0.22)] bg-[rgba(157,45,36,0.08)] px-3 py-2 text-xs font-medium text-[var(--erp-danger)]">
              ...
              {locations.error && (
                <div className="flex items-center justify-between gap-3 rounded-xl border border-[rgba(157,45,36,0.22)] bg-[rgba(157,45,36,0.08)] px-3 py-2 text-xs font-medium text-[var(--erp-danger)]">
                  <span>No se pudieron cargar las ubicaciones.</span>
                  <button
                    className="font-semibold underline underline-offset-2"
          ...
          <AsyncState
            ...
            emptyMessage="No hay productos para estos filtros. Prueba con una ubicación, categoría o SKU."
          ...
          >
            <div className="overflow-hidden rounded-2xl border border-[var(--erp-border)] bg-[var(--erp-surface-elevated)] shadow-[0_18px_50px_rgba(16,24,32,0.06)]">
              <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[var(--erp-border)] px-4 py-3">
                <div>
                  <h2 className="text-sm font-semibold text-[var(--erp-foreground)]">
                    Catálogo operativo
                  </h2>
                  <p className="text-xs text-[var(--erp-muted-foreground)]">
                    Tabla compacta para consulta, edición y ajuste autorizado.
                  </p>
                </div>
                ...
                <table className="min-w-full text-left text-sm">
                  <thead className={tableHeadClass}>
                    <tr>
                      ...
                      <th className={tableCellClass}>Categoría</th>
                      <th className={tableCellClass}>Presentación</th>
                      <th className={tableCellClass}>Precio de venta</th>
                      {canViewPurchaseCost && (
                        <th className={tableCellClass}>Costo</th>
                      )}
                      ...
                      <th className={tableCellClass}>Acciones</th>
                    </tr>
                  </thead>
                  <tbody>
                    {pagination.pageItems.map((product) => {
                      ...
                      return (
                        <tr
                          key={product.id}
                          className="border-t border-[var(--erp-border)] align-top transition hover:bg-[var(--erp-surface-muted)]/70"
                        >
                          <td
                            className={`${tableCellClass} min-w-52 font-semibold text-[var(--erp-foreground)]`}
                          >
                            {product.name}
                          </td>
                          <td
                            className={`${tableCellClass} font-mono text-xs text-[var(--erp-muted-foreground)]`}
                          >
                            {product.sku ?? "—"}
                          </td>
                          ...
                          <td
                            className={`${tableCellClass} text-right text-[var(--erp-muted-foreground)]`}
                          >
                            {balance?.minQuantityKg ??
                              balance?.minimumKg ??
                              balance?.minQuantityPieces ??
                              balance?.minimumPieces ??
                              "—"}
                          </td>
                          <td className={tableCellClass}>
                            <LowStockBadge
                          ...
                          <td className={tableCellClass}>
                            <span
                              className={`rounded-full border px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.12em] ${isProductActive(product) ? "border-[rgba(63,123,65,0.22)] bg-[rgba(63,123,65,0.10)] text-[var(--erp-success)]" : "border-[var(--erp-border)] bg-[var(--erp-surface-muted)] text-[var(--erp-muted-foreground)]"}`}
                            >
        ...
        {canManage && (
          <>
            <div
              ...
              className="min-w-0"
              hidden={selectedSection !== "cedisSummary"}
              id={getInventorySectionPanelId("cedisSummary")}
              role="tabpanel"
              tabIndex={0}
            >
            ...
            <div
              aria-labelledby={getInventorySectionTabId("balances")}
              className="min-w-0"
              hidden={selectedSection !== "balances"}
              id={getInventorySectionPanelId("balances")}
              role="tabpanel"
              tabIndex={0}
            >
            ...
            <div
              aria-labelledby={getInventorySectionTabId("transfers")}
              className="min-w-0"
              hidden={selectedSection !== "transfers"}
              id={getInventorySectionPanelId("transfers")}
              role="tabpanel"
              tabIndex={0}
            >
            ...
            <div
              aria-labelledby={getInventorySectionTabId("movements")}
              className="min-w-0"
              hidden={selectedSection !== "movements"}
              id={getInventorySectionPanelId("movements")}
              role="tabpanel"
              tabIndex={0}
            >
      ...
      {canManage && editingProduct !== undefined && (
        <ProductFormModal
          product={editingProduct}
          onClose={() => setEditingProduct(undefined)}
        />
      )}
      {canManage && adjustingProduct && (
