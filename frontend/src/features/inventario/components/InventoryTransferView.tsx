    ...
    <section className="grid gap-4 rounded-2xl border border-[var(--erp-border)] bg-[var(--erp-surface-elevated)] p-5 shadow-[0_18px_50px_rgba(16,24,32,0.06)]">
      ...
      {canManage && (
        <form onSubmit={submitTransfer} className="grid gap-4 rounded-2xl border border-[var(--erp-border)] bg-[var(--erp-surface-muted)] p-4">
          ...
          {values.items.map((item, index) => <div key={index} className="grid gap-3 rounded-2xl border border-[var(--erp-border)] bg-[var(--erp-surface-elevated)] p-3 md:grid-cols-4"><input className={fieldClass} placeholder="ID del producto" value={item.productId} onChange={(event) => setValues({ ...values, items: values.items.map((line, lineIndex) => lineIndex === index ? { ...line, productId: event.target.value } : line) })} /><select className={fieldClass} value={item.unit} onChange={(event) => setValues({ ...values, items: values.items.map((line, lineIndex) => lineIndex === index ? { ...line, unit: event.target.value as typeof item.unit } : line) })}><option value="KG">Kilo</option><option value="PIECE">Pieza</option><option value="KG_AND_PIECE">Kilo y pieza</option></select><input className={fieldClass} min="0" step="0.001" type="number" placeholder="Kg" value={item.quantityKg ?? 0} onChange={(event) => setValues({ ...values, items: values.items.map((line, lineIndex) => lineIndex === index ? { ...line, quantityKg: Number(event.target.value) } : line) })} /><input className={fieldClass} min="0" step="1" type="number" placeholder="Piezas" value={item.quantityPieces ?? 0} onChange={(event) => setValues({ ...values, items: values.items.map((line, lineIndex) => lineIndex === index ? { ...line, quantityPieces: Number(event.target.value) } : line) })} /></div>)}
