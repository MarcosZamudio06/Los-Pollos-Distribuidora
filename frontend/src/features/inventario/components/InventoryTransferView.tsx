...
const emptyTransfer: InventoryTransferValues = {
  originLocationId: "",
  destinationLocationId: "",
  notes: "",
  items: [{ productId: "", unit: "KG" }],
};
const fieldClass =
...
export function InventoryTransferView({
  ...
  function validate() {
    ...
    if (
      ...
      values.items.some((item) => !item.productId)
    )
      return "Se requiere al menos un producto.";
    if (
      values.items.some(
        (item) =>
          (item.quantityKg ?? 0) <= 0 && (item.quantityPieces ?? 0) <= 0,
      )
    )
    ...
    if (
      values.items.some(
        (item) =>
          item.quantityPieces !== undefined &&
          !Number.isInteger(item.quantityPieces),
      )
    )
      return "Las piezas deben ser números enteros.";
    return null;
  }
  ...
  return (
    <section className="grid gap-4 rounded-2xl border border-[var(--erp-border)] bg-[var(--erp-surface-elevated)] p-5 shadow-[0_18px_50px_rgba(16,24,32,0.06)]">
      <div>
        <h2 className="text-xl font-bold tracking-[-0.03em] text-[var(--erp-foreground)]">
          Traspasos de inventario
        </h2>
        <p className="mt-1 text-sm text-[var(--erp-muted-foreground)]">
          Registro visual de movimientos entre ubicaciones operativas. Las
          sucursales se abastecen mediante ciclos CEDIS.
        </p>
      </div>
      {canManage && (
        <form
          onSubmit={submitTransfer}
          className="grid gap-4 rounded-2xl border border-[var(--erp-border)] bg-[var(--erp-surface-muted)] p-4"
        >
          {error && (
            <p
              role="alert"
          ...
          {values.items.map((item, index) => (
            ...
            >
                  ...
                  setValues({
                    ...
                    items: values.items.map((line, lineIndex) =>
                      lineIndex === index
                        ? {
                            ...line,
                            unit: event.target.value as typeof item.unit,
                          }
                        : line,
                    ),
                  })
              ...
              >
                ...
                <option value="KG_AND_PIECE">Kilo y pieza</option>
              </select>
              <input
                className={fieldClass}
                min="0"
                ...
                onChange={(event) =>
                  setValues({
                    ...
                    items: values.items.map((line, lineIndex) =>
                      lineIndex === index
                        ? { ...line, quantityKg: Number(event.target.value) }
                        : line,
                    ),
                  })
                }
                  ...
                  setValues({
                    ...
                    items: values.items.map((line, lineIndex) =>
                      lineIndex === index
                        ? {
                            ...line,
                            quantityPieces: Number(event.target.value),
                          }
                        : line,
                    ),
                  })
          ...
          <button
            ...
            className="inline-flex h-11 items-center justify-center rounded-xl border border-[var(--erp-brand-red)] bg-[var(--erp-brand-red)] px-5 text-sm font-semibold text-[var(--erp-on-brand)] shadow-[0_14px_32px_rgba(157,45,36,0.18)] transition hover:bg-[var(--erp-brand-red-strong)] disabled:opacity-60"
          >
            Crear traspaso
          </button>
        </form>
      ...
      <AsyncState
        empty={!transfers.data?.length}
        emptyMessage="No se han registrado traspasos."
      ...
      >
        <div className="grid gap-3 md:grid-cols-[1fr_1fr]">
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full text-left text-sm">
                <thead className="border-b border-[var(--erp-border)] bg-[var(--erp-surface-muted)] text-[11px] font-semibold uppercase tracking-[0.16em] text-[var(--erp-muted-foreground)]">
                  <tr>
                    <th className={cellClass}>Origen</th>
                    <th className={cellClass}>Destino</th>
                    <th className={cellClass}>Estado</th>
                    <th className={cellClass}>Responsable</th>
                    <th className={cellClass}>Acciones</th>
                  </tr>
                </thead>
                <tbody>
                  {transfers.data?.map((transfer) => (
                    <tr
                      key={transfer.id}
                      className="border-t border-[var(--erp-border)] transition hover:bg-[var(--erp-surface-muted)]/70"
                    >
                      <td className={cellClass}>
                        {transfer.originLocationName ??
                      ...
                      <td className={cellClass}>
                        <span className="rounded-full border border-[var(--erp-border)] bg-[var(--erp-surface-muted)] px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.12em] text-[var(--erp-muted-foreground)]">
                          {transfer.status}
                        </span>
                      </td>
                      <td className={cellClass}>
                        {transfer.responsibleName ?? transfer.userId ?? "—"}
                      </td>
                      <td className={cellClass}>
                        <button
                          className="rounded-lg border border-[var(--erp-border)] px-3 py-1.5 text-xs font-semibold text-[var(--erp-danger)] transition hover:bg-[rgba(157,45,36,0.08)]"
          ...
          <aside className="rounded-2xl border border-[var(--erp-border)] bg-[var(--erp-surface-elevated)] p-5">
            <h3 className="text-lg font-bold text-[var(--erp-foreground)]">
            ...
            {detail.error && (
              <p
                role="alert"
                className="mt-3 text-sm font-semibold text-[var(--erp-danger)]"
              >
            ...
            {detail.data ? (
              <div className="mt-4 grid gap-3 text-sm text-[var(--erp-foreground)]">
                <p>
                  <strong>Estado:</strong> {detail.data.status}
                </p>
                <p>
                ...
                <p>
                  ...
                  {detail.data.confirmedAt
                    ? new Date(detail.data.confirmedAt).toLocaleString()
                    : "Pendiente"}
                </p>
                <div className="grid gap-2">
                  {detail.data.items?.map((item, index) => (
                    <p
                      className="rounded-xl bg-[var(--erp-surface-muted)] px-3 py-2"
                      key={index}
                    >
                      {item.productName ?? item.productId}:{" "}
                      {item.quantityKg ?? 0} kg / {item.quantityPieces ?? 0}{" "}
                      piezas
                    </p>
                  ))}
                </div>
                {detail.data.movements?.length ? (
                  <div className="rounded-2xl border border-[var(--erp-border)] bg-[var(--erp-surface-muted)] p-3">
                    <p className="font-semibold">Movimientos del traspaso</p>
                    {detail.data.movements.map((movement) => (
                      <p key={movement.id}>
                        {movement.type}: {movement.quantityKg ?? 0} kg /{" "}
                        {movement.quantityPieces ?? 0} piezas
                      </p>
                    ))}
                  </div>
                ) : null}
                {canManage && (
                  <div className="grid gap-3">
                    <textarea
                      ...
                      disabled={!canCancelTransfer(detail.data)}
                    ...
                    <div className="flex flex-wrap gap-3">
                      <button
                        className="rounded-xl border border-[var(--erp-success)] bg-[var(--erp-success)] px-4 py-2 font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50"
                        disabled={
                      ...
                      <button
                        className="rounded-xl border border-[var(--erp-danger)] bg-[var(--erp-danger)] px-4 py-2 font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50"
                        disabled={
                          !canCancelTransfer(detail.data) ||
                          cancelTransfer.isPending
                        }
                        onClick={() => void handleCancel(detail.data.id)}
                        type="button"
                      >
                        Cancelar
                      </button>
              ...
              <p className="mt-4 text-sm text-[var(--erp-muted-foreground)]">
                Selecciona un traspaso para consultar su detalle en la API.
              </p>
            )}
          </aside>
        </div>
      ...
      <ConfirmationDialog
        ...
        description="Verifique el movimiento antes de afectar el flujo de inventario."
      ...
      >
        <p>
          <strong>Origen:</strong> {pendingTransfer?.originLocationId}
        </p>
        <p>
          <strong>Destino:</strong> {pendingTransfer?.destinationLocationId}
        </p>
        <p>
          <strong>Productos:</strong> {pendingTransfer?.items.length ?? 0}
        </p>
        {error && (
          <p className="font-semibold text-[var(--erp-danger)]" role="alert">
            {error}
          </p>
        )}
      </ConfirmationDialog>
