...
const validFieldClass =
  "border-[rgba(33,150,83,0.42)] focus:border-[rgba(33,150,83,0.42)] focus:ring-[rgba(33,150,83,0.10)]";
const invalidFieldClass =
  "border-[rgba(157,45,36,0.50)] focus:border-[rgba(157,45,36,0.60)] focus:ring-[rgba(157,45,36,0.12)]";
const textareaClass = cn(fieldBaseClass, "h-auto min-h-24 py-3");

const presentationTypeOptions: Array<{
  value: ProductFormValues["presentationType"];
  label: string;
}> = [
  { value: "KG", label: "Kilo" },
...
const equivalentPolicyOptions: Array<{
  value: NonNullable<ProductFormValues["equivalentPolicyStatus"]>;
  label: string;
}> = [
...
function inputClass(hasError: boolean, isValid: boolean) {
  return cn(
    fieldBaseClass,
    hasError ? invalidFieldClass : isValid ? validFieldClass : null,
  );
}
...
function mergeDescribedBy(
  ...
  return (
    [hasError ? getErrorId(field) : null, hasHelp ? getHelpId(field) : null]
      .filter(Boolean)
      .join(" ") || undefined
  );
}
...
export function ProductFormModal({ product, onClose }: Props) {
  ...
  const [focusedNumeric, setFocusedNumeric] = useState<
    Partial<Record<NumericField, boolean>>
  >({});
  const [submitError, setSubmitError] = useState<string | null>(null);
  const saveProduct = useSaveProduct(product?.id);
  const activeError =
    submitError ??
    (hasProductFormErrors(errors)
  ...
  function setDraftField(
    ...
    const unitChangedByPresentation =
    ...
    if (
      (field === "unit" || unitChangedByPresentation) &&
      (touched.minStock || isSubmitted)
    ) {
      const nextMinStockError = validateProductField("minStock", nextDraft);
      setErrors((current) => ({
        ...current,
        minStock: nextMinStockError ?? undefined,
      }));
    }
    setSubmitError(null);
  }
  ...
  function setNumericField(field: NumericField, rawValue: string) {
    const nextDraft = {
      ...draft,
      [field]: normalizeNumericValue(field, rawValue, draft.unit),
    } as ProductFormDraft;
    setDraft(nextDraft);
    if (touched[field] || isSubmitted) {
      const nextError = validateProductField(field, nextDraft);
  ...
  function focusNumericField(field: NumericField) {
    ...
    setDraft((current) => ({
      ...current,
      [field]: normalizeNumericValue(field, current[field], current.unit),
    }));
  }
  ...
  function blurNumericField(field: NumericField) {
    ...
    const nextDraft = {
      ...draft,
      [field]:
        formatNumericDisplay(field, draft[field], draft.unit) || draft[field],
    } as ProductFormDraft;
    setDraft(nextDraft);
    markTouched(field, nextDraft);
  }
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const nextDraft: ProductFormDraft = {
      ...
      minStock: normalizeDecimalInput(
        draft.minStock,
        draft.unit === "PIECE" ? 0 : 2,
      ),
      pieceWeightEquivalent: normalizeDecimalInput(
  ...
  function numericDisplay(field: NumericField) {
    return focusedNumeric[field]
      ? draft[field]
      : formatNumericDisplay(field, draft[field], draft.unit);
  }
    ...
    <div className="fixed inset-0 z-50 overflow-y-auto bg-[rgba(16,24,32,0.56)] px-4 py-8 backdrop-blur-sm">
      ...
      >
        <header className="flex justify-between gap-4 border-b border-[color:var(--erp-border)] pb-5">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.24em] text-[var(--erp-danger)]">
              Catálogo de productos
            </p>
            <h2 className="text-3xl font-black tracking-[-0.05em] text-[var(--erp-foreground)]">
              {product ? "Editar producto" : "Nuevo producto"}
            </h2>
            <p className="mt-2 text-sm text-[var(--erp-muted-foreground)]">
              Solo campos de catálogo. El stock se controla mediante movimientos
              por ubicación.
            </p>
          </div>
          <button
        ...
        {activeError && (
          <p
            className="rounded-2xl bg-[rgba(157,45,36,0.10)] p-3 text-sm font-semibold text-[var(--erp-danger)]"
            role="alert"
          >
            {activeError}
          </p>
        ...
        <fieldset className="grid gap-4">
          <div className="grid gap-4 md:grid-cols-3">
            ...
            >
              ...
              <Input
                ...
                className={inputClass(
                  Boolean(errors.sku),
                  Boolean(touched.sku && draft.sku && !errors.sku),
                )}
                id={getFieldId("sku")}
                onBlur={blurSkuField}
                onChange={(event) =>
                  setDraftField("sku", event.target.value, cleanSku)
                }
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            >
              ...
              <Input
                ...
                className={inputClass(
                  Boolean(errors.salePrice),
                  Boolean(
                    touched.salePrice && draft.salePrice && !errors.salePrice,
                  ),
                )}
                ...
                onChange={(event) =>
                  setNumericField("salePrice", event.target.value)
                }
              ...
              <span
                ...
                id={getHelpId("salePrice")}
              >
                Se guarda como monto numérico y se muestra con dos decimales al
                salir del campo.
              </span>
              {errors.salePrice && (
                <span
                  className="text-xs font-medium text-[var(--erp-danger)]"
                  id={getErrorId("salePrice")}
                >
                  {errors.salePrice}
                </span>
              )}
            ...
            <label
              className="grid gap-2 text-sm font-semibold"
              htmlFor={getFieldId("purchaseCost")}
            >
              Costo de compra
              <Input
                aria-describedby={mergeDescribedBy(
                  "purchaseCost",
                  Boolean(errors.purchaseCost),
                  true,
                )}
                ...
                onChange={(event) =>
                  setNumericField("purchaseCost", event.target.value)
                }
              ...
              <span
                className="text-xs text-[var(--erp-muted-foreground)]"
                id={getHelpId("purchaseCost")}
              >
                Permite cero cuando no hay costo registrado.
              </span>
              {errors.purchaseCost && (
                <span
                  className="text-xs font-medium text-[var(--erp-danger)]"
                  id={getErrorId("purchaseCost")}
                >
                  {errors.purchaseCost}
                </span>
              )}
          ...
          <div className="grid gap-4 md:grid-cols-3">
            <label
              className="grid gap-2 text-sm font-semibold"
              htmlFor={getFieldId("presentationType")}
            >
              Presentación
              <Select
                aria-describedby={mergeDescribedBy(
                  "presentationType",
                  Boolean(errors.presentationType),
                  false,
                )}
                ...
                onChange={(event) =>
                  setDraftField("presentationType", event.target.value)
                }
              ...
              {errors.presentationType && (
                <span
                  className="text-xs font-medium text-[var(--erp-danger)]"
                  id={getErrorId("presentationType")}
                >
                  {errors.presentationType}
                </span>
              )}
            ...
            >
              ...
              <Select
                ...
                className={inputClass(
                  Boolean(errors.unit),
                  Boolean(touched.unit && draft.unit && !errors.unit),
                )}
                id={getFieldId("unit")}
            ...
            <label
              className="grid gap-2 text-sm font-semibold"
              htmlFor={getFieldId("minStock")}
            >
              Mínimo comercial
              <Input
                aria-describedby={mergeDescribedBy(
                  "minStock",
                  Boolean(errors.minStock),
                  true,
                )}
                ...
                className={inputClass(
                  Boolean(errors.minStock),
                  Boolean(
                    touched.minStock && draft.minStock && !errors.minStock,
                  ),
                )}
                ...
                onChange={(event) =>
                  setNumericField("minStock", event.target.value)
                }
              ...
              <span
                className="text-xs text-[var(--erp-muted-foreground)]"
                id={getHelpId("minStock")}
              >
                Pieza requiere enteros; kilo permite decimales.
              </span>
              {errors.minStock && (
                <span
                  className="text-xs font-medium text-[var(--erp-danger)]"
                  id={getErrorId("minStock")}
                >
                  {errors.minStock}
                </span>
              )}
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <label
              className="grid gap-2 text-sm font-semibold"
              htmlFor={getFieldId("pieceWeightEquivalent")}
            >
              Equivalencia kg por pieza
              <Input
                aria-describedby={mergeDescribedBy(
                  "pieceWeightEquivalent",
                  Boolean(errors.pieceWeightEquivalent),
                  true,
                )}
                ...
                className={inputClass(
                  Boolean(errors.pieceWeightEquivalent),
                  Boolean(
                    touched.pieceWeightEquivalent &&
                    draft.pieceWeightEquivalent &&
                    !errors.pieceWeightEquivalent,
                  ),
                )}
                ...
                onChange={(event) =>
                  setNumericField("pieceWeightEquivalent", event.target.value)
                }
              ...
              <span
                className="text-xs text-[var(--erp-muted-foreground)]"
                id={getHelpId("pieceWeightEquivalent")}
              >
                Opcional. Cuando se captura, debe ser mayor que cero y permite
                hasta 3 decimales.
              </span>
              {errors.pieceWeightEquivalent && (
                <span
                  className="text-xs font-medium text-[var(--erp-danger)]"
                  id={getErrorId("pieceWeightEquivalent")}
                >
                  {errors.pieceWeightEquivalent}
                </span>
              )}
            ...
            <label
              className="grid gap-2 text-sm font-semibold"
              htmlFor={getFieldId("equivalentPolicyStatus")}
            >
              Política de equivalencia
              <Select
                aria-describedby={mergeDescribedBy(
                  "equivalentPolicyStatus",
                  Boolean(errors.equivalentPolicyStatus),
                  false,
                )}
                ...
                className={inputClass(
                  Boolean(errors.equivalentPolicyStatus),
                  Boolean(
                    touched.equivalentPolicyStatus &&
                    draft.equivalentPolicyStatus &&
                    !errors.equivalentPolicyStatus,
                  ),
                )}
                id={getFieldId("equivalentPolicyStatus")}
                onBlur={() => markTouched("equivalentPolicyStatus")}
                onChange={(event) =>
                  setDraftField("equivalentPolicyStatus", event.target.value)
                }
              ...
              {errors.equivalentPolicyStatus && (
                <span
                  className="text-xs font-medium text-[var(--erp-danger)]"
                  id={getErrorId("equivalentPolicyStatus")}
                >
                  {errors.equivalentPolicyStatus}
                </span>
              )}
          ...
          {draft.unit === "KG_AND_PIECE" && (
            <p className="rounded-xl border border-[rgba(214,155,45,0.30)] bg-[rgba(214,155,45,0.12)] p-3 text-sm text-[var(--erp-brand-gold-deep)]">
              La equivalencia oficial se gestiona en el flujo autorizado de
              equivalencias; este formulario no modifica saldos operativos.
            </p>
          )}
