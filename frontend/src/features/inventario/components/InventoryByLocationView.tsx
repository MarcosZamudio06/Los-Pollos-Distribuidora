...
export function InventoryByLocationView({
  locationId,
}: {
  locationId?: string;
}) {
    ...
    <section className="grid gap-4 rounded-2xl border border-[var(--erp-border)] bg-[var(--erp-surface-elevated)] p-5 shadow-[0_18px_50px_rgba(16,24,32,0.06)]">
      <div>
        ...
        <p className="mt-1 text-sm text-[var(--erp-muted-foreground)]">
          Usa el endpoint especificado para saldos; aquí no se infiere stock
          global por producto.
        </p>
      </div>
      <AsyncState
        empty={!balances.data?.length}
        emptyMessage="No hay saldos por ubicación para estos filtros."
            ...
            <table className="min-w-full text-left text-sm">
              ...
              <tbody>
                {pagination.pageItems.map((balance) => (
                  <tr
                    key={`${balance.productId ?? "product"}-${balance.locationId}`}
                  ...
                  >
                    ...
                    >
                      {balance.productName ?? balance.productId ?? "—"}
                    </td>
                    <td className={cellClass}>
                      {balance.locationName ?? balance.locationId}
                    </td>
                    ...
                    >
                      {balance.minQuantityKg ?? balance.minimumKg ?? "—"}
                    </td>
                    <td
                      className={`${cellClass} text-right text-[var(--erp-muted-foreground)]`}
                    >
                    ...
                    <td className={cellClass}>
                      <LowStockBadge isLowStock={balance.isLowStock} />
                    </td>
                  </tr>
                ))}
              </tbody>
