...
const labelClass =
  "grid gap-1.5 text-sm font-semibold text-[var(--erp-foreground)]";
...
export function InventoryAdjustmentModal({
  ...
  function validate() {
    ...
    if (!Number.isInteger(quantityPieces))
      return "Las piezas deben ser números enteros.";
    if (values.unit === "KG" && (quantityKg <= 0 || quantityPieces !== 0))
      return "Los ajustes por kilo requieren kilos positivos y cero piezas.";
    if (values.unit === "PIECE" && (quantityPieces <= 0 || quantityKg !== 0))
      return "Los ajustes por pieza requieren piezas positivas y cero kilos.";
    if (
      values.unit === "KG_AND_PIECE" &&
      quantityKg <= 0 &&
      quantityPieces <= 0
    )
      return "Los ajustes por kilo y pieza requieren kilos, piezas o ambos.";
    ...
    <div className="fixed inset-0 z-20 overflow-y-auto bg-[rgba(16,24,32,0.56)] px-4 py-8 backdrop-blur-sm">
      ...
      >
        <header className="flex justify-between">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[var(--erp-info)]">
              Movimiento de inventario
            </p>
            <h2 className="text-3xl font-bold tracking-[-0.04em] text-[var(--erp-foreground)]">
              Registrar ajuste
            </h2>
            <p className="mt-2 text-sm text-[var(--erp-muted-foreground)]">
              Cada corrección queda registrada contra una ubicación operativa.
            </p>
          </div>
          <button
            type="button"
            onClick={onClose}
            className="rounded-xl px-3 py-2 text-sm font-semibold text-[var(--erp-muted-foreground)] transition hover:bg-[var(--erp-surface-muted)]"
          >
            Cerrar
          </button>
        </header>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <label className={labelClass}>
            ...
            <input
              ...
              onChange={(event) =>
                setValues({ ...values, locationId: event.target.value })
              }
              required
            />
          </label>
          <label className={labelClass}>
            ...
            <select
              ...
              onChange={(event) =>
                setValues({
                  ...values,
                  type: event.target.value as InventoryAdjustmentValues["type"],
                })
              }
            >
          ...
          <label className={labelClass}>
            ...
            <select
              ...
              onChange={(event) =>
                setValues({
                  ...values,
                  unit: event.target.value as InventoryAdjustmentValues["unit"],
                })
              }
            >
          ...
          <label className={labelClass}>
            ...
            <input
              ...
              step="0.001"
              type="number"
              value={values.quantityKg ?? 0}
              onChange={(event) =>
                setValues({ ...values, quantityKg: Number(event.target.value) })
              }
            />
          </label>
          <label className={labelClass}>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <label className={labelClass}>
            ...
            <input
              ...
              onChange={(event) =>
                setValues({ ...values, referenceType: event.target.value })
              }
            />
          </label>
          <label className={labelClass}>
            ...
            <input
              ...
              onChange={(event) =>
                setValues({ ...values, referenceId: event.target.value })
              }
            />
          </label>
        </div>
