    ...
    <section className="grid gap-4 rounded-2xl border border-[var(--erp-border)] bg-[var(--erp-surface-elevated)] p-5 shadow-[0_18px_50px_rgba(16,24,32,0.06)]">
      <div>
        ...
        <p className="mt-1 text-sm text-[var(--erp-muted-foreground)]">
          Consulta operativa de entradas, salidas, ajustes, mermas y traspasos.
        </p>
      </div>
      <div className="grid gap-3 rounded-2xl border border-[var(--erp-border)] bg-[var(--erp-surface-muted)] p-4 md:grid-cols-6">
        ...
        >
          ...
          <option value="TRANSFER_OUT">Traspaso de salida</option>
          <option value="TRANSFER_IN">Traspaso de entrada</option>
        ...
        <select
          ...
          className={fieldClass}
          onChange={(event) =>
            setFilters({ ...filters, referenceType: event.target.value })
          }
          value={filters.referenceType ?? ""}
        >
          <option value="">Todos los tipos de referencia</option>
          <option value="Sale">Venta</option>
          <option value="DeliveryOrder">Pedido de reparto</option>
          <option value="POINT_OF_SALE_DAILY_CLOSE">
            Cierre diario de punto de venta
          </option>
          <option value="PURCHASE">Compra</option>
          <option value="INVENTORY_TRANSFER">Traspaso</option>
        ...
        <input
          ...
          onChange={(event) =>
            setFilters({ ...filters, referenceId: event.target.value })
          }
        />
        <div className="grid gap-2 sm:grid-cols-2 md:col-span-6">
          <label className="grid gap-1 text-xs font-semibold uppercase tracking-[0.14em] text-[var(--erp-muted-foreground)]">
            Desde
            <input
      ...
      <AsyncState
        ...
        emptyMessage="No hay movimientos para estos filtros."
            ...
            <table className="min-w-full text-left text-sm">
              <thead className="border-b border-[var(--erp-border)] bg-[var(--erp-surface-muted)] text-[11px] font-semibold uppercase tracking-[0.16em] text-[var(--erp-muted-foreground)]">
                <tr>
                  ...
                  <th className={cellClass}>Tipo</th>
                  <th className={cellClass}>Kg</th>
                  <th className={cellClass}>Piezas</th>
                  <th className={cellClass}>Anterior / nuevo</th>
                  <th className={cellClass}>Motivo</th>
                  <th className={cellClass}>Referencia</th>
                  <th className={cellClass}>Usuario</th>
                  <th className={cellClass}>Fecha</th>
                </tr>
              </thead>
              <tbody>
                {pagination.pageItems.map((movement) => (
                  ...
                  >
                    <td
                      className={`${cellClass} font-semibold text-[var(--erp-foreground)]`}
                    >
                    ...
                    <td
                      className={`${cellClass} min-w-48 text-[var(--erp-muted-foreground)]`}
                    >
                      {movement.reason ?? "—"}
                    </td>
                    <td className={cellClass}>
                      {movement.referenceType || movement.referenceId
                        ? `${movement.referenceType ?? "REFERENCE"} ${movement.referenceId ?? ""}`.trim()
                        : (movement.reference ?? "—")}
                    </td>
                    <td className={cellClass}>
                    ...
                    <td
                      className={`${cellClass} whitespace-nowrap text-[var(--erp-muted-foreground)]`}
                    >
