...
export const productService = {
  async listCategories(accessToken?: string | null) {
    const response = await apiClient.get<ListEnvelope<InventoryCategory>>(
      "/categories?isActive=true&limit=100",
      {
  ...
  async listProducts(
    ...
    const response = await apiClient.get<ListEnvelope<Product>>(
      withParams("/products", filters),
      {
        headers: authHeaders(accessToken),
      },
    );
  ...
  async listBalances(
    ...
    const response = await apiClient.get<ListEnvelope<InventoryBalance>>(
      withParams("/inventory/balances", filters),
      {
        headers: authHeaders(accessToken),
      },
    );
  ...
  async listMovements(
    ...
    const response = await apiClient.get<ListEnvelope<InventoryMovement>>(
      withParams("/inventory/movements", filters),
      {
        headers: authHeaders(accessToken),
      },
    );
  ...
  async listTransfers(accessToken?: string | null) {
    const response = await apiClient.get<ListEnvelope<InventoryTransfer>>(
      "/inventory-transfers",
      {
        headers: authHeaders(accessToken),
      },
  ...
  async getTransfer(id: string, accessToken?: string | null) {
    const response = await apiClient.get<ItemEnvelope<InventoryTransfer>>(
  ...
  async createTransfer(
    values: InventoryTransferValues,
    accessToken?: string | null,
  ) {
    const response = await apiClient.post<
