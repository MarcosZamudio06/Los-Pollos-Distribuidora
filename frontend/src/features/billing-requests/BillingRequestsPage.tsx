...
const field =
  "h-11 w-full rounded-xl border border-[color:var(--erp-border)] bg-white px-3 text-sm font-semibold outline-none focus:border-[var(--erp-brand-gold)]";
...
function CreateRequestCard() {
  ...
  const disabledReason = !saleId
    ? "Selecciona una venta."
    : sale.isLoading
      ? "Consultando venta…"
      : !customerId
        ? "La venta no tiene cliente."
        : sale.data?.status === "CANCELLED"
          ? "La venta está cancelada."
          : sale.data?.billingRequestId
            ? "La venta ya tiene una solicitud."
            : !reason.trim()
              ? "Captura el motivo."
              : null;
    ...
    <Card className="p-5">
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          <Plus className="h-5 w-5 text-[var(--erp-brand-gold-deep)]" />
          Nueva solicitud
        </CardTitle>
        <CardDescription>
          Créala para una venta existente sin escribir identificadores internos
          de solicitud.
        </CardDescription>
      </CardHeader>
      <CardContent className="mt-4 grid gap-3">
        <MiniAjaxSelect
          ...
          label="Venta"
          onChange={setSaleId}
          placeholder="Busca por número de venta"
          value={saleId}
        />
        <input
          className={field}
        ...
        <Button
          disabled={Boolean(disabledReason) || create.isPending}
      ...
      <ConfirmationDialog
        confirmLabel="Crear solicitud"
        description="Se registrará un control administrativo asociado a la venta. No se emitirá ningún documento fiscal."
      ...
      >
        <p>
          <strong>Venta:</strong> {sale.data?.saleNumber ?? saleId}
        </p>
        <p>
...
export function BillingRequestsPage() {
  ...
  const update = (next: Partial<BillingRequestFilters>) =>
    ...
    <main className="min-h-screen bg-[var(--erp-background)] px-4 py-6 text-[var(--erp-foreground)] sm:px-6 lg:px-8">
      <div className="mx-auto grid max-w-7xl gap-6">
        ...
        <Card className="p-5">
          ...
          <CardContent className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <select
              ...
              onChange={(event) =>
                update({
                  status: event.target.value as BillingRequestStatus | "",
                })
              }
        ...
        <Card className="overflow-hidden">
          <div className="flex items-center justify-between border-b border-[color:var(--erp-border)] p-5">
            <div>
              <CardTitle className="flex items-center gap-2">
            ...
            <Search className="h-5 w-5 text-[var(--erp-muted-foreground)]" />
          </div>
          {requests.isLoading && <p className="p-6">Cargando solicitudes…</p>}
          {requests.error && (
            <p className="p-6 font-semibold text-[var(--erp-danger)]">
              ...
              <tbody>
                {data?.items.map((item) => (
                  ...
                  >
                    ...
                    <td className="p-4">
                      <BillingRequestStatusBadge status={item.status} />
                    </td>
                    <td className="max-w-sm p-4">{item.reason ?? "—"}</td>
                    <td className="p-4">
                      {new Date(item.requestedAt).toLocaleDateString("es-MX")}
                    </td>
                    <td className="p-4">
                      <Link
                        className="font-black text-[var(--erp-info)]"
                        to={`/billing-requests/${item.id}`}
                      >
                        Ver detalle
                      </Link>
                    </td>
          ...
          {!requests.isLoading && !data?.items.length && (
            <p className="p-8 text-center text-[var(--erp-muted-foreground)]">
