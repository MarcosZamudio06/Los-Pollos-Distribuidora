...
import {
  availableBillingRequestActions,
...
function RequestEditor({
  ...
  async function mutate(status?: BillingRequestStatus) {
    ...
    await update.mutateAsync({
      status,
      expectedVersion: status ? data.version : undefined,
      reason: reason.trim(),
  ...
  return (
    <Card className="p-5">
      <CardHeader>
        ...
        <CardDescription>
          El estado no modifica inventario, pagos ni importes de la venta.
        </CardDescription>
      </CardHeader>
      <CardContent className="mt-4 grid gap-4">
        <label className="grid gap-2 text-sm font-bold">
          Motivo
          <textarea
            className="min-h-24 rounded-xl border border-[color:var(--erp-border)] p-3"
            disabled={!sellerCanEdit && role !== "ADMIN"}
...
export function BillingRequestDetailPage() {
  ...
  if (request.isLoading)
    return <main className="p-8">Cargando solicitud…</main>;
  if (!data || request.error)
    return (
      <main className="p-8">
        ...
        <Link to="/billing-requests">Volver</Link>
      </main>
    );
      ...
      <div className="mx-auto grid max-w-5xl gap-6">
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <h1 className="mt-3 text-3xl font-black">
                Venta {data.sale?.saleNumber ?? data.saleNumber ?? data.saleId}
              </h1>
              <p className="mt-2 text-white/70">
                {data.customer?.name ?? data.customerName ?? data.customerId}
              </p>
            </div>
            <BillingRequestStatusBadge status={data.status} />
          </div>
        </header>
          ...
          <Card className="p-5">
            ...
            <CardContent className="mt-4 grid gap-3 text-sm">
              <p>
                <strong>Venta:</strong>{" "}
                <Link
                  className="text-[var(--erp-info)]"
                  to={`/sales/${data.saleId}`}
                >
                  {data.sale?.saleNumber ?? data.saleId}
                </Link>
              </p>
              <p>
                <strong>Cliente:</strong>{" "}
                {data.customer?.name ?? data.customerId}
              </p>
              <p>
                <strong>Cuenta por cobrar:</strong>{" "}
                {data.accountReceivable?.id ?? "No aplica"}
              </p>
              <p>
                <strong>Solicitó:</strong>{" "}
                {data.requestedBy?.name ?? data.requestedByUserId}
              </p>
              <p>
                <strong>Revisó:</strong>{" "}
                {data.reviewedBy?.name ?? data.reviewedByUserId ?? "Pendiente"}
              </p>
            </CardContent>
          </Card>
        </div>
        ...
        <Card className="p-5">
          <CardHeader>
            ...
            <CardDescription>
              Bitácora cronológica de actor, fecha, motivo y notas.
            </CardDescription>
          </CardHeader>
          <CardContent className="mt-5 grid gap-4">
            {data.history?.map((entry) => (
              <article
                className="relative border-l-2 border-[var(--erp-brand-gold)] pl-5"
                key={entry.id}
              >
                <div className="flex flex-wrap items-center gap-2">
                  <BillingRequestStatusBadge status={entry.toStatus} />
                  <span className="text-xs font-semibold text-[var(--erp-muted-foreground)]">
                    {new Date(entry.changedAt).toLocaleString("es-MX")}
                  </span>
                </div>
                <p className="mt-2 font-bold">{entry.reason}</p>
                <p className="text-sm text-[var(--erp-muted-foreground)]">
                  {entry.notes || "Sin notas"} ·{" "}
                  {entry.changedBy?.name ?? entry.changedByUserId}
                </p>
              </article>
            ))}
          </CardContent>
