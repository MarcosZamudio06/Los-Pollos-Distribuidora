...
describe("invoice reconciliation", () => {
  it("prepares document and item applications from the approved request", () => {
          ...
          requestedItems: [
            {
              ...
              requestedTax: "10.00",
              requestedTotal: "100.00",
              saleItem: { id: "item-1", productNameSnapshot: "Pollo" },
            },
          ],
          saleDocument: {
            id: "document-1",
    ...
    expect(draft.applications[0].items[0]).toEqual(
      expect.objectContaining({ saleItemId: "item-1", totalApplied: "100.00" }),
    );
    expect(draft.applications[0].items).toHaveLength(1);
    expect(reconciliationBalances(draft)).toEqual({
