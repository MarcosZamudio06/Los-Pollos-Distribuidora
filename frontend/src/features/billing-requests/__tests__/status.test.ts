...
describe("billing request status rules", () => {
  it("maps the documented state machine to ADMIN actions", () => {
    expect(availableBillingRequestActions("REQUESTED", "ADMIN")).toEqual([
      "IN_REVIEW",
      "CANCELLED",
    ]);
    expect(availableBillingRequestActions("IN_REVIEW", "ADMIN")).toEqual([
      "APPROVED",
      "REJECTED",
      "CANCELLED",
    ]);
    expect(availableBillingRequestActions("REQUESTED", "BILLING")).toEqual([
      "IN_REVIEW",
      "CANCELLED",
    ]);
