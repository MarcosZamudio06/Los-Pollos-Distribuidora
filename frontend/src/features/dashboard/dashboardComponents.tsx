    ...
    <Card className="group relative overflow-hidden p-5 transition duration-200 hover:-translate-y-0.5 hover:shadow-[var(--erp-shadow-elevated)]">
      ...
      <div className="relative flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-3 text-3xl font-black tracking-[-0.06em] text-[var(--erp-foreground)]">
            {value}
          </p>
      ...
      {action && (
        <Link
          className="relative mt-4 inline-flex items-center gap-1 text-sm font-bold text-[var(--erp-info)]"
          to={action.to}
        >
          {action.label}
    ...
    <div className="rounded-[1.2rem] border border-[color:var(--erp-border)] bg-white/78 p-4 shadow-sm">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div className="flex items-start gap-3">
          <span
            className={cn(
              "mt-1 grid size-9 place-items-center rounded-2xl",
              isStale ? toneClasses.amber.icon : toneClasses.green.icon,
            )}
          >
            <Clock3 aria-hidden="true" size={18} />
          </span>
          <div>
            <p className="text-sm font-black text-[var(--erp-foreground)]">
              Actualización de datos
            </p>
            <p className="mt-1 text-sm text-[var(--erp-muted-foreground)]">
              Generado: {generatedAt} · Datos al: {dataAsOf}
            </p>
          </div>
        </div>
        <StatusBadge tone={isStale ? "amber" : "green"}>
          {isStale ? "Revisar frescura" : `Frescura ${freshnessSeconds ?? 0}s`}
        </StatusBadge>
      </div>
    </div>
  );
  ...
  return (
    <Card className="p-5 sm:p-6">
      <CardHeader className="gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          {eyebrow && (
            <p className="mb-2 text-xs font-black uppercase tracking-[0.18em] text-[var(--erp-brand-gold-deep)]">
              {eyebrow}
            </p>
          )}
...
export function EmptyState({
  ...
  return (
    <div className="rounded-[1.2rem] border border-dashed border-[color:var(--erp-border)] bg-[var(--erp-surface)] p-6 text-center">
      <ShieldCheck
        className="mx-auto text-[var(--erp-muted-foreground)]"
        size={28}
      />
      <p className="mt-3 font-black text-[var(--erp-foreground)]">{title}</p>
      <p className="mx-auto mt-2 max-w-xl text-sm leading-6 text-[var(--erp-muted-foreground)]">
        {description}
      </p>
    </div>
  );
...
export function FieldLabel({
  ...
  return (
    <label
      className={cn(
        "flex flex-col gap-1 text-xs font-black uppercase tracking-[0.14em] text-[var(--erp-muted-foreground)]",
        className,
      )}
      {...props}
    >
