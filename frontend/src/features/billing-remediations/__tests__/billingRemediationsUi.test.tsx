  ...
  useBillingRemediations: () => ({
    data: {
      ...
      legalEntities: [
        {
          id: "legal-1",
          legalName: "Distribuidora Principal",
          taxId: "AAA010101AAA",
        },
      ],
    },
...
describe("billing remediations UI contracts", () => {
  it("builds backend filters and renders a validated-resolution inbox", () => {
    expect(
      ...
      "/billing/remediations?page=2&limit=25&status=OPEN&code=INVALID_SALE_TOTAL&search=V-1",
    );
    ...
    await billingRemediationsService.resolve(
      {
        ...
        expectedSaleVersion: 7,
        expectedDocumentVersions: [
          { saleDocumentId: "document-1", expectedVersion: 3 },
        ],
        reason: "Correction",
      },
      "access-token",
  ...
  it("exposes canonical consistency findings as actionable messages", () => {
    const error = new ApiClientError(
      ...
      409,
      {
        ...
        findings: [
          {
            code: "ITEM_TOTALS_MISMATCH",
            message: "Las partidas no coinciden con la cabecera.",
          },
          {
            code: "RECEIVABLE_BALANCE_MISMATCH",
            message: "La cuenta por cobrar no coincide.",
          },
        ],
      },
    );
