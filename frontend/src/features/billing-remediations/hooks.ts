...
export function useBillingRemediations(filters: BillingRemediationFilters) {
  ...
  return useQuery({
    queryKey: ["billing-remediations", filters],
    queryFn: () => billingRemediationsService.list(filters, accessToken),
...
export function useResolveBillingRemediation() {
  ...
  return useMutation({
    mutationFn: (input: ResolveBillingRemediationInput) =>
      billingRemediationsService.resolve(input, accessToken),
    onSuccess: () => {
