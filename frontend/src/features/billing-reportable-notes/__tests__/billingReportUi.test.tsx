...
const mockState = vi.hoisted(() => ({
  ...
  detail: {
    data: undefined as BillingReportDetail | undefined,
    error: null,
    isLoading: false,
  },
  report: {
    data: undefined as BillingReportList | undefined,
...
const note = {
  ...
  activeRequested: "0.00",
  pendingInvoice: "100.00",
  pendingSubtotal: "90.00",
  pendingTax: "10.00",
  pendingTotal: "100.00",
  requestableItems: [
    {
...
const reportData: BillingReportList = {
  ...
  summary: {
    ...
    billableDocuments: 1,
    blockedDocuments: 0,
    totalBillable: "100.00",
    totalRequested: "0.00",
    totalInvoiced: "0.00",
    totalPending: "100.00",
    totalCollected: "20.00",
    totalReceivable: "80.00",
  },
  generatedAt: "2026-07-18T12:00:00.000Z",
  dataAsOf: "2026-07-18T11:58:00.000Z",
...
function renderPage() {
  ...
  return renderToStaticMarkup(
    <QueryClientProvider client={queryClient}>
      <MemoryRouter initialEntries={["/billing/reportable-notes"]}>
...
describe("billing reportable notes UI contracts", () => {
  ...
  it("persists report filters and backend sorting in the URL", () => {
    expect(
      buildBillingReportPath({
        ...
        search: "V-101",
        billingStatus: "BILLABLE",
        sortBy: "pendingInvoice",
        sortOrder: "desc",
      }),
    ).toBe(
      "/billing/reportable-notes?page=2&limit=25&search=V-101&billingStatus=BILLABLE&sortBy=pendingInvoice&sortOrder=desc",
    );
  });
  ...
  it("forwards every advanced report filter supported by the backend", () => {
    ...
    expect(
      buildBillingReportPath({ hasRequest: false, blocked: false }),
    ).toContain("hasRequest=false&blocked=false");
  });
  ...
  it("selects only documents with the same customer, currency and legal entity and an available balance", () => {
    expect(areNotesCompatible(note, { ...note, saleDocumentId: "doc-2" })).toBe(
      true,
    );
    expect(
      areNotesCompatible(note, {
        ...note,
        saleDocumentId: "doc-3",
  ...
  it("rejects individually non-requestable notes, including the first selection", () => {
    ...
    expect(
      isRequestableNote({ ...note, billingStatus: "PARTIALLY_INVOICED" }),
    ).toBe(true);
    expect(
      isRequestableNote({ ...note, billingStatus: "PENDING_INFORMATION" }),
    ).toBe(false);
    expect(isRequestableNote({ ...note, billingStatus: "REQUESTED" })).toBe(
      false,
    );
    expect(isRequestableNote({ ...note, billingStatus: "IN_PROCESS" })).toBe(
      false,
    );
    expect(isRequestableNote({ ...note, billingStatus: "BLOCKED" })).toBe(
      false,
    );
    expect(isRequestableNote({ ...note, billingStatus: "CANCELLED" })).toBe(
      false,
    );
    expect(isRequestableNote({ ...note, billingStatus: "NOT_BILLABLE" })).toBe(
      false,
    );
    expect(
      isRequestableNote({ ...note, billingStatus: "FULLY_INVOICED" }),
    ).toBe(false);
  ...
  it("keeps the detail panel mounted while its closing animation finishes", async () => {
    ...
    mockState.report = { ...mockState.report, data: reportData };
    mockState.detail = {
      data: normalizeBillingReportDetail({
        ...reportData.items[0],
        items: [],
        requests: [],
        activeInvoices: [{ id: "invoice-active", status: "ACTIVE" }],
        invoiceHistory: [{ id: "invoice-old", status: "CANCELLED" }],
        payments: [],
        delivery: null,
        audit: [],
      }),
    ...
    const detailButton = [...container.querySelectorAll("button")].find(
      (button) => button.textContent === "Ver detalle",
    ...
    expect(
      container.querySelector('[aria-label="Detalle de nota facturable"]'),
    ).not.toBeNull();
