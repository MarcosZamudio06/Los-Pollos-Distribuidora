...
export function useBillingReport(filters: BillingReportFilters) {
  ...
  return useQuery({
    ...
    queryFn: () => billingReportService.list(filters, accessToken),
    placeholderData: (previous) => previous,
  });
}
export function useBillingReportDetail(id?: string) {
  const { accessToken } = useAuth();
  return useQuery({
    enabled: Boolean(id),
    queryKey: ["billing-report-detail", id],
    queryFn: () => billingReportService.detail(id!, accessToken),
  });
}
export function useBillingRequestCommand() {
  const { accessToken } = useAuth();
  ...
  return useMutation({
    mutationFn: (
      ...
      input.kind === "create"
        ? billingReportService.createRequest(input.body, accessToken)
        : billingReportService.reviewRequest(
            input.id,
            input.action,
