...
import { validateEmployeeForm, type EmployeeFormErrors } from './employeesFormUtils'
...
export function EmployeesPage() {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setCredentials(response.data); setForm({ name: '', email: '', phone: '', roleId: '', operationalLocationId: '' }); setFormErrors({}); await load()
  ...
  return <main className="min-h-full bg-[var(--erp-background)] p-4 text-[var(--erp-foreground)] sm:p-6 lg:p-8">
    <div className="mx-auto grid max-w-7xl gap-6">
      <header className="overflow-hidden rounded-[1.6rem] border border-[color:var(--erp-border)] bg-white p-6 text-[var(--erp-foreground)] shadow-[var(--erp-shadow)] sm:p-8"><div className="flex flex-col justify-between gap-6 md:flex-row md:items-end"><div><p className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-[.2em] text-[var(--erp-brand-red)]"><ShieldCheck className="h-4 w-4"/> Control de acceso</p><h1 className="mt-4 text-3xl font-black tracking-[-.05em] sm:text-4xl">Administración de empleados</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-[var(--erp-muted-foreground)]">Alta controlada de personal operativo con credenciales temporales de un solo uso.</p></div><div className="rounded-2xl border border-[color:var(--erp-border)] bg-[var(--erp-surface-muted)] px-5 py-4"><p className="text-xs uppercase tracking-[.16em] text-[var(--erp-muted-foreground)]">Personal registrado</p><p className="mt-1 text-3xl font-black text-[var(--erp-brand-black)]">{employees.total}</p></div></div></header>
       {user?.permissions?.includes(PERMISSIONS.accessProfilesManage) && <div className="flex justify-end"><Link className="inline-flex items-center rounded-xl border border-[color:var(--erp-border)] bg-white px-4 py-2.5 text-sm font-bold text-[var(--erp-brand-red)] shadow-sm hover:border-[var(--erp-brand-red)]" to="/admin/access-profiles">Administrar perfiles y sesiones</Link></div>}
       <section className="grid gap-6 xl:grid-cols-[minmax(0,1.45fr)_minmax(19rem,.8fr)]">
        ...
        <form noValidate className="h-fit rounded-[1.4rem] border border-[color:var(--erp-border)] bg-white p-5 shadow-[var(--erp-shadow)]" onSubmit={submit}><div className="flex items-center gap-3"><span className="rounded-xl bg-[rgba(182,42,34,.08)] p-2 text-[var(--erp-brand-red)]"><UserPlus className="h-5 w-5"/></span><div><h2 className="font-black">Registrar empleado</h2><p className="text-xs text-[var(--erp-muted-foreground)]">Se asigna una contraseña temporal segura.</p></div></div><div className="mt-5 grid gap-4">{[['name','Nombre completo','text'],['email','Correo electrónico','email'],['phone','Teléfono','tel']].map(([key,label,type]) => { const field = key as 'name' | 'email' | 'phone'; return <label className="grid gap-1.5 text-sm font-semibold" key={key}>{label}<Input required type={type} maxLength={field === 'name' ? 300 : field === 'phone' ? 10 : undefined} minLength={field === 'phone' ? 10 : undefined} inputMode={field === 'phone' ? 'numeric' : undefined} aria-invalid={Boolean(formErrors[field])} aria-describedby={formErrors[field] ? `${field}-error` : undefined} value={form[field]} onChange={(event) => { const nextForm = { ...form, [key]: event.target.value }; setForm(nextForm); setFormErrors({ ...formErrors, [field]: validateEmployeeForm(nextForm)[field] }) }}/>{formErrors[field] && <span id={`${field}-error`} role="alert" className="text-xs font-medium text-[var(--erp-danger)]">{formErrors[field]}</span>}</label> })}<label className="grid gap-1.5 text-sm font-semibold">Rol<Select required aria-invalid={Boolean(formErrors.roleId)} aria-describedby={formErrors.roleId ? 'roleId-error' : undefined} value={form.roleId} onChange={(event) => { const nextForm = { ...form, roleId: event.target.value }; setForm(nextForm); setFormErrors({ ...formErrors, roleId: validateEmployeeForm(nextForm).roleId }) }}><option value="">Selecciona un rol</option>{roles.map((role) => <option key={role.id} value={role.id}>{role.name}</option>)}</Select>{formErrors.roleId && <span id="roleId-error" role="alert" className="text-xs font-medium text-[var(--erp-danger)]">{formErrors.roleId}</span>}</label><label className="grid gap-1.5 text-sm font-semibold">Punto de venta<Select required aria-invalid={Boolean(formErrors.operationalLocationId)} aria-describedby={formErrors.operationalLocationId ? 'operationalLocationId-error' : undefined} value={form.operationalLocationId} onChange={(event) => { const nextForm = { ...form, operationalLocationId: event.target.value }; setForm(nextForm); setFormErrors({ ...formErrors, operationalLocationId: validateEmployeeForm(nextForm).operationalLocationId }) }}><option value="">Selecciona un punto de venta</option>{locations.map((location) => <option key={location.id} value={location.id}>{location.name}</option>)}</Select>{formErrors.operationalLocationId && <span id="operationalLocationId-error" role="alert" className="text-xs font-medium text-[var(--erp-danger)]">{formErrors.operationalLocationId}</span>}</label><Button disabled={creating} size="lg" type="submit">{creating ? 'Registrando…' : 'Registrar empleado'}</Button></div></form>
      </section>
    </div>
