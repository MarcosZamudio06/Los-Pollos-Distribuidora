  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await apiClient.post<
        Envelope<Employee & { temporaryPassword: string }>,
        typeof form
      >("/users", { body: form, headers });
  ...
  return (
    <main className="min-h-full bg-[var(--erp-background)] p-4 text-[var(--erp-foreground)] sm:p-6 lg:p-8">
      <div className="mx-auto grid max-w-7xl gap-6">
        <header className="overflow-hidden rounded-[1.6rem] border border-[color:var(--erp-border)] bg-white p-6 text-[var(--erp-foreground)] shadow-[var(--erp-shadow)] sm:p-8">
          <div className="flex flex-col justify-between gap-6 md:flex-row md:items-end">
            <div>
              <p className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-[.2em] text-[var(--erp-brand-red)]">
                <ShieldCheck className="h-4 w-4" /> Control de acceso
              </p>
              ...
              <p className="mt-3 max-w-2xl text-sm leading-6 text-[var(--erp-muted-foreground)]">
                Alta controlada de personal operativo con credenciales
                temporales de un solo uso.
              </p>
            </div>
            <div className="rounded-2xl border border-[color:var(--erp-border)] bg-[var(--erp-surface-muted)] px-5 py-4">
              <p className="text-xs uppercase tracking-[.16em] text-[var(--erp-muted-foreground)]">
                Personal registrado
              </p>
              <p className="mt-1 text-3xl font-black text-[var(--erp-brand-black)]">
                {employees.total}
              </p>
            </div>
          </div>
        </header>
        {user?.permissions?.includes(PERMISSIONS.accessProfilesManage) && (
          <div className="flex justify-end">
            <Link
              className="inline-flex items-center rounded-xl border border-[color:var(--erp-border)] bg-white px-4 py-2.5 text-sm font-bold text-[var(--erp-brand-red)] shadow-sm hover:border-[var(--erp-brand-red)]"
              to="/admin/access-profiles"
            >
              Administrar perfiles y sesiones
            </Link>
          </div>
        )}
        <section className="grid gap-6 xl:grid-cols-[minmax(0,1.45fr)_minmax(19rem,.8fr)]">
          <div className="grid gap-5">
            <section className="rounded-[1.4rem] border border-[color:var(--erp-border)] bg-white p-4 shadow-[var(--erp-shadow)]">
              <div className="grid gap-3 md:grid-cols-4">
                <label className="relative md:col-span-2">
                  <Search className="pointer-events-none absolute left-3 top-3 h-4 w-4 text-[var(--erp-muted-foreground)]" />
                  <Input
                    className="pl-9"
            ...
            <section className="overflow-hidden rounded-[1.4rem] border border-[color:var(--erp-border)] bg-white shadow-[var(--erp-shadow)]">
              ...
              <footer className="flex items-center justify-between border-t border-[color:var(--erp-border)] px-4 py-3 text-xs text-[var(--erp-muted-foreground)]">
                <span>
                  Página {employees.page} de{" "}
                  {Math.max(1, Math.ceil(employees.total / employees.limit))} ·{" "}
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="font-black">Registrar empleado</h2>
                <p className="text-xs text-[var(--erp-muted-foreground)]">
                  Se asigna una contraseña temporal segura.
                </p>
              </div>
            </div>
            <div className="mt-5 grid gap-4">
              {[
                ...
                const field = key as "name" | "email" | "phone";
                return (
                  <label
                  ...
                  >
                    ...
                    <Input
                      ...
                      onChange={(event) => {
                        ...
                        setFormErrors({
                          ...formErrors,
                          [field]: validateEmployeeForm(nextForm)[field],
                        });
                      }}
                    />
                    {formErrors[field] && (
              ...
              <label className="grid gap-1.5 text-sm font-semibold">
                ...
                <Select
                  ...
                  aria-describedby={
                    formErrors.roleId ? "roleId-error" : undefined
                  }
                  value={form.roleId}
                  onChange={(event) => {
                    ...
                    setFormErrors({
                      ...formErrors,
                      roleId: validateEmployeeForm(nextForm).roleId,
                    });
                  }}
                >
              ...
              <label className="grid gap-1.5 text-sm font-semibold">
                ...
                <Select
                  ...
                  aria-describedby={
                    formErrors.operationalLocationId
                      ? "operationalLocationId-error"
                      : undefined
                  }
                  ...
                  onChange={(event) => {
                    ...
                    setFormErrors({
                      ...formErrors,
                      operationalLocationId:
                        validateEmployeeForm(nextForm).operationalLocationId,
                    });
                  }}
                >
                ...
                {formErrors.operationalLocationId && (
                  <span
                    id="operationalLocationId-error"
                    role="alert"
                    className="text-xs font-medium text-[var(--erp-danger)]"
                  >
                    {formErrors.operationalLocationId}
      ...
      {credentials && (
        ...
        >
          <section className="w-full max-w-md rounded-[1.5rem] border border-[var(--erp-brand-gold)] bg-white p-6 shadow-2xl">
            ...
            <h2 className="mt-3 text-2xl font-black">Empleado registrado</h2>
            <p className="mt-2 text-sm text-[var(--erp-muted-foreground)]">
              Guarda estos datos ahora. La contraseña no volverá a mostrarse y
              deberá cambiarse al iniciar sesión.
            </p>
            <dl className="mt-5 grid gap-3 rounded-xl bg-[var(--erp-surface-muted)] p-4 text-sm">
              <div>
                <dt className="text-xs text-[var(--erp-muted-foreground)]">
                  Número de control
                </dt>
                <dd className="font-mono font-bold">
                  {credentials.controlNumber}
                </dd>
              </div>
              <div>
                <dt className="text-xs text-[var(--erp-muted-foreground)]">
              ...
              <div>
                ...
                <dd className="font-mono font-bold break-all">
            ...
            <div className="mt-5 flex justify-end gap-3">
              ...
              <Button onClick={() => setCredentials(null)}>Entendido</Button>
            </div>
          </section>
        </div>
      )}
