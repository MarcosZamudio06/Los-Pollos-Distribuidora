...
export const customerService = {
  async listCustomers(filters: CustomerFilters, accessToken?: string | null) {
    const response = await apiClient.get<ListEnvelope<Customer>>(
      withParams("/customers", filters),
      {
  ...
  async getCustomer(id: string, accessToken?: string | null) {
    const response = await apiClient.get<ItemEnvelope<Customer>>(
      `/customers/${id}`,
      {
        headers: authHeaders(accessToken),
      },
  ...
  async getCreditSummary(id: string, accessToken?: string | null) {
    const response = await apiClient.get<ItemEnvelope<CustomerCreditSummary>>(
  ...
  async listSales(id: string, accessToken?: string | null) {
    const response = await apiClient.get<ListEnvelope<CustomerSale>>(
      `/customers/${id}/sales`,
      {
        headers: authHeaders(accessToken),
      },
  ...
  async listPayments(id: string, accessToken?: string | null) {
    const response = await apiClient.get<ListEnvelope<CustomerPayment>>(
  ...
  async deactivateCustomer(id: string, accessToken?: string | null) {
    const response = await apiClient.delete<ItemEnvelope<Customer>>(
      `/customers/${id}`,
      {
        headers: authHeaders(accessToken),
      },
