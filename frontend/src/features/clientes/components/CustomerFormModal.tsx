...
export function CustomerFormModal({
  ...
  const [draft, setDraft] = useState<CustomerFormDraft>(() =>
    toCustomerFormDraft(customer),
  );
  const [errors, setErrors] = useState<CustomerFormErrors>({});
  const [touched, setTouched] = useState<
    Partial<Record<CustomerFormField, boolean>>
  >({});
  const [isSubmitted, setIsSubmitted] = useState(false);
  const [creditLimitFocused, setCreditLimitFocused] = useState(false);
  const [submitError, setSubmitError] = useState<string | null>(null);
  const [pendingCustomer, setPendingCustomer] = useState<ReturnType<
    typeof toCustomerFormValues
  > | null>(null);
  ...
  function setDraftField(
    ...
    const normalizedValue =
      typeof nextValue === "string" && transform
        ? transform(nextValue)
        : nextValue;
    const nextDraft = {
      ...draft,
  ...
  function setMoneyField(rawValue: string) {
    ...
    if (touched.creditLimit || isSubmitted) {
      ...
      setErrors((current) => ({
        ...current,
        creditLimit: nextError ?? undefined,
      }));
    }
    setSubmitError(null);
  }
  ...
  function blurMoneyField() {
    ...
    const nextDraft = {
      ...draft,
      creditLimit:
        formatCurrencyDisplay(draft.creditLimit) || draft.creditLimit,
    } as CustomerFormDraft;
    setDraft(nextDraft);
    const nextError = validateCustomerField(
      "creditLimit",
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const nextDraft = {
      ...draft,
      creditLimit: creditLimitFocused
        ? normalizeCurrencyInput(draft.creditLimit)
        : draft.creditLimit,
    } as CustomerFormDraft;
    ...
    <div className="fixed inset-0 z-50 overflow-y-auto bg-[rgba(17,24,21,0.58)] px-4 py-8 backdrop-blur-sm">
      ...
      >
        <header className="flex justify-between gap-4 border-b border-[color:var(--erp-border)] pb-5">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.24em] text-[var(--erp-info)]">
              Clientes
            </p>
            ...
            <p className="mt-2 text-sm text-[var(--erp-muted-foreground)]">
              Captura validada para formato empresarial en México, con RFC,
              teléfono y crédito controlados desde el formulario.
            </p>
          </div>
          <button
            aria-label="Cerrar"
        ...
        <fieldset className="grid gap-4">
          <div className="grid gap-4 md:grid-cols-3">
            <label
              ...
              htmlFor={getFieldId("customerNumber")}
            >
              Número interno
              <Input
                aria-describedby={mergeDescribedBy(
                  "customerNumber",
                  Boolean(errors.customerNumber),
                  false,
                )}
          ...
          <div className="grid gap-4 md:grid-cols-3">
            <label
              className="grid gap-2 text-sm font-semibold"
              htmlFor={getFieldId("commercialName")}
            >
              Nombre comercial
              <Input
                aria-describedby={mergeDescribedBy(
                  "commercialName",
                  Boolean(errors.commercialName),
                  false,
                )}
            ...
            >
              ...
              <Input
                ...
                className={inputClass(
                  Boolean(errors.phone),
                  Boolean(
                    touched.phone && draft.phone.length === 10 && !errors.phone,
                  ),
                )}
                id={getFieldId("phone")}
                inputMode="tel"
                ...
                onChange={(event) =>
                  setDraftField("phone", event.target.value, formatMexicanPhone)
                }
              ...
              <span
                ...
                id={getHelpId("phone")}
              >
                Captura nacional de 10 dígitos. El sistema guarda solo números.
              </span>
              {errors.phone && (
                <span
                  className="text-xs font-medium text-[var(--erp-danger)]"
            ...
            <label
              className="grid gap-2 text-sm font-semibold"
              htmlFor={getFieldId("customerType")}
            >
              Tipo
              <Select
                aria-describedby={mergeDescribedBy(
                  "customerType",
                  Boolean(errors.customerType),
                  false,
                )}
                ...
                onChange={(event) =>
                  setDraftField("customerType", event.target.value)
                }
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            >
              ...
              <Input
                ...
                className={inputClass(
                  Boolean(errors.email),
                  Boolean(touched.email && draft.email && !errors.email),
                )}
                id={getFieldId("email")}
                inputMode="email"
                onBlur={() => markTouched("email")}
                onChange={(event) =>
                  setDraftField("email", event.target.value, cleanEmail)
                }
          ...
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            ...
            >
              ...
              <Input
                ...
                onFocus={() => {
                  ...
                  setDraft((current) => ({
                    ...current,
                    creditLimit: normalizeCurrencyInput(current.creditLimit),
                  }));
                }}
                placeholder="25,000.00"
                value={
                  creditLimitFocused ? draft.creditLimit : creditLimitDisplay
                }
              />
              <span
                className="text-xs text-[var(--erp-muted-foreground)]"
                id={getHelpId("creditLimit")}
              >
                Se guarda como monto numérico y siempre se muestra con separador
                de miles y dos decimales.
              </span>
              {errors.creditLimit && (
                <span
                  className="text-xs font-medium text-[var(--erp-danger)]"
                  id={getErrorId("creditLimit")}
                >
                  {errors.creditLimit}
                </span>
              )}
            ...
            <label
              className="grid gap-2 text-sm font-semibold"
              htmlFor={getFieldId("creditDays")}
            >
              Días de crédito
              <Input
                aria-describedby={mergeDescribedBy(
                  "creditDays",
                  Boolean(errors.creditDays),
                  true,
                )}
              ...
              <span
                className="text-xs text-[var(--erp-muted-foreground)]"
                id={getHelpId("creditDays")}
              >
                Rango recomendado para ERP: 0 a 365 días.
              </span>
              {errors.creditDays && (
                <span
                  className="text-xs font-medium text-[var(--erp-danger)]"
                  id={getErrorId("creditDays")}
                >
                  {errors.creditDays}
                </span>
              )}
            ...
            <label
              className="grid gap-2 text-sm font-semibold"
              htmlFor={getFieldId("creditStatus")}
            >
              Estado de crédito
              <Select
                aria-describedby={mergeDescribedBy(
                  "creditStatus",
                  Boolean(errors.creditStatus),
                  false,
                )}
                ...
                onChange={(event) =>
                  setDraftField("creditStatus", event.target.value)
                }
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <label
              className="grid gap-2 text-sm font-semibold"
              htmlFor={getFieldId("deliveryAddress")}
            >
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            >
              ...
              <Input
                ...
                className={inputClass(
                  Boolean(errors.taxId),
                  Boolean(touched.taxId && draft.taxId && !errors.taxId),
                )}
                id={getFieldId("taxId")}
                onBlur={() => markTouched("taxId")}
                onChange={(event) =>
                  setDraftField("taxId", event.target.value, cleanTaxId)
                }
              ...
              <span
                ...
                id={getHelpId("taxId")}
              >
                RFC SAT: 12 caracteres para persona moral y 13 para persona
                física.
              </span>
              {errors.taxId && (
                <span
          ...
          <label className="flex items-center gap-3 rounded-xl border border-[color:var(--erp-border)] bg-[var(--erp-surface)] p-3 text-sm font-semibold">
            <input
              ...
              onChange={(event) =>
                setDraftField("requiresBilling", event.target.checked)
              }
              type="checkbox"
            />
      ...
      <ConfirmationDialog
        ...
        description="Verifique que la información del cliente sea correcta antes de guardarla."
        isLoading={saveCustomer.isPending}
        onConfirm={confirmRegistration}
        onOpenChange={(open) => {
          if (!open) setPendingCustomer(null);
        }}
        open={Boolean(pendingCustomer)}
        title="Confirmar registro"
      >
        <p>
          <strong>Cliente:</strong> {pendingCustomer?.name}
        </p>
        <p>
          <strong>Tipo:</strong> {pendingCustomer?.customerType}
        </p>
        <p>
          <strong>Teléfono:</strong> {pendingCustomer?.phone || "—"}
        </p>
        {submitError && (
