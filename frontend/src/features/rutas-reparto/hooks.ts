...
export function useRoutePlannerCatalog(search = "", originLocationId = "") {
  ...
  const drivers = useQuery({
    queryKey: ["route-planner", "drivers"],
    queryFn: () => deliveryService.listPlannerDrivers(accessToken),
  });
  ...
  const sales = useQuery({
    ...
    queryFn: () =>
      deliveryService.listEligibleSales(
        { limit: 100, originLocationId, search: search || undefined },
        accessToken,
      ),
  });
...
export function useAddressSearch() {
  ...
  return useMutation({
    mutationFn: (params: {
      ...
      latitude?: number;
      longitude?: number;
    }) => deliveryService.searchAddresses(params, accessToken),
  });
}
...
export function useReverseAddress() {
  ...
  return useMutation({
    mutationFn: ({
      ...
      latitude: number;
      longitude: number;
    }) => deliveryService.reverseAddress(latitude, longitude, accessToken),
  });
}
...
export function useCreateRoutePlan() {
  const { accessToken } = useAuth();
  return useMutation({
    mutationFn: (payload: CreateDeliveryRoutePlanPayload) =>
      deliveryService.createRoutePlan(payload, accessToken),
  });
}
...
export function useCreateOptimizedRoute() {
  ...
  return useMutation({
    mutationFn: ({
      ...
      deliveryService.createOptimizedRoute(
        payload,
        idempotencyKey,
        accessToken,
      ),
    onSuccess: () => {
      ...
      void queryClient.invalidateQueries({
        queryKey: ["route-planner", "eligible-sales"],
      });
    },
  });
}
...
export function useCreateDeliveryRoute() {
  ...
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (payload: CreateDeliveryRoutePayload) =>
      deliveryService.createRoute(payload, accessToken),
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: ["delivery-routes"] });
    },
  });
...
export function useAssignDeliveryRouteOrders(routeId: string) {
  ...
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (payload: AssignDeliveryRouteOrdersPayload) =>
      deliveryService.assignOrders(routeId, payload, accessToken),
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: ["delivery-routes"] });
    },
  });
...
export function useRouteSettlement(settlementId?: string) {
  ...
  return useQuery({
    ...
    queryFn: () =>
      deliveryService.getSettlement(settlementId as string, accessToken),
  });
}
...
export function useCloseRouteSettlement(settlementId: string) {
  ...
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (payload: CloseRouteSettlementPayload) =>
      deliveryService.closeSettlement(settlementId, payload, accessToken),
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: ["delivery-routes"] });
      void queryClient.invalidateQueries({ queryKey: ["route-settlements"] });
    },
