...
vi.mock("../hooks", () => ({
  useRoutePlannerCatalog: (search = "", originLocationId = "") => {
    ...
    const saleItems = [
      {
        saleId: "sale-ver",
        saleNumber: "V-2001",
        customerName: "Cliente Veracruz",
        suggestedDeliveryAddress: "Centro, Veracruz",
        status: "CONFIRMED",
        routeId: null,
      },
      {
        saleId: "sale-alv",
        saleNumber: "V-2002",
        customerName: "Cliente Alvarado",
        suggestedDeliveryAddress: "Centro, Alvarado",
        status: "CONFIRMED",
        routeId: null,
      },
    ];
    return {
      drivers: {
        data: [{ id: "driver-1", name: "Repartidor Uno" }],
        error: null,
      },
      locations: {
        data: [
...
vi.mock("../components/RoutePlannerMap", () => ({
  RoutePlannerMap: ({
    ...
    <div aria-label="Mapa para planificar la ruta">
      ...
      {stops.map((stop) => (
        <p key={stop.saleId}>
          {stop.saleId}: {stop.latitude.toFixed(4)}, {stop.longitude.toFixed(4)}
        </p>
      ))}
    </div>
  ),
...
describe("route planner delivery points", () => {
  ...
  it("hides eligible sales until an origin branch is selected and filters the catalog by it", async () => {
    ...
    try {
      ...
      expect(mockState.catalogCalls.at(-1)).toEqual({
        search: "",
        originLocationId: "origin-bdr",
      });
  ...
  it("does not let a late reverse-geocode response for one sale overwrite the active sale query", async () => {
    ...
    try {
      ...
      await act(async () => {
        pending.get(18.7735)?.({ label: "Dirección B resuelta" });
      });
      expect(
        inputByPlaceholder(container, "Buscar calle, número, colonia y ciudad")
          .value,
      ).toBe("Dirección B resuelta");

      await act(async () => {
        pending.get(19.183)?.({ label: "Dirección A tardía" });
      });
      expect(
        inputByPlaceholder(container, "Buscar calle, número, colonia y ciudad")
          .value,
  ...
  it("keeps delayed address-search results and their application bound to the sale that requested them", async () => {
    ...
    >();
    mockState.addressSearch.mockImplementation(
      (params?: AddressSearchPayload) =>
        new Promise((resolve) => {
          pending.set(params?.q ?? "", resolve);
        }),
    );
    const { container, root } = await renderPage();
    try {
      ...
      const addressInput = inputByPlaceholder(
        container,
        "Buscar calle, número, colonia y ciudad",
      );
      await act(async () => {
        setInputValue(addressInput, "Búsqueda A");
      });
      await act(async () => {
        exactButton(container, "Buscar").click();
      });
      ...
      await act(async () => {
        setInputValue(addressInput, "Búsqueda B");
      });
      await act(async () => {
        exactButton(container, "Buscar").click();
      });
      ...
      await act(async () => {
        pending.get("Búsqueda B")?.({
          items: [
            {
              label: "Resultado exclusivo B",
              latitude: 18.774,
              longitude: -95.762,
            },
          ],
      ...
      await act(async () => {
        pending.get("Búsqueda A")?.({
          items: [
            {
              label: "Resultado tardío A",
              latitude: 19.184,
              longitude: -96.135,
            },
          ],
      ...
      await act(async () => {
        exactButton(
          container,
          "Resultado exclusivo B18.774000, -95.762000",
        ).click();
      });
      expect(
        inputByPlaceholder(container, "Buscar calle, número, colonia y ciudad")
          .value,
      ).toBe("Resultado exclusivo B");
  ...
  it("invalidates old async responses when a sale is removed and re-added with the same id", async () => {
    ...
    mockState.addressSearch.mockImplementation(
      (params?: AddressSearchPayload) =>
        new Promise((resolve) => {
          pendingSearch.set(params?.q ?? "", resolve);
        }),
    );
    mockState.reverseAddress.mockImplementation(
      ({ latitude }: { latitude: number }) =>
        new Promise((resolve) => {
          pendingReverse.set(latitude, resolve);
        }),
    );
    const { container, root } = await renderPage();
    try {
      ...
      await act(async () => {
        setInputValue(addressInput, "Búsqueda de instancia anterior");
      });
      await act(async () => {
        exactButton(container, "Buscar").click();
      });
      ...
      expect(
        inputByPlaceholder(container, "Buscar calle, número, colonia y ciudad")
          .value,
      ).toBe("Centro, Veracruz");

      await act(async () => {
        pendingSearch.get("Búsqueda de instancia anterior")?.({
          items: [
            { label: "Resultado obsoleto", latitude: 19.19, longitude: -96.14 },
          ],
        });
      });
      await act(async () => {
        pendingReverse.get(19.183)?.({ label: "Dirección obsoleta" });
      });
      ...
      expect(container.textContent).toContain(
        "V-2001 · Cliente VeracruzCentro, Veracruz",
      );
    } finally {
      await act(async () => {
        root.unmount();
