...
export type EvidenceType =
  "PHOTO" | "SIGNATURE" | "GEOLOCATION" | "NOTE" | string;
...
export type EligibleDeliverySale = {
  saleId: string;
  saleNumber: string;
...
export type RoutePlanStop = RoutePlanStopInput & {
  sequence: number;
...
export type RouteCollectionResponse = {
  ...
  deliveryOrder: Pick<
    DeliveryOrder,
    "id" | "status" | "derivedCollectedAmount"
  >;
};
...
export type CreateDeliveryIncidentPayload = {
  status:
    | Extract<
        DeliveryOrderStatus,
        "NOT_DELIVERED" | "PARTIALLY_REJECTED" | "RETURNED"
      >
    | DeliveryOrderStatus;
  reason: string;
