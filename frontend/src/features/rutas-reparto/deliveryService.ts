...
type ListEnvelope<T> =
  ApiEnvelope<T[] | { items?: T[] }> | { items?: T[] } | T[];
...
export const deliveryService = {
  async getRoutingTechnicalStatus(accessToken?: string | null) {
    const response = await apiClient.get<ItemEnvelope<RoutingTechnicalStatus>>(
      "/delivery-routing/technical-status",
      { headers: authHeaders(accessToken) },
    );
    return unwrapItem(response);
  },
  async listEligibleSales(
    filters: {
      page?: number;
      limit?: number;
      search?: string;
    ...
    const response = await apiClient.get<ListEnvelope<EligibleDeliverySale>>(
      withParams("/delivery-route-planning/eligible-sales", filters),
      { headers: authHeaders(accessToken) },
    );
  ...
  async searchAddresses(
    ...
    const response = await apiClient.get<ListEnvelope<GeocodingResult>>(
      withParams("/geocoding/search", filters),
      { headers: authHeaders(accessToken) },
    );
    return unwrapList(response);
  },
  async reverseAddress(
    ...
    const response = await apiClient.get<ItemEnvelope<GeocodingResult>>(
      withParams("/geocoding/reverse", { latitude, longitude }),
      { headers: authHeaders(accessToken) },
    );
    return unwrapItem(response);
  },
  async createRoutePlan(
    payload: CreateDeliveryRoutePlanPayload,
    accessToken?: string | null,
  ) {
    const response = await apiClient.post<
      ItemEnvelope<DeliveryRoutePlan>,
      CreateDeliveryRoutePlanPayload
  ...
  async listPlannerLocations(accessToken?: string | null) {
    const response = await apiClient.get<ListEnvelope<PlannerLocation>>(
      "/locations?isActive=true&limit=100",
      { headers: authHeaders(accessToken) },
    );
    return unwrapList(response).items.filter(
      (location) =>
  ...
  async listRoutes(
    ...
    const response = await apiClient.get<ListEnvelope<DeliveryRouteListItem>>(
      withParams("/delivery-routes", filters),
      {
        headers: authHeaders(accessToken),
      },
    );
  ...
  async getRoute(routeId: string, accessToken?: string | null) {
    const response = await apiClient.get<ItemEnvelope<DeliveryRouteDetail>>(
  ...
  async createRoute(
    payload: CreateDeliveryRoutePayload,
    accessToken?: string | null,
  ) {
    const response = await apiClient.post<
      ItemEnvelope<DeliveryRouteDetail>,
  ...
  async assignOrders(
    routeId: string,
    payload: AssignDeliveryRouteOrdersPayload,
    accessToken?: string | null,
  ) {
    const response = await apiClient.post<
      ItemEnvelope<DeliveryRouteDetail>,
  ...
  async updateOrderStatus(
    orderId: string,
    payload: UpdateDeliveryOrderStatusPayload,
    accessToken?: string | null,
  ) {
    const response = await apiClient.patch<
      ItemEnvelope<DeliveryOrder>,
      UpdateDeliveryOrderStatusPayload
  ...
  async createOrderCollection(
    ...
    const response = await apiClient.post<
      ItemEnvelope<RouteCollectionResponse>,
      CreateRouteCollectionPayload
  ...
  async listSettlements(
    ...
    const response = await apiClient.get<ListEnvelope<RouteSettlementListItem>>(
      withParams("/route-settlements", filters),
      {
        headers: authHeaders(accessToken),
      },
    );
  ...
  async getSettlement(settlementId: string, accessToken?: string | null) {
    const response = await apiClient.get<ItemEnvelope<RouteSettlementDetail>>(
  ...
  async closeSettlement(
    settlementId: string,
    payload: CloseRouteSettlementPayload,
    accessToken?: string | null,
  ) {
    const response = await apiClient.post<
      ItemEnvelope<RouteSettlementDetail>,
