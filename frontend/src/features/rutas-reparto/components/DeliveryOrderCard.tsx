...
export function DeliveryOrderCard({
  ...
  const hasCollectibleBalance = Boolean(
    order.accountReceivableId && outstandingAmount > 0,
  );
  const evidenceForOrder = evidence.filter(
    (item) => (item.deliveryOrderId ?? item.orderId) === order.id,
  );
    ...
    <Card className="overflow-hidden p-0">
      <div className="grid gap-4 p-5 lg:grid-cols-[1fr_auto] lg:items-start">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="inline-flex items-center gap-1 rounded-full border border-[color:var(--erp-border)] bg-[var(--erp-surface)] px-3 py-1 text-xs font-black uppercase tracking-[0.12em] text-[var(--erp-muted-foreground)]">
          ...
          {order.notes && (
            <p className="mt-3 rounded-2xl border border-[rgba(214,155,45,0.28)] bg-[rgba(214,155,45,0.12)] p-3 text-sm font-semibold text-[var(--erp-brand-gold-deep)]">
              {order.notes}
            </p>
          )}
        </div>
      ...
      <dl className="grid gap-3 border-y border-[color:var(--erp-border)] bg-white/70 p-5 text-sm sm:grid-cols-2 lg:grid-cols-4">
        <div>
          ...
          <dd className="mt-1 text-[var(--erp-muted-foreground)]">
            {order.deliveredByUserName ?? shortId(order.deliveredByUserId)}
          </dd>
        </div>
        <div>
          <dt className="flex items-center gap-2 font-black text-[var(--erp-foreground)]">
            <Clock className="h-4 w-4 text-[var(--erp-brand-gold-deep)]" />
            Entrega
          </dt>
          <dd className="mt-1 text-[var(--erp-muted-foreground)]">
        ...
        <div>
          ...
          <dd className="mt-1 text-[var(--erp-muted-foreground)]">
            {order.collectedByUserName ?? shortId(order.collectedByUserId)}
          </dd>
        </div>
        <div>
          <dt className="flex items-center gap-2 font-black text-[var(--erp-foreground)]">
      ...
      <div className="grid gap-4 p-5 lg:grid-cols-[1fr_auto] lg:items-center">
        <div>
          ...
          <p className="mt-2 text-xs font-semibold text-[var(--erp-muted-foreground)]">
            Liquidación:{" "}
            {routeSettlementId
              ? `asociada ${shortId(routeSettlementId)}`
