      ...
      >
        ...
        <div className="mt-5 grid gap-3 border border-[#1d2420]/10 bg-[#f7f5ef] p-4 text-sm">
          <p>
            ...
            {order.accountReceivableId
              ? shortId(order.accountReceivableId)
              : "Sin cuenta por cobrar"}
          </p>
          <p>
            <strong>Saldo visible pendiente:</strong> {money(outstandingAmount)}
          </p>
          <p>
            <strong>Cobrado derivado de Payment:</strong>{" "}
            {money(order.derivedCollectedAmount)}
          </p>
          {forcedSecondPass && (
            <p className="border-l-4 border-[#2f6f73] bg-white p-3 font-black text-[#2f6f73]">
              Este flujo registra la cobranza como segunda vuelta.
            </p>
          )}
        </div>
        ...
        <div className="mt-5 grid gap-4 md:grid-cols-2">
          <Field
            label="Monto"
            hint="No puede exceder el saldo pendiente mostrado."
          >
            <TextInput
          ...
          <Field
            label="Vuelta de cobranza"
            hint={
              forcedSecondPass
                ? "Bloqueada para cumplir el flujo de segunda vuelta."
                : undefined
            }
          >
            {forcedSecondPass ? (
              <div className="border border-[#2f6f73]/25 bg-[#2f6f73]/8 px-3 py-3 text-sm font-black text-[#2f6f73]">
                Segunda vuelta
              </div>
            ) : (
              <SelectInput
          ...
          <Field
            label="Referencia"
            hint="Opcional: transferencia, depósito o nota de cobro."
          >
            <TextInput
        ...
        {numericAmount > outstandingAmount && (
          <div className="mt-4">
            <StatusMessage tone="error">
              El monto no puede superar el saldo pendiente visible.
            </StatusMessage>
          </div>
        )}
        {createCollection.error && (
          <div className="mt-4">
            <StatusMessage tone="error">
              No se pudo registrar el cobro. Revisa saldo, permisos y cuenta por
              cobrar.
            </StatusMessage>
          </div>
        )}
        ...
        <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:justify-end">
          ...
          <PrimaryButton
            ...
            type="submit"
          >
            {createCollection.isPending ? "Registrando..." : submitLabel}
          </PrimaryButton>
        </div>
      </form>
    </div>
