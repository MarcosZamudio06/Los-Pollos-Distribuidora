...
function MapClick({
  activeSaleId,
  onMoveStop,
}: Pick<Props, "activeSaleId" | "onMoveStop">) {
  useMapEvents({
    click: ({ latlng }) => {
      if (activeSaleId) onMoveStop(activeSaleId, latlng.lat, latlng.lng);
    },
  });
  return null;
}
...
function AnimatedRoute({ plan }: { plan: DeliveryRoutePlan }) {
  ...
  const totalDistance = useMemo(
    () => routeLengthMeters(plan.geometry),
    [plan.geometry],
  );
  ...
  return (
    <>
      <GeoJSON
        ref={routeLayer}
        data={plan.geometry}
        style={{ color: "#b62a22", weight: 6, opacity: 0.88 }}
      />
...
export function RoutePlannerMap({
  ...
  return (
    ...
    >
      ...
      <MapContainer
        center={originPoint ?? fallbackCenter}
      ...
      >
        ...
        {plan?.geometry && (
          <AnimatedRoute key={`${plan.id}:${geometryRevision}`} plan={plan} />
        )}
        {selectedSegment && (
          <GeoJSON
            data={selectedSegment}
            interactive={false}
            style={{ color: "#176b45", weight: 9, opacity: 0.95 }}
          />
        )}
        {originPoint && (
          <RouteStopInfoMarker
        ...
        {stops.map((stop, index) => {
          ...
          return (
            <RouteStopInfoMarker
              ...
              onMove={(lat, lon) => onMoveStop(stop.saleId, lat, lon)}
              onSelect={() => onSelectStop(stop.saleId)}
