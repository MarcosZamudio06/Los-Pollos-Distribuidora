    ...
    >
      ...
      <div className="relative grid min-w-0 gap-4 lg:grid-cols-[1fr_auto] lg:items-end lg:gap-5">
        <div className="min-w-0">
          ...
          >
            <span
              className="h-2 w-2 rounded-full bg-[var(--erp-brand-gold-soft)]"
              aria-hidden="true"
            />
            {eyebrow}
          </div>
...
export function Field({
  ...
  return (
    <label className="grid gap-2 text-xs font-black uppercase tracking-[0.14em] text-[var(--erp-muted-foreground)]">
      ...
      {children}
      {hint && (
        <span className="text-xs font-semibold normal-case leading-5 tracking-normal text-[var(--erp-muted-foreground)]">
          {hint}
        </span>
      )}
    </label>
...
export function StatusMessage({
  ...
  children: ReactNode;
  tone?: "error" | "info" | "empty" | "success";
}) {
  const className = {
    empty:
      "border-dashed border-[color:var(--erp-border)] bg-[var(--erp-surface)] text-[var(--erp-muted-foreground)]",
    error:
      "border-[rgba(157,45,36,0.20)] bg-[rgba(157,45,36,0.08)] text-[var(--erp-danger)]",
    info: "border-[rgba(47,111,115,0.20)] bg-[rgba(47,111,115,0.08)] text-[var(--erp-info)]",
    success:
      "border-[rgba(63,123,65,0.20)] bg-[rgba(63,123,65,0.08)] text-[var(--erp-success)]",
  }[tone];
  return (
    <p className={`rounded-2xl border p-4 text-sm font-bold ${className}`}>
      {children}
    </p>
  );
}
