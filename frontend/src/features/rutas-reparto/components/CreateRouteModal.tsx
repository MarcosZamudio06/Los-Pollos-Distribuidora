...
export function CreateRouteModal({ onClose, onCreated }: Props) {
  ...
  const validOrders = useMemo(
    () =>
      orders.filter(
        (order) => order.saleId.trim() && order.deliveryAddress.trim(),
      ),
    [orders],
  );
  const canSubmit = Boolean(
    name.trim() && driverId.trim() && scheduledDate && validOrders.length > 0,
  );

  function updateOrder(key: string, patch: Partial<DraftOrder>) {
    setOrders((current) =>
      current.map((order) =>
        order.key === key ? { ...order, ...patch } : order,
      ),
    );
  }
  ...
  function removeOrder(key: string) {
    setOrders((current) =>
      current.length === 1
        ? current
        : current.filter((order) => order.key !== key),
    );
  }
      ...
      >
        <div className="flex flex-col gap-3 border-b border-[#1d2420]/10 pb-5 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            >
              Ruta con pedidos confirmados
            </h2>
            <p className="mt-2 max-w-2xl text-sm leading-6 text-[#6f786f]">
              La API valida que las ventas estén confirmadas y que no estén
              canceladas. No se captura liquidación aquí.
            </p>
          </div>
        ...
        <div className="mt-5 grid gap-4 md:grid-cols-2">
          ...
          <Field
            label="Repartidor"
            hint="Usa el identificador del usuario DRIVER asignado."
          >
            <TextInput
          ...
          <Field
            ...
            hint="Opcional cuando la operación la define."
          >
            <TextInput
              onChange={(event) => setOriginLocationId(event.target.value)}
              placeholder="originLocationId"
              value={originLocationId}
            />
          </Field>
          <Field
            label="Ubicación ROUTE_STOCK"
            hint="Opcional si backend la autogenera."
          >
            <TextInput
              onChange={(event) => setRouteStockLocationId(event.target.value)}
              placeholder="routeStockLocationId"
              value={routeStockLocationId}
            />
          </Field>
        </div>
        ...
        <section className="mt-6 border border-[#1d2420]/10 bg-[#f7f5ef] p-4">
          <div className="flex items-center justify-between gap-4">
            ...
            <SecondaryButton
              onClick={() => setOrders((current) => [...current, blankOrder()])}
            >
              Agregar pedido
            </SecondaryButton>
          </div>
          <div className="mt-4 grid gap-3">
            {orders.map((order) => (
              <div
                className="grid gap-3 border border-[#1d2420]/10 bg-white p-3 lg:grid-cols-[1fr_1fr_2fr_auto]"
                key={order.key}
              >
                <Field label="Venta confirmada">
                  <TextInput
                    onChange={(event) =>
                      updateOrder(order.key, { saleId: event.target.value })
                    }
                    placeholder="saleId"
                    required
                    value={order.saleId}
                  ...
                  <TextInput
                    onChange={(event) =>
                      updateOrder(order.key, {
                        accountReceivableId: event.target.value,
                      })
                    }
                    placeholder="accountReceivableId"
                    value={order.accountReceivableId}
                  />
                </Field>
        ...
        {createRoute.error && (
          <div className="mt-4">
            <StatusMessage tone="error">
              No se pudo crear la ruta. Revisa permisos, ventas confirmadas y
        ...
        <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:justify-end">
          ...
          <PrimaryButton
            ...
            type="submit"
          >
            {createRoute.isPending ? "Creando ruta..." : "Crear ruta"}
          </PrimaryButton>
        </div>
      </form>
    </div>
