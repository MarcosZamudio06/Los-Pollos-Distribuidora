...
const statuses: DeliveryOrderStatus[] = [
  "PENDING",
  "IN_ROUTE",
  "DELIVERED",
  "NOT_DELIVERED",
...
export function UpdateDeliveryStatusDialog({ onClose, order, routeId }: Props) {
  const [status, setStatus] = useState<DeliveryOrderStatus>(
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    await updateStatus.mutateAsync({
      ...
      payload: {
        deliveredAt:
          status === "DELIVERED" ? toIsoDateTime(deliveredAt) : undefined,
    ...
    >
      ...
      >
        ...
        <div className="mt-5 grid gap-4 md:grid-cols-2">
          ...
          <Field
            label={requiresNote ? "Motivo o nota obligatoria" : "Notas"}
            hint={
              requiresNote
                ? "Requerido para no entrega, rechazo parcial o devolución."
                : "Opcional."
            }
          >
            <TextInput
