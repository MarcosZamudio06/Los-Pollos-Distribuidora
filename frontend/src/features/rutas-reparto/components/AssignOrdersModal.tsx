...
export function AssignOrdersModal({ onClose, routeId, routeName }: Props) {
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    await assignOrders.mutateAsync({
      orders: [
        {
          accountReceivableId: accountReceivableId.trim() || undefined,
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="assign-orders-title"
    >
      <form
      ...
      >
        <div className="flex items-start justify-between gap-4 border-b border-[#1d2420]/10 pb-5">
          <div>
            ...
            <h2
              ...
              id="assign-orders-title"
            >
              Pedido confirmado a ruta existente
            </h2>
            <p className="mt-2 text-sm leading-6 text-[#6f786f]">
              Ruta: <strong>{routeName}</strong>. Backend rechaza ventas
              canceladas, duplicadas, rutas cerradas o rutas con liquidación
              abierta.
            </p>
          </div>
        ...
        <div className="mt-5 grid gap-4 md:grid-cols-2">
          ...
          <Field label="Cuenta por cobrar">
            <TextInput
              onChange={(event) => setAccountReceivableId(event.target.value)}
        ...
        {assignOrders.error && (
          <div className="mt-4">
            <StatusMessage tone="error">
              No se pudo asignar el pedido. Confirma venta, ruta sin liquidación
              y permisos ADMIN.
            </StatusMessage>
          </div>
        )}
        <div className="mt-6 flex justify-end gap-3">
          ...
          <PrimaryButton
            disabled={!canSubmit || assignOrders.isPending}
            type="submit"
          >
            {assignOrders.isPending ? "Asignando..." : "Asignar pedido"}
          </PrimaryButton>
        </div>
      </form>
    </div>
