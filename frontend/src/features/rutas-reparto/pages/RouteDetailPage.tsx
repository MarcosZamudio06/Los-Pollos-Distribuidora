  ...
  return (
    <PageShell>
      <PageFrame>
        ...
        {!route.isLoading && !route.error && !detail && (
          ...
          <>
                ...
                {detail.routeSettlementId ? (
                  <div className="mt-3 grid gap-4">
                    ...
                    <p className="text-sm leading-6 text-[var(--erp-muted-foreground)]">
                      La ruta ya reporta una liquidación. No se captura
                  ...
                  <div className="mt-3 grid gap-4">
                    ...
                    <p className="text-sm leading-6 text-[var(--erp-muted-foreground)]">
                      Puedes abrirla cuando la ruta sea elegible; backend valida
            ...
            <Card className="grid gap-4 p-5 md:grid-cols-4">
              <div>
                ...
                <p className="mt-2 text-2xl font-black tabular-nums">
                  {money(collections?.expectedAmount)}
                </p>
              </div>
              <div>
                <p className="flex items-center gap-2 text-xs font-black uppercase tracking-[0.16em] text-[var(--erp-muted-foreground)]">
                ...
                <p className="mt-2 text-2xl font-black tabular-nums">
                  {money(collections?.derivedCollectedAmount)}
                </p>
              </div>
              <div>
                ...
                <p className="mt-2 text-2xl font-black tabular-nums">
                  {money(collections?.firstPassAmount)}
                </p>
              </div>
              <div>
                <p className="text-xs font-black uppercase tracking-[0.16em] text-[var(--erp-muted-foreground)]">
            ...
            <Card className="overflow-hidden p-0">
              ...
              {(detail.orders ?? []).length === 0 ? (
                <div className="p-5">
                  <StatusMessage tone="empty">
                    La ruta no muestra pedidos asignados.
                  </StatusMessage>
                </div>
              ) : (
                <>
                            ...
                            >
                              ...
                              <td className="border-b border-[color:var(--erp-border)] px-4 py-4">
                                {order.deliveredByUserName ??
                                  shortId(order.deliveredByUserId)}
                                <span className="block text-xs text-[var(--erp-muted-foreground)]">
                                  {dateTime(order.deliveredAt)}
                                </span>
                              </td>
                              <td className="border-b border-[color:var(--erp-border)] px-4 py-4">
                                {order.collectedByUserName ??
                                  shortId(order.collectedByUserId)}
                              </td>
                              <td className="border-b border-[color:var(--erp-border)] px-4 py-4">
                                {order.collectionPass ?? "Sin cobro"}
                              </td>
                            </tr>
                          ))}
                        </tbody>
