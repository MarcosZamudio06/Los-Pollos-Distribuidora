    ...
    <PageShell>
      <PageFrame>
        <RouteHero
          action={
            <SecondaryLink
              to={routeId ? `/delivery-routes/${routeId}` : "/delivery-routes"}
            >
              Volver al detalle
            </SecondaryLink>
          }
        ...
        {route.isLoading && (
          <StatusMessage>Cargando evidencia...</StatusMessage>
        )}
        {route.error && (
          <StatusMessage tone="error">
            No se pudo cargar la evidencia de la ruta.
          </StatusMessage>
        )}
          ...
          <section className="grid gap-5 lg:grid-cols-[0.85fr_1.15fr]">
            <Card className="p-5">
              ...
              <p className="mt-3 text-sm leading-6 text-[var(--erp-muted-foreground)]">
                La evidencia forma parte del MVP. Esta vista es de revisión
                administrativa; no asume operación offline ni obliga una mezcla
                final de evidencia.
              </p>
              <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-1">
                <div className="rounded-2xl border border-[color:var(--erp-border)] bg-[var(--erp-surface)] p-4">
                  <p className="flex items-center gap-2 text-xs font-black uppercase tracking-[0.16em] text-[var(--erp-muted-foreground)]">
                    <Images className="h-4 w-4 text-[var(--erp-info)]" />
                    Evidencias
                  </p>
                  <p className="mt-2 text-2xl font-black">{evidence.length}</p>
                </div>
                <div className="rounded-2xl border border-[color:var(--erp-border)] bg-[var(--erp-surface)] p-4">
                  <p className="flex items-center gap-2 text-xs font-black uppercase tracking-[0.16em] text-[var(--erp-muted-foreground)]">
                  ...
                  <p className="mt-2 text-2xl font-black">
                    {detail.orders?.length ?? detail.ordersCount ?? 0}
                  </p>
                </div>
              </div>
            </Card>
            ...
            <Card className="overflow-hidden p-0">
              <div className="border-b border-[color:var(--erp-border)] bg-white/70 p-5">
                ...
                <h2 className="mt-1 text-xl font-black tracking-[-0.04em]">
                  Bitácora de prueba de entrega
                </h2>
              </div>
              {evidence.length === 0 ? (
                <div className="p-5">
                  <StatusMessage tone="empty">
                    Esta ruta aún no reporta evidencia capturada.
                  </StatusMessage>
                </div>
              ) : (
                <div className="grid gap-3 p-5">
                  {evidence.map((item, index) => (
                    <article
                      className="grid gap-4 rounded-2xl border border-[color:var(--erp-border)] bg-white p-4 md:grid-cols-[0.7fr_1fr_1fr]"
                      key={`${item.deliveryOrderId ?? item.orderId ?? index}-${item.type}`}
                    >
                      <div>
                        <p className="flex items-center gap-2 text-xs font-black uppercase tracking-[0.16em] text-[var(--erp-muted-foreground)]">
                          <Camera className="h-4 w-4 text-[var(--erp-info)]" />
                          Tipo
                        </p>
                      ...
                      <div>
                        <p className="flex items-center gap-2 text-xs font-black uppercase tracking-[0.16em] text-[var(--erp-muted-foreground)]">
                          <FileText className="h-4 w-4 text-[var(--erp-brand-gold-deep)]" />
                          Pedido
                        </p>
                        <p className="mt-1 font-bold">
                          {item.saleNumber ??
                            shortId(item.deliveryOrderId ?? item.orderId)}
                        </p>
                      </div>
                      <div>
                        <p className="flex items-center gap-2 text-xs font-black uppercase tracking-[0.16em] text-[var(--erp-muted-foreground)]">
                          <CalendarDays className="h-4 w-4 text-[var(--erp-danger)]" />
                          Captura
                        </p>
                        <p className="mt-1 font-bold">
                  ...
                  <div className="grid gap-3 p-5 md:hidden">
                    {(detail.orders ?? []).map((order) => (
                      ...
                      >
                        ...
                        <p className="mt-3 text-xs font-semibold text-[var(--erp-muted-foreground)]">
                          Entregado: {dateTime(order.deliveredAt)}
                        </p>
                      </article>
                    ))}
                  </div>
