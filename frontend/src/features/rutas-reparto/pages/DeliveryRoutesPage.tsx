...
export function DeliveryRoutesPage() {
  ...
  const [filters, setFilters] = useState({
    ...
    originLocationId: "",
    scheduledDate: "",
    status: "" as DeliveryRouteStatus | "",
  });
  const [routeToAssign, setRouteToAssign] =
    useState<DeliveryRouteListItem | null>(null);
  ...
  const totalOrders = items.reduce(
    (sum, route) => sum + Number(route.ordersCount ?? 0),
    0,
  );
  ...
  return (
    <PageShell>
      <PageFrame>
        <RouteHero
          action={
            <PrimaryButton onClick={() => navigate("/delivery-routes/new")}>
        ...
        <section className="grid gap-4 md:grid-cols-3">
          <Card className="p-5">
            <p className="flex items-center gap-2 text-xs font-black uppercase tracking-[0.16em] text-[var(--erp-muted-foreground)]">
              <Route className="h-4 w-4 text-[var(--erp-info)]" />
              Rutas visibles
            </p>
            <p className="mt-3 text-2xl font-black tracking-[-0.05em] tabular-nums">
              {items.length}
            </p>
          ...
          <Card className="p-5">
            <p className="flex items-center gap-2 text-xs font-black uppercase tracking-[0.16em] text-[var(--erp-muted-foreground)]">
              <Truck className="h-4 w-4 text-[var(--erp-success)]" />
              En operación
            </p>
            <p className="mt-3 text-2xl font-black tracking-[-0.05em] tabular-nums">
              {inProgressCount}
        ...
        <Card className="p-5">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <p className="mt-2 text-sm leading-6 text-[var(--erp-muted-foreground)]">
                Filtra repartidor, estado, fecha y origen sin alterar la
                consulta existente.
              </p>
            </div>
            <SecondaryButton
          ...
          <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
            <Field label="Repartidor">
              <SelectInput
                disabled={
                  catalog.drivers.isLoading || Boolean(catalog.drivers.error)
                }
                onChange={(event) =>
                  setFilters({ ...filters, driverId: event.target.value })
                }
            ...
            <Field label="Estado">
              <SelectInput
                onChange={(event) =>
                  setFilters({
                    ...filters,
                    status: event.target.value as DeliveryRouteStatus | "",
                  })
                }
            ...
            <Field label="Fecha programada">
              <TextInput
                onChange={(event) =>
                  setFilters({ ...filters, scheduledDate: event.target.value })
                }
                type="date"
            ...
            <Field label="Ubicación origen">
              <SelectInput
                disabled={
                  catalog.locations.isLoading ||
                  Boolean(catalog.locations.error)
                }
                onChange={(event) =>
                  setFilters({
                    ...filters,
        ...
        <Card className="overflow-hidden p-0">
          <div className="flex flex-col gap-3 border-b border-[color:var(--erp-border)] bg-white/70 p-5 sm:flex-row sm:items-center sm:justify-between">
            ...
            <span className="rounded-full border border-[color:var(--erp-border)] bg-[var(--erp-surface)] px-3 py-1 text-xs font-black uppercase tracking-[0.14em] text-[var(--erp-muted-foreground)]">
          ...
          <div className="p-5">
            {routes.isLoading && (
              <StatusMessage>Cargando rutas de reparto...</StatusMessage>
            )}
            {routes.error && (
              <StatusMessage tone="error">
                No se pudieron cargar las rutas. Revisa sesión, permisos o
                disponibilidad del backend.
              </StatusMessage>
            )}
            {!routes.isLoading && !routes.error && items.length === 0 && (
              <StatusMessage tone="empty">
                No hay rutas para los filtros seleccionados. Crea una ruta con
                ventas confirmadas para iniciar operación.
              </StatusMessage>
            )}
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                          ...
                          <p className="mt-1 text-sm text-[var(--erp-muted-foreground)]">
                            {route.driverName ?? shortId(route.driverId)}
                          </p>
                        </div>
                      ...
                      <div className="mt-4 grid grid-cols-2 gap-3 rounded-xl bg-[var(--erp-surface)] p-3 text-sm">
                        <span>
                          <strong className="block text-lg text-[var(--erp-foreground)]">
                            {route.ordersCount ?? 0}
                          </strong>
                          pedidos
                        </span>
                        <span>
                          <strong className="block text-lg text-[var(--erp-foreground)]">
                            {route.pendingOrdersCount ?? 0}
                          </strong>
                          pendientes
                        </span>
                      </div>
                      {route.optimizationStatus === "OPTIMIZED" && (
                        <p className="mt-3 rounded-xl border border-[rgba(214,155,45,.28)] bg-[rgba(214,155,45,.09)] px-3 py-2 text-xs font-bold text-[var(--erp-brand-gold-deep)]">
                          Mapa disponible ·{" "}
                      ...
                      <div className="mt-4 flex flex-wrap gap-2">
                        <SecondaryLink to={`/delivery-routes/${route.id}`}>
                          Detalle
                        </SecondaryLink>
                        <SecondaryLink
                          to={`/delivery-routes/${route.id}/evidence`}
                        >
                          Evidencia
                        </SecondaryLink>
                        {!route.routeSettlementId &&
                          !["COMPLETED", "CANCELLED"].includes(route.status) &&
                          (route.optimizationStatus === "OPTIMIZED" ? (
                            <SecondaryLink
                              to={`/delivery-routes/${route.id}/reoptimize`}
                            >
                              Agregar y reoptimizar
                            </SecondaryLink>
                          ) : (
                            <SecondaryButton
                              onClick={() => setRouteToAssign(route)}
                            >
                              Asignar pedido
                            </SecondaryButton>
                          ))}
                        {!route.routeSettlementId &&
                          route.status !== "CANCELLED" && (
                            <SecondaryButton
                              ...
                              onClick={() =>
                                void handleOpenSettlement(route.id)
                              }
                            >
                              Abrir liquidación
                            </SecondaryButton>
                  ...
                  <table className="min-w-[1120px] border-separate border-spacing-0 text-left text-sm">
                    <thead className="text-xs uppercase tracking-[0.14em] text-[var(--erp-muted-foreground)]">
                      <tr>
                        ...
                        <th className="border-b border-[color:var(--erp-border)] bg-[var(--erp-surface)] px-4 py-3 text-right">
                          Acciones
                        </th>
                      </tr>
                    </thead>
                    <tbody>
                      {items.map((route) => (
                        <tr
                          ...
                          key={route.id}
                        >
                          <td className="border-b border-[color:var(--erp-border)] px-4 py-4 font-black">
                            {route.name}
                          </td>
                          <td className="border-b border-[color:var(--erp-border)] px-4 py-4">
                            {route.driverName ?? shortId(route.driverId)}
                          </td>
                          <td className="border-b border-[color:var(--erp-border)] px-4 py-4 text-[var(--erp-muted-foreground)]">
                            {date(route.scheduledDate)}
                          </td>
                          <td className="border-b border-[color:var(--erp-border)] px-4 py-4">
                            {route.originLocationName ??
                          ...
                          <td className="border-b border-[color:var(--erp-border)] px-4 py-4">
                            {route.optimizationStatus === "OPTIMIZED" ? (
                              <span className="font-bold text-[var(--erp-brand-gold-deep)]">
                                Mapa ·{" "}
                                {route.distanceMeters == null
                                  ? "—"
                                  : `${(route.distanceMeters / 1000).toFixed(1)} km`}
                          ...
                          <td className="border-b border-[color:var(--erp-border)] px-4 py-4">
                            ...
                            <span className="block text-xs text-[var(--erp-muted-foreground)]">
                              {route.pendingOrdersCount ?? 0} pendientes
                            </span>
                          </td>
                          <td className="border-b border-[color:var(--erp-border)] px-4 py-4">
                            {route.routeSettlementId ? (
                              <SecondaryLink
                                to={`/route-settlements/${route.routeSettlementId}`}
                              >
                                {shortId(route.routeSettlementId)}
                              </SecondaryLink>
                            ) : (
                              <span className="text-[var(--erp-muted-foreground)]">
                                Sin liquidación
