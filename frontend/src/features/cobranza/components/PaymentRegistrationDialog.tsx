...
export function PaymentRegistrationDialog({
  ...
  return (
    <div className="fixed inset-0 z-40 grid place-items-center overflow-y-auto bg-[rgba(17,24,21,0.58)] p-4 backdrop-blur-sm">
      <form
        className="w-full max-w-2xl overflow-hidden rounded-[1.6rem] border border-[color:var(--erp-border)] bg-white shadow-2xl"
        onSubmit={(event) => void handleSubmit(event)}
      >
        <div className="border-b border-[color:var(--erp-border)] bg-[var(--erp-surface)] p-6">
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-[0.22em] text-[var(--erp-info)]">
                <CircleDollarSign className="h-4 w-4" /> Registro de pago
              </p>
              ...
              <p className="mt-2 text-sm text-[var(--erp-muted-foreground)]">
                Saldo pendiente:{" "}
                <strong className="text-[var(--erp-danger)]">
        ...
        <div className="p-6">
          {cannotPay && (
            <p
              role="alert"
              className="mb-4 rounded-2xl bg-[rgba(157,45,36,0.10)] p-4 text-sm font-semibold text-[var(--erp-danger)]"
            >
              No se pueden registrar pagos sobre cuentas pagadas o canceladas.
            </p>
          )}
          {cashSessionError && (
            <p
              ...
              className="mb-4 rounded-2xl bg-[rgba(157,45,36,0.10)] p-4 text-sm font-semibold text-[var(--erp-danger)]"
            >
              Abre un turno en la terminal registrada antes de recibir efectivo.{" "}
              <Link className="underline" to="/daily-close">
                Abrir turno
              </Link>
            </p>
          )}
          ...
          {amountError && (
            <p
              role="alert"
              className="mt-4 text-sm font-semibold text-[var(--erp-danger)]"
            >
              {amountError}
            </p>
          )}
          {registerPayment.error && (
            <p
              ...
              className="mt-4 text-sm font-semibold text-[var(--erp-danger)]"
            >
              {registerPayment.error instanceof Error
                ? registerPayment.error.message
                : "No se pudo registrar el pago."}
            </p>
          )}
          <div className="mt-6 flex flex-wrap justify-end gap-3">
            <button
              className="inline-flex h-11 items-center justify-center rounded-xl border border-[color:var(--erp-border)] bg-white px-5 text-sm font-semibold transition hover:border-[var(--erp-brand-gold)] hover:bg-[var(--erp-surface-muted)]"
              onClick={onClose}
              type="button"
            >
              Cancelar
            ...
            <button
              className="inline-flex h-11 items-center justify-center rounded-xl bg-[var(--erp-charcoal)] px-5 text-sm font-black text-white transition hover:bg-[var(--erp-graphite)] disabled:opacity-50"
              disabled={
                Boolean(amountError) ||
                cannotPay ||
                cashSessionError ||
      ...
      <ConfirmationDialog
        ...
        description="Verifique el pago antes de aplicarlo a la cuenta por cobrar."
      ...
      >
        <p>
          <strong>Cliente:</strong> {account.customerName ?? account.customerId}
        </p>
        <p>
