...
export function CreditBlockedCustomerBadge({
  creditStatus,
  daysOverdue = 0,
  outstandingAmount,
}: CreditBlockedCustomerBadgeProps) {
    ...
    : normalizedStatus === "SUSPENDED"
      ...
      : normalizedStatus === "BLOCKED" && (daysOverdue ?? 0) > 0
        ...
        : normalizedStatus === "BLOCKED" && balance > 0
          ...
          : "Crédito activo";
  const blocked =
    label !== "Crédito activo" && label !== "Estado de crédito no informado";

  return (
    <span
      className={`inline-flex rounded-full px-3 py-1 text-xs font-black uppercase tracking-[0.14em] ${blocked ? "bg-[#d43f2f]/10 text-[#9d2d24]" : "bg-[#39798b]/10 text-[#39798b]"}`}
    >
      {label}
    </span>
  );
