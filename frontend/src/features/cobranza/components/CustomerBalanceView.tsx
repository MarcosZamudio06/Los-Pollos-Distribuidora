        ...
        <div className="mt-6 grid gap-5">
          <header className="rounded-[1.4rem] bg-[var(--erp-charcoal)] p-5 text-white shadow-[var(--erp-shadow-elevated)]">
            <p className="text-xs font-bold uppercase tracking-[0.22em] text-[var(--erp-brand-gold-soft)]">
              {account.saleNumber ?? account.saleId ?? "Cuenta por cobrar"}
            </p>
            ...
            <p className="mt-2 text-sm text-white/70">
              Saldo pendiente {formatMoney(account.outstandingAmount)} · Estado{" "}
              {account.status}
            </p>
          </header>
          <section className="rounded-[1.4rem] border border-[color:var(--erp-border)] bg-white p-5 text-sm shadow-[var(--erp-shadow)]">
            <div className="mb-4 flex flex-wrap gap-2">
              ...
              <CreditBlockedCustomerBadge
                creditStatus={detail.data?.customer?.creditStatus}
            ...
            <div className="grid gap-3">
              <p className="flex justify-between gap-4">
                ...
                <strong>{formatMoney(account.originalAmount)}</strong>
              </p>
              <p className="flex justify-between gap-4">
                <span className="text-[var(--erp-muted-foreground)]">
                  Saldo final
              ...
              <p className="flex justify-between gap-4">
                ...
                <strong>{formatDate(account.saleDate)}</strong>
              </p>
              <p className="flex justify-between gap-4">
                <span className="text-[var(--erp-muted-foreground)]">
              ...
              <p className="flex justify-between gap-4">
                ...
                <strong>{account.paymentTermsDays ?? "—"}</strong>
              </p>
              <p className="flex justify-between gap-4">
                <span className="text-[var(--erp-muted-foreground)]">
                  Días de atraso
                </span>
                <strong>{account.daysOverdue ?? 0}</strong>
              </p>
              <p className="flex justify-between gap-4">
                <span className="text-[var(--erp-muted-foreground)]">
                  Último pago
                </span>
                <strong>{formatDate(account.lastPaymentDate)}</strong>
              </p>
              <p className="flex justify-between gap-4">
                <span className="text-[var(--erp-muted-foreground)]">
                  Ubicación operativa
                </span>
                <strong>{detail.data?.sale?.locationId ?? "—"}</strong>
              </p>
              <p className="flex justify-between gap-4">
                <span className="text-[var(--erp-muted-foreground)]">
                  Tipo documental
                </span>
                <strong>{detail.data?.sale?.documentType ?? "—"}</strong>
              </p>
            </div>
          </section>
          <section className="rounded-[1.4rem] border border-[color:var(--erp-border)] bg-white p-5 text-sm shadow-[var(--erp-shadow)]">
            <div className="flex items-center justify-between gap-4">
              ...
              <ReceiptText className="h-5 w-5 text-[var(--erp-muted-foreground)]" />
            </div>
            {!detail.data?.payments?.length && (
              <p className="mt-3 rounded-2xl bg-[var(--erp-surface)] p-3 text-[var(--erp-muted-foreground)]">
              ...
              {detail.data?.payments?.map((payment) => (
                ...
                >
                  ...
                  <p className="text-[var(--erp-muted-foreground)]">
                    CxC {payment.accountReceivableId} · Banco{" "}
                    {payment.bankName ?? "—"} · Ref{" "}
