    ...
    <main className="min-h-screen bg-[var(--erp-background)] px-4 py-6 text-[var(--erp-foreground)] sm:px-6 lg:px-8">
      <section className="mx-auto grid max-w-7xl gap-6">
        ...
        <section className="rounded-[1.4rem] border border-[color:var(--erp-border)] bg-white p-4 shadow-[var(--erp-shadow)]">
          ...
          <div className="grid gap-3 md:grid-cols-8"><label className="relative"><Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[var(--erp-muted-foreground)]" /><input className={`${fieldClass} w-full pl-9`} placeholder="Cliente ID" value={filters.customerId ?? ''} onChange={(event) => setFilters({ ...filters, customerId: event.target.value })} /></label><input className={fieldClass} placeholder="Venta ID" value={filters.saleId ?? ''} onChange={(event) => setFilters({ ...filters, saleId: event.target.value })} /><input className={fieldClass} placeholder="Solicitud administrativa" value={filters.billingRequestId ?? ''} onChange={(event) => setFilters({ ...filters, billingRequestId: event.target.value })} /><select className={fieldClass} value={filters.status ?? ''} onChange={(event) => setFilters({ ...filters, status: event.target.value as CollectionStatus | '' })}><option value="">Estado</option><option value="UNPAID">No pagada</option><option value="PARTIALLY_PAID">Parcial</option><option value="PAID">Pagada</option><option value="CANCELLED">Cancelada</option></select><select className={fieldClass} value={filters.agingStatus ?? ''} onChange={(event) => setFilters({ ...filters, agingStatus: event.target.value as AgingStatus | '' })}><option value="">Envejecimiento</option><option value="CURRENT">Vigente</option><option value="DUE_SOON">Por vencer</option><option value="OVERDUE">Vencida</option></select><input className={fieldClass} type="date" value={filters.dueDateFrom ?? ''} onChange={(event) => setFilters({ ...filters, dueDateFrom: event.target.value })} /><input className={fieldClass} type="date" value={filters.dueDateTo ?? ''} onChange={(event) => setFilters({ ...filters, dueDateTo: event.target.value })} /><div className="flex flex-col justify-center gap-2 rounded-xl border border-[color:var(--erp-border)] bg-[var(--erp-surface)] px-3 py-2 text-xs font-semibold text-[var(--erp-muted-foreground)]"><label className="inline-flex items-center gap-2"><input checked={Boolean(filters.onlyOverdue)} type="checkbox" onChange={(event) => setFilters({ ...filters, onlyOverdue: event.target.checked, onlyUpcoming: false })} /> Solo vencidas</label><label className="inline-flex items-center gap-2"><input checked={Boolean(filters.onlyUpcoming)} type="checkbox" onChange={(event) => setFilters({ ...filters, onlyUpcoming: event.target.checked, onlyOverdue: false })} /> Solo por vencer</label></div></div>
        </section>
