...
export const accountsReceivableService = {
  async list(filters: AccountsReceivableFilters, accessToken?: string | null) {
    const response = await apiClient.get<ListEnvelope<AccountReceivable>>(
      withParams("/accounts-receivable", filters),
      {
  ...
  async get(id: string, accessToken?: string | null) {
    const response = await apiClient.get<ItemEnvelope<AccountReceivableDetail>>(
  ...
  async registerPayment(
    ...
    const response = await apiClient.post<
      ItemEnvelope<RegisterPaymentResponse>,
      ReturnType<typeof toPaymentPayload>
