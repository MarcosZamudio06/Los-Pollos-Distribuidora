...
type AccessUser = {
  ...
  isActive: boolean
  roleId: string
  role: { id: string; name: string; description?: string | null; version: number }
...
type UserAccess = {
  ...
  permissions: string[]
  activeSessionCount: number
  sessions: Array<{ id: string; createdAt: string; lastUsedAt: string; absoluteExpiresAt: string }>
}
type AuditLog = {
  id: string
  action: string
...
type AuditData = { items: AuditLog[]; total: number; page: number; limit: number }
...
export function AccessProfilesPage() {
  ...
  const [audit, setAudit] = useState<AuditData>({ items: [], total: 0, page: 1, limit: 25 })
  ...
  const originalKeys = useMemo(() => new Set(selectedProfile?.permissions.map((permission) => permission.key) ?? []), [selectedProfile])
  ...
  const canManageProfiles = hasPermission(user, PERMISSIONS.accessProfilesManage)

  async function load() {
    setLoading(true)
    setError('')
    try {
      const requests: [Promise<{ data: AccessProfile[] }>, Promise<{ data: Permission[] }>, Promise<{ data: AuditData }>] = [
        apiClient.get<Envelope<AccessProfile[]>>('/roles', { headers }),
        apiClient.get<Envelope<Permission[]>>('/permissions', { headers }),
        apiClient.get<Envelope<AuditData>>('/access-control/audit-logs?limit=25', { headers }),
      ]
      const [profileResponse, permissionResponse, auditResponse] = await Promise.all(requests)
      setProfiles(profileResponse.data)
      setPermissions(permissionResponse.data)
      setAudit(auditResponse.data)
      const initialProfile = profileResponse.data.find((profile) => profile.id === selectedId) ?? profileResponse.data[0]
      if (initialProfile) {
        setSelectedId(initialProfile.id)
        setDraftKeys(initialProfile.permissions.map((permission) => permission.key))
      }
      if (userId) {
        const accessResponse = await apiClient.get<Envelope<UserAccess>>(`/users/${userId}/access`, { headers })
      ...
      setError(caught instanceof Error ? caught.message : 'No se pudo cargar el control de acceso.')
    } finally {
      setLoading(false)
    }
  }
  ...
  async function savePermissions() {
    ...
    setNotice('')
    try {
      const response = await apiClient.patch<Envelope<{ role: AccessProfile; currentSessionRevoked: boolean }>, { permissionKeys: string[]; expectedVersion: number; reason: string }>(`/roles/${selectedProfile.id}/permissions`, {
        body: { permissionKeys: draftKeys, expectedVersion: selectedProfile.version, reason: reason.trim() },
        headers,
      })
      setNotice(`Perfil actualizado. ${response.data.role.userCount} empleados afectados y sus sesiones activas fueron cerradas.`)
      ...
      setError(caught instanceof Error ? caught.message : 'No se pudo actualizar el perfil.')
    } finally {
      setSaving(false)
    }
  }
  ...
  async function updateUserProfile() {
    ...
    setNotice('')
    try {
      const response = await apiClient.patch<Envelope<{ access: UserAccess; currentSessionRevoked: boolean }>, { roleId: string; expectedRoleId: string; reason: string }>(`/users/${access.user.id}/access-profile`, {
        body: { roleId: nextUserRoleId, expectedRoleId: access.user.roleId, reason: profileReason.trim() },
        headers,
      })
      ...
      setError(caught instanceof Error ? caught.message : 'No se pudo reasignar el perfil.')
    } finally {
      setSaving(false)
    }
  }
  ...
  async function revokeSessions() {
    ...
    setNotice('')
    try {
      const response = await apiClient.post<Envelope<{ currentSessionRevoked: boolean }>, { reason: string }>(`/users/${access.user.id}/sessions/revoke`, {
      ...
      setError(caught instanceof Error ? caught.message : 'No se pudieron revocar las sesiones.')
    } finally {
      setSaving(false)
    }
  }
      ...
      <div className="mx-auto grid max-w-[1480px] gap-6">
        ...
        <section className="grid gap-6 xl:grid-cols-[18rem_minmax(0,1fr)_20rem]">
          ...
          <aside className="grid content-start gap-6">
            {access ? <section className="rounded-[1.4rem] border border-[color:var(--erp-border)] bg-white p-5 shadow-[var(--erp-shadow)]"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[.16em] text-[var(--erp-brand-red)]">Empleado seleccionado</p><h2 className="mt-2 text-xl font-black">{access.user.name}</h2><p className="mt-1 text-xs text-[var(--erp-muted-foreground)]">{access.user.controlNumber} · {access.user.operationalLocation.name}</p></div><Link aria-label="Cerrar empleado seleccionado" className="rounded-lg p-2 text-[var(--erp-muted-foreground)] hover:bg-[var(--erp-surface-muted)]" to="/admin/access-profiles"><X className="h-4 w-4" /></Link></div><div className="mt-5 grid gap-3"><label className="grid gap-1.5 text-sm font-bold">Nuevo perfil<Select disabled={!canManageProfiles || saving} onChange={(event) => setNextUserRoleId(event.target.value)} value={nextUserRoleId}>{profiles.map((profile) => <option key={profile.id} value={profile.id}>{profile.name}</option>)}</Select></label><label className="grid gap-1.5 text-sm font-bold">Motivo del cambio<Input disabled={!canManageProfiles || saving} placeholder="Cambio de responsabilidad" value={profileReason} onChange={(event) => setProfileReason(event.target.value)} /></label><Button disabled={!canManageProfiles || saving || !profileReason.trim() || nextUserRoleId === access.user.roleId} onClick={() => void updateUserProfile()} variant="secondary"><Users className="h-4 w-4" /> Reasignar perfil</Button></div><div className="mt-5 border-t border-[color:var(--erp-border)] pt-4"><p className="text-xs font-black uppercase tracking-[.14em] text-[var(--erp-muted-foreground)]">Sesiones activas: {access.activeSessionCount}</p><label className="mt-3 grid gap-1.5 text-sm font-bold">Motivo de cierre<Input disabled={!canManageProfiles || saving} placeholder="Revocación preventiva" value={sessionReason} onChange={(event) => setSessionReason(event.target.value)} /></label><Button className="mt-3 w-full" disabled={!hasPermission(user, PERMISSIONS.userSessionsRevoke) || saving || !sessionReason.trim()} onClick={() => void revokeSessions()} variant="outline"><KeyRound className="h-4 w-4" /> Cerrar sesiones</Button></div></section> : <section className="rounded-[1.4rem] border border-dashed border-[color:var(--erp-border)] bg-white p-5 shadow-[var(--erp-shadow)]"><Users className="h-5 w-5 text-[var(--erp-brand-gold-deep)]" /><h2 className="mt-3 font-black">Impacto por empleado</h2><p className="mt-1 text-sm leading-6 text-[var(--erp-muted-foreground)]">Abre un empleado desde la administración de personal para revisar su perfil efectivo y cerrar sus sesiones.</p><Link className="mt-4 inline-flex text-sm font-black text-[var(--erp-brand-red)] hover:underline" to="/admin/employees">Ir a empleados</Link></section>}
            <section className="rounded-[1.4rem] border border-[color:var(--erp-border)] bg-white p-5 shadow-[var(--erp-shadow)]"><div className="flex items-center gap-2"><History className="h-4 w-4 text-[var(--erp-brand-gold-deep)]" /><h2 className="font-black">Últimos cambios</h2></div><div className="mt-4 grid gap-3">{audit.items.length === 0 ? <p className="text-sm text-[var(--erp-muted-foreground)]">Aún no hay movimientos de acceso.</p> : audit.items.slice(0, 5).map((item) => <div className="border-l-2 border-[var(--erp-brand-gold)] pl-3" key={item.id}><p className="font-mono text-[11px] font-bold">{item.action}</p><p className="mt-1 text-xs text-[var(--erp-muted-foreground)]">{item.actor.name} · {formatDate(item.createdAt)}</p><p className="mt-1 text-xs leading-5">{item.reason}</p></div>)}</div></section>
          </aside>
        </section>
      </div>
    </main>
