...
export function DailyCloseHeader({ close }: { close: DailyClose }) {
  const cashStatus = close.cashSessionStatus ?? (close.status === 'DRAFT' ? 'OPEN' : 'CLOSED')
  const terminalIdentifier = close.terminalIdentifier || 'Caja 01'
  return <header className="sticky top-2 z-20 overflow-hidden rounded-2xl border border-[var(--erp-border)] bg-[color:var(--erp-surface-elevated)]/95 p-4 shadow-md backdrop-blur-md">
    <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
      <div className="min-w-0">
        ...
        <h2 className="mt-1 truncate text-xl font-black tracking-tight">{terminalIdentifier} · {close.operationalLocation.name}</h2>
        <p className="mt-1 text-xs text-[var(--erp-muted-foreground)]">Sesión monetaria y cierre operativo · {close.operationalLocation.code ?? 'Sin código de sucursal'}</p>
      </div>
      <dl className="grid grid-cols-2 gap-x-5 gap-y-3 sm:grid-cols-3 lg:grid-cols-7">
        ...
        <HeaderValue icon={Hash} label="Caja/terminal" value={terminalIdentifier} />
        <HeaderValue icon={UserRound} label="Cajero" value={close.openedBy?.name ?? 'No disponible'} />
        <HeaderValue icon={Clock3} label="Turno abierto" value={close.openedAt ? time(close.openedAt) : 'No disponible'} />
        <HeaderValue icon={GitBranch} label="Estado caja" value={cashStatus === 'OPEN' ? 'Abierta' : 'Cerrada'} />
        <HeaderValue icon={Hash} label="Fondo inicial" value={money(close.initialCashFund)} />
    ...
    <div className="mt-3 flex items-center justify-between border-t border-[var(--erp-border)] pt-3 text-xs text-[var(--erp-muted-foreground)]">
      <span>Sesión {cashStatus === 'OPEN' ? 'abierta' : 'cerrada'} · Cierre operativo: {statusLabel[close.status]} · Depósito inicial {money(close.initialCashIn)} · Retiro inicial {money(close.initialCashOut)}</span>
