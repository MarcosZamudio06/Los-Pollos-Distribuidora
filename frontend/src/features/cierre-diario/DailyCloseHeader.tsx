import { CalendarDays, Clock3, GitBranch, Monitor, UsersRound, type LucideIcon } from 'lucide-react'
  ...
  return <header className="relative z-0 overflow-hidden rounded-2xl border border-[var(--erp-border)] bg-[color:var(--erp-surface-elevated)]/95 p-4 shadow-md backdrop-blur-md">
    <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
      <div className="min-w-0">
        ...
        <h2 className="mt-1 truncate text-xl font-black tracking-tight">{close.operationalLocation.name} · {date(close.businessDate)}</h2>
        <p className="mt-1 text-xs text-[var(--erp-muted-foreground)]">Consolidado diario de sucursal · {close.operationalLocation.code ?? 'Sin código de sucursal'}</p>
      </div>
      <dl className="grid grid-cols-2 gap-x-5 gap-y-3 sm:grid-cols-3 lg:grid-cols-6">
        ...
        <HeaderValue icon={Monitor} label="Terminales" value={String(terminalCount)} />
        <HeaderValue icon={UsersRound} label="Turnos" value={`${shifts.length} · ${openShiftCount} abiertos`} />
        <HeaderValue icon={GitBranch} label="Estado cierre" value={statusLabel[close.status]} />
