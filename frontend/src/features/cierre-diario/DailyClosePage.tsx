...
import { canUseLocationForDailyClose, canValidateDailyClose, type DailyClose } from './types'
...
export function DailyClosePage() {
  ...
  const load = async () => { try { setLoading(true); const data = await dailyCloseService.list(accessToken); setItems(data); setSelected((current) => data.find((item) => item.id === current?.id) ?? data[0] ?? null) } catch (error) { toast.error(error instanceof Error ? error.message : 'No fue posible cargar los cierres.') } finally { setLoading(false) } }
  useEffect(() => { void load() }, [accessToken])
  const run = async (operation: () => Promise<DailyClose>, message: string) => { try { const close = await operation(); setSelected(close); await load(); toast.success(message) } catch (error) { toast.error(error instanceof Error ? error.message : 'No fue posible completar la operación.') } }
  ...
  const transition = (action: 'validate' | 'review' | 'close' | 'cancel' | 'reopen') => { if (!selected || (action === 'validate' && !canValidateDailyClose(selected.status))) return; const reasoned = action === 'cancel' || action === 'reopen'; const reason = reasoned ? window.prompt(`Motivo para ${action === 'cancel' ? 'cancelar' : 'reabrir'} el cierre:`) : undefined; if (reasoned && !reason?.trim()) return; void run(() => dailyCloseService.action(selected.id, action, { version: selected.version, ...(reason ? { reason } : {}) }, accessToken), 'Estado del cierre actualizado.') }
  ...
  return <PageContainer className="space-y-6">
    ...
    <div className="grid gap-6 xl:grid-cols-[280px_minmax(0,1fr)]">
      ...
      {!selected ? <section className="grid min-h-80 place-items-center rounded-2xl border border-dashed border-[var(--erp-border)]"><p className="text-[var(--erp-muted-foreground)]">Abre o selecciona un cierre para comenzar.</p></section> : <section className="space-y-5">
        ...
        <div className="grid gap-4 lg:grid-cols-2"><article className="rounded-2xl border border-[var(--erp-border)] bg-[var(--erp-surface-elevated)] p-5"><h3 className="flex items-center gap-2 font-bold"><Banknote size={18}/> Ingresos y utilidad</h3><dl className="mt-4 grid grid-cols-2 gap-3 text-sm">{[['Efectivo',money(selected.cashTotal)],['Vouchers',money(selected.cardVoucherTotal)],['Transferencias',money(selected.transferTotal)],['Gastos',money(selected.expenseTotal)],['Utilidad bruta',money(selected.grossProfitTotal)],['Utilidad neta',money(selected.netProfitTotal)]].map(([k,v])=><div key={k} className="rounded-xl bg-[var(--erp-surface-muted)] p-3"><dt className="text-[var(--erp-muted-foreground)]">{k}</dt><dd className="mt-1 font-bold tabular-nums">{v}</dd></div>)}</dl></article><article className="rounded-2xl border border-[var(--erp-border)] bg-[var(--erp-surface-elevated)] p-5"><h3 className="flex items-center gap-2 font-bold"><Scale size={18}/> Existencia conciliada</h3><dl className="mt-4 space-y-3">{[['Existencia restante',selected.totalRemainingKg],['Faltante',selected.totalShortageKg],['Sobrante',selected.totalSurplusKg]].map(([k,v])=><div key={k} className="flex justify-between border-b border-[var(--erp-border)] pb-2 text-sm"><dt>{k}</dt><dd className="font-bold tabular-nums">{Number(v).toFixed(3)} kg</dd></div>)}</dl></article></div>
