...
export function DailyClosePage() {
  ...
  const canViewCosts = user?.role === 'ADMIN' || user?.role === 'SELLER'
  ...
  const recordCashCount = (event: FormEvent) => { event.preventDefault(); if (!selected || !cashCountedTotal.trim()) return; void run(() => dailyCloseService.recordCashCount(selected.id, { cashCountedTotal: Number(cashCountedTotal) }, accessToken), 'Efectivo contado registrado.'); setCashCountedTotal('') }
    ...
    <header className="relative overflow-hidden rounded-[1.75rem] border border-[var(--erp-border)] bg-[var(--erp-surface-elevated)] p-6 shadow-sm">
      ...
      <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
        ...
        <form className="grid gap-2 sm:grid-cols-[minmax(220px,1fr)_160px_auto]" onSubmit={open}>
          ...
          <Input aria-label="Fecha operativa" max={today} type="date" value={businessDate} onChange={(e) => setBusinessDate(e.target.value)} />
