...
export function CashShiftSummary({
  ...
  return (
    <section className="overflow-hidden rounded-2xl border border-[var(--erp-border)] bg-[var(--erp-surface-elevated)]">
      <header className="flex flex-col gap-2 border-b border-[var(--erp-border)] px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-xs text-[var(--erp-muted-foreground)]">
            Cada turno conserva su cajero, fondo, conteo y diferencia.
          </p>
        </div>
        <strong className="text-sm">
          {shifts.length} turno(s) · {openShifts.length} abiertos
        </strong>
      </header>
      {openShifts.length > 0 && (
        <div
          className="border-b border-[var(--erp-border)] bg-[rgba(214,155,45,0.08)] p-4"
          role="alert"
        >
          <div className="flex items-start gap-3">
            <AlertTriangle
              className="mt-0.5 shrink-0 text-[var(--erp-warning)]"
              size={18}
            />
            <div>
              <h4 className="font-black">Turnos abiertos</h4>
              <p className="mt-1 text-sm text-[var(--erp-muted-foreground)]">
                Cierra cada turno y revisa su diferencia antes de finalizar la
                jornada.
              </p>
            </div>
          </div>
          <div className="mt-4 grid gap-3">
            {openShifts.map((shift) => {
              ...
              const administrative =
                !canCloseNormally && canAdministrativelyClose;
              const canClose = canCloseNormally || administrative;
              return (
                <article
                  className="rounded-xl border border-[var(--erp-border)] bg-[var(--erp-surface-elevated)] p-4"
                  key={shift.id}
                >
                  <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                    <div>
                      <p className="font-black">
                        {shift.terminal.name}{" "}
                        <span className="font-mono text-xs font-normal text-[var(--erp-muted-foreground)]">
                          {shift.terminal.code}
                        </span>
                      </p>
                      <p className="mt-1 text-sm text-[var(--erp-muted-foreground)]">
                        {shift.cashier.name} · Abierto{" "}
                    ...
                    {canClose ? (
                      <form
                        className="grid gap-2 lg:min-w-[440px] lg:grid-cols-[1fr_1.4fr_auto] lg:items-end"
                        onSubmit={(event) => {
                          event.preventDefault();
                          void submit(shift, administrative).catch(
                            () => undefined,
                          );
                        }}
                      >
                        <label className="grid gap-1 text-xs font-bold text-[var(--erp-muted-foreground)]">
                          Efectivo contado
                          <Input
                            aria-label={`Efectivo contado de ${shift.terminal.name}`}
                            min="0"
                            onChange={(event) =>
                              setCounts((current) => ({
                                ...current,
                        ...
                        {administrative ? (
                          <label className="grid gap-1 text-xs font-bold text-[var(--erp-muted-foreground)]">
                            ...
                            <Input
                              aria-label={`Motivo administrativo de ${shift.terminal.name}`}
                              onChange={(event) =>
                                setReasons((current) => ({
                                  ...current,
                                  [shift.id]: event.target.value,
                              ...
                              placeholder="Terminal inaccesible o turno abandonado"
                              required
                              value={reasons[shift.id] ?? ""}
                            />
                          </label>
                        ) : (
                        ...
                        <Button
                          ...
                          type="submit"
                          variant={administrative ? "secondary" : "primary"}
                        >
                      ...
                      <p className="flex items-center gap-2 text-sm font-semibold text-[var(--erp-muted-foreground)]">
                        <LockKeyhole size={16} /> El cajero debe cerrar desde su
                        terminal.
                      </p>
                    )}
                  </div>
      ...
      {historicalShifts.length === 0 ? (
        <p className="p-4 text-sm text-[var(--erp-muted-foreground)]">
          Aún no hay turnos cerrados en este cierre diario.
        </p>
      ) : (
              ...
              {historicalShifts.map((shift) => (
                ...
                >
                  <td className="px-4 py-3 font-bold">
                    {shift.terminal.name}
                    <span className="block font-mono text-xs font-normal text-[var(--erp-muted-foreground)]">
                      {shift.terminal.code}
                    </span>
                  </td>
                  ...
                  <td className="px-4 py-3 text-xs">
                    {shift.status === "CANCELLED" ? (
                    ...
                    ) : shift.closeMode === "ADMINISTRATIVE" ? (
                      <span className="font-semibold text-[var(--erp-warning)]">
                        Administrativo
                        {shift.closeReason ? (
                          <span className="block text-[var(--erp-muted-foreground)]">
                            {shift.closeReason}
                          </span>
                        ) : null}
                      </span>
                      ...
                      <span className="flex items-center gap-1 text-emerald-700">
                        <CheckCircle2 size={14} /> Cajero
                      </span>
                    )}
                  </td>
                  <td className="px-4 py-3 text-right font-mono">
                    {money(shift.initialCashFund)}
                  </td>
                  <td className="px-4 py-3 text-right font-mono">
                    {shift.cashCountedTotal == null
                      ? "Pendiente"
                      : money(shift.cashCountedTotal)}
                  </td>
