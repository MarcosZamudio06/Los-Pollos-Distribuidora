...
export const dailyCloseService = {
  ...
  open: async (body: { operationalLocationId: string; businessDate: string; terminalIdentifier: string; initialCashFund: number; initialCashIn: number; initialCashOut: number; notes?: string }, token: string | null) => (await apiClient.post<Envelope<DailyClose>, typeof body>('/point-of-sale-daily-closes', { body, headers: headers(token) })).data,
  expense: async (id: string, body: { cashShiftId: string; deviceId: string; amount: number; reason: string; reference?: string }, token: string | null, idempotencyKey: string) => (await apiClient.post<Envelope<DailyClose>, typeof body>(`/point-of-sale-daily-closes/${id}/expenses`, { body, headers: idempotencyHeaders(token, idempotencyKey) })).data,
  ...
  justifyDifference: async (id: string, differenceId: string, body: { version: number; reason: string; evidence: string }, token: string | null) => (await apiClient.patch<Envelope<DailyClose>, typeof body>(`/point-of-sale-daily-closes/${id}/differences/${differenceId}/justify`, { body, headers: headers(token) })).data,
  authorizeDifference: async (id: string, differenceId: string, body: { version: number }, token: string | null) => (await apiClient.patch<Envelope<DailyClose>, typeof body>(`/point-of-sale-daily-closes/${id}/differences/${differenceId}/authorize`, { body, headers: headers(token) })).data,
