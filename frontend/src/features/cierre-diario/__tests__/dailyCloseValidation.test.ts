...
const close = {
  ...
  initialCashOut: "0",
  version: 1,
  operationalLocation: { id: "location-1", name: "Sucursal Centro" },
  totalInputKg: "10",
  totalSoldKg: "5",
  totalRemainingKg: "5",
  totalShortageKg: "1.5",
  ...
  purchaseCostTotal: "60",
  grossProfitTotal: "40",
  netProfitTotal: "40",
  lastValidatedAt: null,
  costQuality: "ESTIMATED",
  dataAsOf: "2026-07-22T12:00:00.000Z",
  cashMovements: [],
  scaleTicketReferences: [
    { id: "ticket-1", physicalFolio: "B-10", weightKg: null, pieceCount: null },
  ],
  sales: [
    { saleNumber: "V-100", physicalFolio: null, documentType: "SIMPLE_NOTE" },
  ],
...
describe("daily close validation contract", () => {
  ...
  it("preserves invalid validation results instead of extracting only the close", async () => {
      ...
      errors: [
        {
          ...
          message: "Registra el efectivo contado.",
        },
      ],
      differences: [{ code: "CASH_DIFFERENCE", value: -20, unit: "MXN" }],
    };
    ...
    await expect(
      dailyCloseService.validate("close-1", "access-token"),
    ).resolves.toEqual(result);
    expect(vi.mocked(fetch)).toHaveBeenCalledWith(
      "/api/point-of-sale-daily-closes/close-1/validate",
  ...
  it("groups stock differences and operational warnings for the validation panel", () => {
    const result: DailyCloseValidationResult = {
      ...
      valid: false,
      errors: [],
      differences: [{ code: "SCALE_DIFFERENCE", value: -1, unit: "kg" }],
    };

    expect(validationDifferences(result).map((item) => item.code)).toEqual([
      "SCALE_DIFFERENCE",
      "SHORTAGE",
      "SURPLUS",
    ]);
    expect(validationWarnings(close).map((item) => item.code)).toEqual([
      "ESTIMATED_COST",
      "MISSING_FOLIO_V-100",
      "INCOMPLETE_SCALE_REFERENCE_ticket-1",
    ]);
    expect(validationWarnings(close, false).map((item) => item.code)).toEqual([
      "MISSING_FOLIO_V-100",
      "INCOMPLETE_SCALE_REFERENCE_ticket-1",
    ]);
  });
