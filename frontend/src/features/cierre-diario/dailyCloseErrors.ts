...
const operationalMessages: Record<string, string> = {
  ...
  CASH_SHIFT_ALREADY_OPEN: "Ya existe un turno abierto en esta terminal.",
  CASH_SHIFT_CANCELLED: "Los turnos cancelados no pueden reabrirse.",
  CASH_SHIFT_NOT_CLOSED: "Solo puedes reabrir turnos cerrados.",
  ...
  DAILY_CLOSE_NOT_EDITABLE:
    "El cierre diario ya no está en borrador. Reábrelo antes de modificar el turno.",
...
export function dailyCloseErrorMessage(error: unknown, fallback: string) {
  ...
  if (error instanceof ApiClientError && error.message === "Invalid credentials")
