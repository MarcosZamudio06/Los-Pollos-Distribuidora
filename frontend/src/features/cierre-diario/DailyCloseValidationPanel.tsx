...
function Group({
  ...
  return (
    <article className="rounded-xl border border-[var(--erp-border)] bg-[var(--erp-surface-elevated)] p-4">
...
export function DailyCloseValidationPanel({
  canViewProfit = true,
  result,
}: {
  ...
  const warnings = validationWarnings(result.close, canViewProfit);
  return (
    <section
      aria-live="polite"
      className={`rounded-2xl border p-5 ${result.valid ? "border-emerald-300 bg-emerald-50/70" : "border-amber-400 bg-amber-50/70"}`}
    >
      <div className="flex gap-3">
        ...
        <div>
          ...
          <p className="mt-1 text-sm">
            {result.valid
              ? "No hay bloqueantes para avanzar a revisión."
              : "Corrige los bloqueantes antes de marcar el cierre como revisado."}
          </p>
        </div>
      </div>
      <div className="mt-4 grid gap-3 lg:grid-cols-3">
        <Group
          empty="No hay bloqueantes."
          items={result.errors}
          title="Bloqueantes"
        />
        <Group
          empty="No hay diferencias detectadas."
          items={differences}
          title="Diferencias"
        />
        <Group
          empty="No hay advertencias."
          items={warnings}
          title="Advertencias"
        />
      </div>
      {!result.valid && (
        <div className="mt-4 flex items-center gap-2 text-sm font-semibold text-amber-900">
          <AlertTriangle size={16} /> El cierre no está validado.
        </div>
      )}
    </section>
  );
