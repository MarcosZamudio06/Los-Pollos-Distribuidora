...
export function validationDifferences(result: DailyCloseValidationResult): ValidationItem[] {
  const differences = result.differences.map((difference) => ({
    ...
    message: difference.expectedValue === undefined
...
export function validationWarnings(close: DailyCloseValidationResult['close'], canViewProfit = true): ValidationItem[] {
  return [
    ...(canViewProfit && close.costQuality === 'ESTIMATED' ? [{ code: 'ESTIMATED_COST', message: 'El costo del producto es estimado.' }] : []),
...
export function DailyCloseValidationPanel({ canViewProfit = true, result }: { canViewProfit?: boolean; result: DailyCloseValidationResult }) {
