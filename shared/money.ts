...
export class Money {
  ...
  static from(value: DecimalInput | Money | null | undefined): Money {
    ...
    const parsed = parseDecimal(value)
    return new Money(roundFraction(parsed.coefficient * 100n, pow10(parsed.scale)))
  }
  ...
  multiply(factor: DecimalInput): Money {
    ...
    return new Money(roundFraction(this.cents * parsed.coefficient, pow10(parsed.scale)))
  }

  percentage(percentage: DecimalInput): Money {
    const parsed = parseDecimal(percentage)
    return new Money(roundFraction(this.cents * parsed.coefficient, pow10(parsed.scale) * 100n))
  }

  divide(divisor: DecimalInput): Money {
    const parsed = parseDecimal(divisor)
    if (parsed.coefficient === 0n) throw new Error('Cannot divide money by zero')
    return new Money(roundFraction(this.cents * pow10(parsed.scale), parsed.coefficient))
  }
  ...
  allocate(weights: DecimalInput[]): Money[] {
    ...
    const totalWeight = parsedWeights.reduce((sum, weight) => sum + weight.coefficient * pow10(Math.max(...parsedWeights.map((item) => item.scale)) - weight.scale), 0n)
    if (totalWeight <= 0n) return weights.map(() => Money.zero())

    const commonScale = Math.max(...parsedWeights.map((weight) => weight.scale))
    const denominator = totalWeight
    const allocations = parsedWeights.map((weight) => {
      const numerator = this.cents * weight.coefficient * pow10(commonScale - weight.scale)
      const floor = numerator >= 0n ? numerator / denominator : -((-numerator) / denominator)
...
export function toMoneyString(value: DecimalInput | Money | null | undefined): string {
